...
!       Reduce level if modification factor is too large.
    DO 20 K = 2,10
        IF (TK*FLOAT(ISLOW(ISBIN)) < DT .OR. ISBIN == 1) GO TO 30
        ISBIN = ISBIN - 1
    20 END DO

!       Exit if level is unchanged.
    30 IF (ISBIN == IMOD) GO TO 100
...
!       Note: explicit relation agrees with iterated value (bug fix 9/99).
    ...
    R(IPAIR))*DTU
    ITER = ITER + 1
    IF (ABS(DT - DT1) > 1.0D-10*STEP(I1) .AND. ITER < 5) GO TO 40
