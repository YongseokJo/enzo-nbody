...
!       Check merger limit.
    IF (NMERGE >= MMAX .AND. rank == 0) THEN
        WRITE (6,5)  NMERGE
...
!       Use double hierarchy procedure if one binary is already a merger.
    IF (NAME(N+KSPAIR) < 0 .OR. NAME(JCOMP) < 0) THEN
        CALL MERGE2
        GO TO 100
    END IF
...
!       Check optional diagnostics for hierarchy.
    IF ((KZ(18) == 1 .OR. KZ(18) == 3) .AND. KSTAR(I) <= 20) THEN
        CALL HIARCH(IPAIR)
    END IF
...
!       Set current energy of any second perturbed binary (from XVPRED).
    IF (JCOMP > N) THEN
        CALL XVPRED(JCOMP,-1)
        JPAIR = JCOMP - N
        IF (LIST(1,2*JPAIR-1) > 0) H(JPAIR) = HT
...
!       Save the neighbours for correction procedure & rename if moved up.
    ...
    DO 15 L = 1,NNB
        J = LIST(L+1,I)
        IF (J > I) J = J - 1
        IF (J <= 2*NPAIRS .AND. J > 2*IPAIR) J = J - 2
        JSAVE(L) = J
    15 END DO
...
!     Remove ghost from NXTLST and add into ghost list
    IF(TIME+TOFF /= 0.D0) call delay_remove_tlist(JCOMP1,STEP,DTK)
    STEP(JCOMP1) = 2*DTK(1)
    IF(TIME+TOFF /= 0.D0) call add_tlist(JCOMP1,STEP,DTK)

!       Define negative c.m. name for identification & termination.
    NAME(NTOT) = NZERO - NAME(NTOT)
!     --03/04/14 13:25-lwang-debug--------------------------------------*
!**** Note:------------------------------------------------------------**
!$$$      print*,rank,'MERGER NTOT',NTOT,'NAME',NAME(NTOT),'S',step(ntot),
!$$$     &     't',time,'t0',t0(ntot),'stepr',stepr(ntot),
!$$$     &     'f',f(1,ntot),'fdot',fdot(1,ntot),'nb',list(1,ntot)
!$$$      call flush(6)
