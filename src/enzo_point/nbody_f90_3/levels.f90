...
!       Print histograms of block-steps (STEPR with KZ(33) > 1).
    ...
    if(rank == 0)then
        ...
        IF (KZ(33) > 1)WRITE (6,301)  (IHISTR(J),J=1,JMAXR)
        301 FORMAT (' STEP R ',50I7,(/,24I5))
        IF (KZ(8) > 0 .OR. NBIN0 > 0 )WRITE (6,3001) &
        (IHISTU(J),J=1,JMAXU)
...
!       presently   4 to 1024 in powers of 2.
    ...
    !       (IYE, JHISTR do the same for the regular steps)
        ...
        DO 37 I = 1,NMAX
            IF(JHIST(I) == 0)GOTO 37
            J = MOD(I,IPES)
            K = I/IPES
            IF(J > 0)IY(J)=IY(J)+JHIST(I)
            IF(K > 0)IY(IPES)=IY(IPES)+K*JHIST(I)
        37 END DO
    
        DO 371 I = 1,NMAX
            IF(JHISTR(I) == 0)GOTO 371
            J = MOD(I,IPES)
            K = I/IPES
            IF(J > 0)IYR(J)=IYR(J)+JHISTR(I)
            IF(K > 0)IYR(IPES)=IYR(IPES)+K*JHISTR(I)
        371 END DO
