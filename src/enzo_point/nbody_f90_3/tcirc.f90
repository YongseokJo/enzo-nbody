...
!     Set large circularization time for merged binary.
    IF (RADIUS(I1) == 0.0D0 .OR. RADIUS(I2) == 0.0D0) THEN
        TC = 1.0D+10
        GO TO 30
    END IF
...
!     Include an appropriate numerical factor (Chris Tout 07/08).
    ...
    FF = 1.0
    IF (KSTAR(J1) == 1 .AND. BODY(J1)*SMU > 1.25) THEN
        FF = 100.0
    ELSE IF (KSTAR(J1) == 4 .OR. KSTAR(J1) == 7) THEN
        FF = 100.0
    ELSE IF (KSTAR(J1) >= 10) THEN
        FF = 1.0D+17/AR**2
    END IF
    Q12 = BODY(J1)/BODY(J2)
    TAUC = FF*2.0*Q12**2/(1.0 + Q12)*AR**8
!     Exit on TC > 1.0D+08 yr (Chris Tout, Cambody Book 2008).
    IF (TAUC > 1.0D+08) THEN
        TC = 1.0D+10
        GO TO 30
    END IF
...
!     Adopt WD scaling for any NS to avoid numerical problem.
    IF (KSTAR(I1) == 13 .OR. KSTAR(I2) == 13) THEN
        CONST = 1.0D-04*CONST
    END IF
...
!     Activate tidal indicator if TC < 2x10**9 yrs or hyperbolic orbit.
    IF (TC < 2000.0 .OR. ES0 > 1.0) THEN
        IP = KVEC(I1)
        ITIME = ITIME + 1
    !     Note possibility of counter exceeding the limit.
        IF (ITIME > 2000000000) ITIME = 0
        IF (ICIRC == 0 .AND. KZ(27) == 2 .AND. ITIME < 100) THEN
    ...
    !     Define Roche search indicator for circularized orbit (ECCM1 > 0.002).
        IF (ES0 <= ECCM1 .AND. KSTAR(N+IP) == 0) THEN
            KSTAR(N+IP) = 10
