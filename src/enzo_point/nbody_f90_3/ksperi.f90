...
!       Avoid nearly circular orbits (undefined pericentre).
    IF (ECC < 0.0001) GO TO 30

!       Distinguish between near-collision, elliptic & hyperbolic case.
    Q = (PSI/ECC)**2
    IF (ZETA > 0.0D0 .AND. ABS(Q/SEMI) < 0.1) THEN
...
!       Integrate backwards for perturbed motion (reflection gives errors).
    IF (LIST(1,I1) > 0 .AND. ITIME == 0 .AND. DT > STEP(I1)) THEN
        TIME = TIME0
        IMOD = KSLOW(IPAIR)
    ...
    !       Determine the regularized step by Newton-Raphson iteration (DT > 0).
        ...
        DT1 = ZMOD*DT1
        ITER = ITER + 1
        IF (ABS(DT - DT1) > 1.0D-10*STEP(I1) .AND. ITER < 5) GO TO 20
...
!       Predict c.m. coordinates & velocities.
    IF (ABS(DT) < STEP(ICM) .AND. TIME > 0.0) THEN
        CALL JPRED(ICM,time,time)
    END IF

    30 RETURN

    END SUBROUTINE KSPERI
