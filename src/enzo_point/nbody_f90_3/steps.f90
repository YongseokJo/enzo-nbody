...
!       Set new steps and initialize prediction variables.
    ...
    !**** Note:------------------------------------------------------------**
    !$$$         IF(I.EQ.12534.and.rank.eq.0) then
    !$$$             print*,'I',I,'T',time,'TB',tblock,'DT',dt,'DTN',dtn
    !$$$          end if
    !     --11/22/13 20:26-lwang-end----------------------------------------*
        ...
        !       Reduce steps by factor 2 until commensurate with current time.
            ...
            STEPR(I) = DTRN
            ITER = 0
            10 IF (DMOD(TIME,STEP(I)) /= 0.0D0) THEN
                STEP(I) = 0.5D0*STEP(I)
                ITER = ITER + 1
                IF (ITER < 16 .OR. STEP(I) > DTK(40)) GO TO 10
                STEP(I) = DTK(40)
                if(rank == 0) &
            ...
            END IF
            ITER = 0
            18 IF (DMOD(TIME,STEPR(I)) /= 0.0D0) THEN
                STEPR(I) = 0.5D0*STEPR(I)
                ITER = ITER + 1
                IF (ITER < 16 .OR. STEPR(I) > DTK(40)) GO TO 18
                STEPR(I) = DTK(40)
                if(rank == 0) &
