...
!       Begin dual purpose force loop (all RIJ2 < RCM2, J > N or I <= N).
    ...
    !       Decide appropriate expressions from perturber comparison.
        IF (J /= JP) THEN
            IF (J <= N) GO TO 30
            IF (RIJ2 > CMSEP2*R(J-N)**2) GO TO 30
            KDUM = 2*(J - N) - 1
            IF (LIST(1,KDUM) > 0) THEN
                K = KDUM
                J2 = K + 1
                GO TO 50
            END IF
