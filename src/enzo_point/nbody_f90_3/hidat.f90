...
!     ISTAB = 0
    DO 30 JPAIR = 1,NPAIRS
        ICM = N + JPAIR
        IF (NAME(ICM) > 0 .AND. BODY(ICM) > 0.0D0) GO TO 30
        J2 = 2*JPAIR
        J1 = J2 - 1
    !       Determine merger & ghost index (delay ghost c.m.).
        CALL FINDJ(J1,J,IM)
        KCM = KSTARM(IM)
        IF (BODY(ICM) > 0.0D0) THEN
    ...
    !       Consider any multiple hierarchies first (c.m. mass > 0 also here).
        ...
        !       Perform stability check.
            IF (PMIN*(1.0 - GAMMA(JPAIR)) < PCR .AND. E1 < 0.96 .AND. &
            LIST(1,J1) > 0) THEN
            !                 ISTAB = JPAIR
            !                  if(rank.eq.0)
