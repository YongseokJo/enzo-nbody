...
!       Ensure at least one perturber first time (max 5 tries except CHAOS).
    IF (NNB1 == 1 .AND. IPHASE > 0 .AND. NNB2 > 1 .AND. TIME > 0.0) THEN
        RCRIT2 = 2.0*RCRIT2
        RCRIT3 = 2.0*RCRIT3
        IT = IT + 1
    !       Skip repeat for small size (next KSLIST requires many periods).
        IF ((SEMI*SU > 10.0 .AND. IT <= 5) .OR. KSTAR(I) == -1) GO TO 6
    END IF
