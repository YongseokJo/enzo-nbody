...
!     Exit if binary is not identified but include updating of small ECC.
    ...
    !     Check enforcement of circularization for small TCIRC.
        IF (ECC < 0.10 .AND. LIST(1,I1) == 0) THEN
            ICIRC = -1
            QPERI = A0*(1.0 - ECC)
...
!     Rectify elements after possible common envelope, Roche or GR process.
    QPERI = A0*(1.0 - ECC)
    IF (ES0 > 0.01 .OR. ABS(QPERI-RP0) > 0.02*QPERI) THEN
        DT = TIME - TOSC(IC)
        IF (ITIME < 50) THEN
            if(rank == 0) &
...
!     Obtain stellar parameters for evolving stars at current epoch.
    IF (KSTAR(J1) >= 2 .AND. KSTAR(J1) <= 9) THEN
        KW = KSTAR(J1)
        M1 = BODY(J1)*SMU
        M0 = BODY0(J1)*ZMBAR
        MC = 0.D0
...
!     Define oscillation period (dimensionless time) and damping constants.
    ...
    !     Specify polytropic index for each star (n = 3, 2 or 3/2).
        ...
        ELSE
            ...
            W(K) = WW(IP)
            Q(K) = QQ(IP)
            IF (KSTAR(IK) <= 2 .OR. KSTAR(IK) == 7) THEN
                rg2(k) = 0.1
            ELSE IF (KSTAR(IK) == 4) THEN
...
!     Set synchronous spin for degenerate stars.
    ...
    IF (KSTAR(J1) >= 10) spin1 = omeq
    IF (KSTAR(J2) >= 10) spin2 = omeq
    DS1 = (spin1 - omeq)/omeq
    DS2 = (spin2 - omeq)/omeq

!     Skip integration if both spins are synchronous.
    IF (ABS(DS1) < 0.01 .AND. ABS(DS2) < 0.01) THEN
        KX = MAX(KSTAR(J1),KSTAR(J2))
        IF (ABS(DS20) > 0.01 .AND. KX < 10) THEN
            if(rank == 0) &
...
!     &        SUM2,SUM3,SUM4,DS
! 21      FORMAT ('K*(I1) K*(I2) M1 A S1 S2 S3 S4 DS ',
!     &        2I4,F8.4,1P,E12.4,4E10.2,0P,F6.1)
!         CALL FLUSH(92)
!      END IF
...
!     Include change in moment of inertia for rapid evolution of primary.
    ...
    !     Include safety check on small time interval (SJA 4/09).
        IF (DT <= 1.0D-10) DT = ABS(TEV(J1) - TIME)
...
!     Increase number of steps on slow primary rotation.
    IF (DS1 < -0.2) nstep = nstep + 10
    IF (DS1 < -0.8) nstep = nstep + 50
    IF (DS1 < -0.9) nstep = nstep + 50
    DT = TIME - TIME0
...
!     Reduce integration interval for slow evolution or after merger.
    IF (DT > 0.5) THEN
        ...
        IF (ABS(DS2) > 0.05 .OR. DS10 < -0.99) THEN
            DT = 0.1*DT
            nstep = nstep + 50
        END IF
...
!     Re-scale the semi-major axis and angular velocities to N-body units.
    ...
    ITT = ITT + 1
!     IF (ITT.GE.3) STOP
    IF (spin1 < 0.0 .OR. spin2 < 0.0) THEN
        if(rank == 0) &
    ...
    IF ((DS10 < 0.0 .AND. DS1 > 0.01) .OR. &
    (DS20 < 0.0 .AND. DS2 > 0.01)) THEN
        ITER = ITER + 1
        nstep = nstep + 50
        IF (ITER > 1) DT = 0.5*DT
        IF (ITER < 4) GO TO 40
    ...
    IF ((DS1 > 0.0 .AND. DS10 < -0.01) .OR. &
    (DS2 > 0.0 .AND. DS20 < -0.01)) THEN
        ITER = ITER + 1
        nstep = nstep + 50
        IF (ITER > 1) DT = 0.5*DT
        IF (ITER < 4) GO TO 40
...
!     Convert back to angular momenta.
    50 IF (KSTAR(J1) <= 2 .OR. (KSTAR(J1) >= 7 .AND. KSTAR(J1) /= 9)) THEN
...
!     Prevent new SEMI < R.
    IF (H(IPAIR) + DH < -0.5*BODY(I)/R(IPAIR)) THEN
...
!     Save final reference values.
    ...
    IF (ICOUNT <= 100 .AND. ABS(DS1) > 0.01) THEN
        ICOUNT = ICOUNT + 1
        DT = TEV(I) - TIME
        if(rank == 0) &
    ...
    IF (spin1 < 0.0) THEN
        ...
        I6,I4,1P,2E10.2)
        IF (SPIN1 < 0.0) SPIN1 = 0.1*omeq
        IF (SPIN2 < 0.0) SPIN2 = 0.1*omeq
        ITRY = ITRY + 1
        IF (ITRY <= 2) GO TO 50
    !     STOP
    END IF
