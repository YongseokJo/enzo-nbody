...
!       Search for global index of escaper.
    DO 40 J = IFIRST,NTOT
        IF (NAME(J) == NAMEC(IESC)) THEN
            I = J
            IF (rank == 0 .AND. BODY(J) > 0.0D0) &
...
!       Identify global index of new reference body (including binary c.m.).
    DO 45 J = IFIRST,NTOT
        IF (NAME(J) == NAMEC(NEW)) THEN
            ICH = J
        !*     Replace old c.m. index to new index in NXTLST
        !              call exchange_tlist(I,ICH,STEP,DTK)
            GO TO 50
        END IF
    45 END DO

!       Include warning if no reference body (this should not occur).
    if(rank == 0) WRITE (6,48)  IESC, NAMEC(NEW)
