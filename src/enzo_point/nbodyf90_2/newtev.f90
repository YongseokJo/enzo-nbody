...
!       Skip increase of smallest look-up time above 10 Myr.
    IF ((TEV(IX) - TIME)*TSTAR > 10.0) THEN
        GO TO 20
    END IF

!       Distinguish between ghost, single star or KS component.
    IF (BODY(IX) == 0.0D0) THEN
        CALL FINDM(IX,ITERM,MG)
        IF (ITERM < 0) GO TO 20
        M1 = MG*SMU
    ELSE IF (IX >= IFIRST) THEN
        M1 = BODY(IX)*SMU
    ELSE
        ITER = ITER + 1
        IPAIR = KVEC(IX)
        IF (NAME(N+IPAIR) > 0) THEN
            M1 = BODY(IX)*SMU
        ELSE
        !       Find the merger index by identifying original c.m. name.
            IM = 0
            DO 10 K = 1,NMERGE
                IF (NAMEM(K) == NAME(N+IPAIR)) IM = K
            10 END DO
            IF (NAME(N+IPAIR) < -2*NZERO) THEN
