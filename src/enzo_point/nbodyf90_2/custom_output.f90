...
!     choose full or active particles output
    NLOOP = NXTLEN
    IF(KZ(46) == 3 .OR. KZ(46) == 4 .OR. TTOT == 0.0D0) THEN
        NLOOP = NXTLIMIT
    END IF
    ...
    !**** Note:------------------------------------------------------------**
    !$$$         IF (NXTLEN.GT.60.and.TTOT.NE.0.D0) THEN
    !$$$            RRMIN = 1.E6
    !$$$            I2 = IMINR(I)
    !$$$            DO L = 2, LIST(1,I)+1
    !$$$               K = LIST(L,I)
    ...
    !$$$               DO KK =1,3
    !$$$                  RRC = RRC + (X(KK,K)-X(KK,I))**2
    !$$$               END DO
    !$$$               IF (RRC.LT.RRMIN) THEN
    !$$$                  IIC = K
    !$$$                  RRMIN = RRC
    !$$$               END IF
    !$$$               IF (K.EQ.I2) RRI2 = RRC
    !$$$            END DO
    !$$$            IF(IIC.NE.I2.and.LIST(1,I).NE.0) THEN
    !$$$            print*,'J',J,'I1',I,'I2',I2,'IIC',IIC,'RM',RRMIN,'RI2',RRI2,
    !$$$     &              'NNB',LIST(1,I),'TIME',TIME,'RS',RS(I),'X',X(1:3,I)
    !$$$            call flush(6)
    !$$$            END IF
    !$$$            IF (J.GE.NXTLEN) call abort()
    !$$$         END IF
    !     --10/18/14 18:04-lwang-end----------------------------------------*
    
    !     --K.S. binary case--
        IF(I > N .AND. NAME(I) > 0) THEN
            N_BINARY = N_BINARY + 1
            IK = N_BINARY
            IPAIR = I - N
        ...
        !     Name
            if (GAMMA(IPAIR) < GALIMIT .AND. H(IPAIR) < 0) THEN
                NB_NAMC(IK) = NAME(I)
                NB_NAM1(IK) = NAME(J1)
                NB_NAM2(IK) = NAME(J2)
            !     Dynamic information
    ...
    !     --Merger case--
        ...
        !     Check possible quartet [[B,S],S] and quintet [[B,S],B], save two components seperately
            ...
            !     error messsage
                ...
                10 DO K = NXTLIMIT+1,NXTLIMIT+NGHOSTS
                    IF (NAME(NXTLST(K)) == NAMEG(IM)) THEN
                        IG = NXTLST(K)
                        GO TO 20
                    END IF
                END DO
        ...
        !     Check whether ipair is avaiable
            ...
            !               RDOT = XREL(1,IM2)*VREL(1,IM2) + XREL(2,IM2)*VREL(2,IM2)
            !     &              + XREL(3,IM2)*VREL(3,IM2)
    ...
    !     Nearest neighbor index
        I2 = IMINR(I)
        IF(I2 <= 0) I2 = I + 1
        12 IF(BODY(I2) == 0.D0 .AND. I2 <= N) THEN
            I2 = I2 + 1
            GO TO 12
        END IF
    ...
    !     escape the registered wide binary index or second component is multiple system
        IF(TMASK(I) == TTOT .OR. TMASK(I2) == TTOT &
         .OR. I2 > N .OR. NAME(I2) <= 0) THEN
            IF(TMASK(I) /= TTOT) THEN
                TMASK(I) = TTOT
                N_SINGLE = N_SINGLE + 1
        ...
        !     Check whether satisfy wide binary criterion
            IF (SEMI > 0.0 .AND. SEMI < 0.1*RSCALE) THEN
                IWBINC = IWBINC + 1
                N_BINARY = N_BINARY + 1
                IK = N_BINARY
...
!**** Note:------------------------------------------------------------**
    if(rank == 0) print*,'CUSTOM OUTPUT: TTOT',TTOT,'N_SINGLE', &
    N_SINGLE,'N_BINARY',N_BINARY,'N_MERGER',N_MERGER
!$$$      print*,'NS_NAM',NS_NAM(1:5),'S_M',S_M(1:5),'S_X1',S_X1(1:5),
!$$$     &     'S_V1',S_V1(1:5),'NS_KW',NS_KW(1:5),'S_RS',S_RS(1:5),
!$$$     &     'S_L',S_L(1:5),'S_TE',S_TE(1:5),'S_MC',S_MC(1:5),'S_RC',
!$$$     &     S_RC(1:5)
!     --10/25/14 22:16-lwang-end----------------------------------------*

    if(rank == 0) call output_single(TTOT,N_SINGLE,KZ(12),KZ(46))
!$$$     &     NS_NAM,
!$$$     &     S_M,S_X1,S_X2,S_X2,S_V1,S_V2,S_V3,
!$$$     &     S_F1,S_F2,S_F3,
!$$$     &     NS_KW,S_RS,S_L,S_TE,S_MC,S_RC)

    if(rank == 0) call output_binary(TTOT,N_BINARY,KZ(12),KZ(46))
!$$$     &     NS_NAMC,NS_NAM1,NS_NAM2,
!$$$     &     B_M1,B_M2,B_XC1,B_XC2,B_XC3,B_VC1,B_VC2,B_VC3,
!$$$     &     B_XR1,B_XR2,B_XR3,B_VR1,B_VR2,B_VR3,
!$$$C     &     B_FC1,B_FC2,B_FC3,
!$$$     &     B_A,B_ECC,B_P,
!$$$     &     NB_KWC,NB_KW1,NB_KW2,B_RS1,B_RS2,B_L1,B_L2,
!$$$     &     B_TE1,B_TE2,B_MC1,B_MC2,B_RC1,B_RC2)

    if(rank == 0) call output_merger(TTOT,N_MERGER,KZ(12),KZ(46))
!$$$     &     NM_NAMC,NM_NAM1,NM_NAM2,NM_NAM3,
!$$$     &     M_M1,M_M2,M_M3,M_XC1,M_XC2,M_XC3,M_VC1,M_VC2.M_VC3,
!$$$     &     M_XR01,M_XR02,M_XR03,M_VR01,M_VR02,M_VR03,
!$$$     &     M_XR11,M_XR12,M_XR13,M_VR11,M_VR12,M_VR13,
!$$$C     &     M_FC1,M_FC2,M_FC3,
!$$$     &     M_A0,M_A2,M_ECC0,M_ECC1,M_P0,M_P1,
!$$$     &     NM_KWC,NM_KW1,NM_KW2,NM_KW3,
!$$$     &     M_RS1,M_RS2,M_RS3,M_L1,M_L2,M_L3,M_TE1,M_TE2,M_TE3,
!$$$     &     M_MC1,M_MC2,M_MC3,M_RC1,M_RC2,M_RC3)
    ...
    if (NXTLEN == NXTLIMIT .OR. TTOT == 0.D0) then
...
!     Stellar evolution of single star
    ...
    end subroutine custom_store_single
...
!     Binary parameters
    ...
    end subroutine custom_store_binary
