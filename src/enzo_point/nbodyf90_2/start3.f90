...
!       Identify current global indices by searching all single particles.
    I = 0
    DO 102 J = IFIRST,N
        IF (NAME(J) == NAM1) ICOMP = J
        IF (NAME(J) == NAM2) JCOMP = J
        IF (NAME(J) == NAM3) I = J
    102 END DO

!       Identify global index for c.m. body.
    ICM = I
    IF (BODY(ICOMP) > 0.0D0) ICM = ICOMP
    IF (BODY(JCOMP) > 0.0D0) ICM = JCOMP
...
!       Add any other ghosts to perturber list for replacement of #ICM.
    DO 160 JSUB = 1,NSUB
        DO 155 L = 1,4
            IF (NAMES(L,JSUB) == 0) GO TO 155
            DO 154 J = 1,N
                IF (NAME(J) == NAMES(L,JSUB) .AND. BODY(J) <= 0.0D0) THEN
                    NP = NP + 1
                    JPERT(NP) = J
                    GO TO 155
                END IF
...
!       Exclude the dominant interaction for c.m. approximation (large FDOT).
    IF (R3**2 > CMSEP2*MIN(R1,R2)**2) THEN
        NNB = 1
    ELSE
        NNB = 3
    END IF
...
!       Loop over all single particles & c.m. but skip subsystem members.
    DO 210 L = 2,NNB2
        ...
        IF (RIJ2 < RP2) THEN
            IF (J == I) GO TO 210
            IF (J == ICOMP .OR. J == JCOMP) GO TO 210
            NP = NP + 1
            JPERT(NP) = J
        END IF
    210 END DO
