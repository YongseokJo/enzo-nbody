...
!     Check SMAX to make sure it have correct value
    if(rank == 0) THEN
        IF(SMAX > 1) THEN
            ...
            DTNSMAX = 1.0
            1 IF(SMAX/DTNSMAX <= 0.75) THEN
                DTNSMAX = 0.5D0*DTNSMAX
                IF(DTNSMAX > 1E-19) GO TO 1
            END IF
            SMAX = DTNSMAX
        END IF
    END IF
...
!      IDUM1 = NRAND
! NEW version (14.08.98, P.Kroupa):
!*       Set random number SEED for routine DATA.
    IDUM1 = -1*NRAND
!+++ Notify others of this change on log file:
!      if(rank.eq.0)then
!      write(6,*)
