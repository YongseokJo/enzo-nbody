...
!       Find the heaviest body (exclude any merged binary components).
    ...
    1 BMAX = 0.0D0
    DO 2 I = 1,N
        IF (BODY(I) < BMAX .OR. I <= IMAX) GO TO 2
        BMAX = BODY(I)
        IMAX = I
    2 END DO
...
!       Assume neutron star instead if mass > 6 solar masses.
    IF (ZMBAR*BMAX > 6.0) ZMSTAR = 1.5
    DM = BODY(IMAX) - ZMSTAR/ZMBAR
    ZMASS = ZMASS - DM
    ZMDOT = ZMDOT + DM*ZMBAR
    NMDOT = NMDOT + 1
    BODY(IMAX) = ZMSTAR/ZMBAR
    IF (TIDAL(1) /= 0.0D0) RTIDE = (ZMASS/TIDAL(1))**0.3333
...
!       Correct total energy, forces & first derivatives.
    ...
    DO 20 J = 1,NTOT
        ...
        IF (J <= N) POTI = POTI + BODY(J)/SQRT(RIJ2)
        IF (J <= 2*NPAIRS .OR. J == IBODY) GO TO 20
    ...
    !       Use neighbour list to distinguish irregular & regular terms.
        NNB = LIST(1,J) + 1
        DO 12 L = 2,NNB
            IF (LIST(L,J) == IBODY)  GO TO 16
            IF (LIST(L,J) > IBODY)  GO TO 13
        12 END DO
