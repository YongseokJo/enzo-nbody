...
!       Check for sufficient perturbers (RMAX = apocentre set in IMPACT).
    50 NNB = LIST(1,ICM)
    IF (SQRT(CMSEP2)*RMAX > 2.0*RS(ICM)) THEN
...
!     Remove ICM first
    IF(TIME+TOFF /= 0.D0) CALL delay_remove_tlist(icm,step,dtk)
...
!     Add c.m. into NXTLST
    IF(TIME+TOFF /= 0.D0) CALL delay_store_tlist(ICM)
...
!       Suppress the following for now.
    ...
    !       Estimate an appropriate KS slow-down index for G < GMIN.
        IF (KZ(26) > 0 .AND. STEP(ICM) > TK) THEN
...
!       Increase regularization counters (NKSHYP for hyperbolic orbits).
    ...
    IF (KZ(10) > 0 .AND. ICASE >= 2) THEN
        RI = SQRT((X(1,ICM) - RDENS(1))**2 + &
        (X(2,ICM) - RDENS(2))**2 + &
        (X(3,ICM) - RDENS(3))**2)
        WHICH1 = ' MERGE2 '
        IF (ICASE == 3) WHICH1 = ' RESET2 '
        if(rank == 0) &
...
!       Check case of initial binary and loop over disrupted pairs.
    IF (IABS(NAME(ICOMP) - NAME(JCOMP)) == 1) K = -1
    DO 80 L = 2,NNB
