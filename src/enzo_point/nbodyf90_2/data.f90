...
!       And plot interval for HR diagram.
    ...
    !     Check whether metallicity is reasonable
        IF (KZ(19) > 0 .AND. (ZMET < 0.0001 .OR. ZMET > 0.03)) THEN
    ...
    IF(KZ(22) >= 2 .AND. KZ(22) /= 9)GO TO 50

!       Include the case of equal masses (ALPHAS = 1 or BODY1 = BODYN).
    IF (ALPHAS == 1.0 .OR. BODY1 == BODYN) THEN
        DO 10 I = 1,N
            BODY(I) = 1.0
            BODY0(I) = 1.0/REAL(N)
        10 END DO
