...
!       Save orbital parameters in case of KS binary (called from KSAPO).
    ...
    !       Skip on #25 = 0/1 for consistent net WD modification of EKICK.
        IF ((KW < 13 .AND. KZ(25) == 0) .OR. &
        (KW == 12 .AND. KZ(25) /= 2)) GO TO 30
    ...
    !       Remove any circularized KS binary from the CHAOS/SYNCH table.
        IF (KSTAR(N+IPAIR) >= 10 .AND. NCHAOS > 0) THEN
            II = -(N + IPAIR)
            CALL SPIRAL(II)
            KSTAR(N+IPAIR) = 0
        !     ks MPI communication KSTAR
...
!*       Adopt the Maxwellian of Hansen & Phinney (MNRAS 291, 569, 1997).
!      DISP = 190.0
...
!       Allow for optional type-dependent WD kick.
    IF (KW < 13) THEN
        IF (KZ(25) == 1 .AND. (KW == 10 .OR. KW == 11)) THEN
            DISP = 5.0
        ELSE IF (KZ(25) > 1 .AND. KW == 12) THEN
            DISP = 5.0
        ELSE
            DISP = 0.0
        END IF
...
!       Use Henon's method for pairwise components (Douglas Heggie 22/5/97).
    ...
    VK(4) = VKICK
    IF(KW == 14) VKICK = VKICK*(1.D0-fallback)
...
!       Ensure escape of massless star.
    IF (IPAIR <= 0 .AND. BODY(I) == 0.0D0) VKICK = 10.0*VSTAR
!      END IF
    VKICK = VKICK/VSTAR

    IF (VKICK == 0.0) GO TO 30
...
!       Highlight BH/NS velocities below 4 times rms velocity.
    IF (VKICK < 4.0*SQRT(0.5) .AND. KW >= 13) THEN
        if(rank == 0) &
