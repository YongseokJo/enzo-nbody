...
!**** Note:------------------------------------------------------------**
!$$$      print*,rank,'KSREG ICOMP',ICOMP,'JCOMP',JCOMP,'NB',NNB,'t',time
!$$$      call flush(6)
!     --03/05/14 16:21-lwang-end----------------------------------------*
    IF (NNB == 1 .AND. LIST(2,ICOMP) == JCOMP) THEN
        LIST(2,ICOMP) = JCOMP + 1
        IF (JCOMP + 1 > NTOT) THEN
...
!       Copy neighbour list of #ICOMP without JCOMP.
    NNB1 = 1
    DO 1 L = 1,NNB
        IF (LIST(L+1,ICOMP) == JCOMP) GO TO 1
        NNB1 = NNB1 + 1
...
!       Save basic variables for components unless in correct location.
    ...
    !     Exchange NXTLST index also
        ...
        ELSE
            IF(TIME+TOFF /= 0.D0) call exchange_tlist(I,J,STEP,DTK)
        END IF
...
!      Update the tlist array in intgrt
    IF(IPHASE /= 8 .AND. time+TOFF /= 0.0D0) then
