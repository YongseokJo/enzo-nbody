...
!      Check for circularized case.
    IF(KSTAR(I) < 10 .AND. ECC1 < 0.002)THEN
        ECC1 = 0.001D0
        KSTAR(I) = 10
    ENDIF
...
!       Include GR coalescence criterion for compact objects.
    ...
    IF (KSX >= 13 .AND. KZ(28) > 0) THEN
        RCOAL = 6.0*BODY(I)/CLIGHT**2
    ELSE
        RCOAL = R1 + R2
    END IF
    IF (rank == 0 .AND. KZ(28) > 1 .AND. DSEP > 0.0) THEN
...
!       Include some diagnostic output.
    IF(KSTAR(I) == 13)THEN
        NDIAG = NDIAG + 1
        IF(NDIAG < 100 .OR. MOD(NDIAG,100) == 100)THEN
