...
!     Include case of chain chaos without identified NAMEC.
    ...
    !     See whether wrong component name (+ NZERO) saved as NAMEC in CHAOS2.
        ...
        DO 6 L = 1,2
            ...
            IF (KSAVE(2*L-1) < 0 .AND. NAM1 == NAME(I1)) THEN
                NAM2 = NZERO + NAME(I2)
            END IF
        6 END DO
        IC = NCHAOS
    ...
    !     Check identification for correct value (two K*=-2 are possible).
        DO 8 K = 1,NCHAOS
            IF (NAMEC(K) == NAM2) IC = K
        8 END DO
        NAMEC(IC) = NAME(I)
        IF (rank == 0 .AND. (NAM2 == NAMC .OR. IC < NCHAOS)) THEN
            WRITE (6,9)  IC, NCHAOS, NAM2, NAMC, NAME(I)
            9 FORMAT (' CHRECT RESTORE    IC NCH NM2 NMC NMI ',2I4,3I8)
        END IF
    ELSE IF (NCHAOS == 0) THEN
        if(rank == 0)WRITE (6,3)  NCHAOS, IPAIR, KSTAR(I), NAME(I)
...
!     Obtain current values of KS variables in case of spiral.
    ...
    !     Skip on call from RESET/MDOT with arbitrary phase (only updating).
        IF (ABS(TDOT2(IPAIR)) > 1.0D-12 .AND. DMR >= 0.0) THEN
    ...
    !     Update orbital parameters after merger, mass loss or radius change.
        ...
        !     ks MPI communication TEV
            ...
            CALL SPIRAL(IPAIR)
            IF (IPHASE < 0 .OR. KSTAR(I) > 0) GO TO 30
        END IF
...
!     Include check for SLEEP after recent WD/NS formation.
    ...
    !     Determine index of most recent degenerate object formation.
        IF (KSTAR(I2) < 10) THEN
            J1 = I1
        ELSE IF (KSTAR(I1) < 10) THEN
            J1 = I2
        ELSE
            J1 = I1
            IF (EPOCH(I2) > EPOCH(I1)) J1 = I2
        END IF
        IF (TIME - TEV0(J1) < 2.0*STEPX .AND. TC > 3000.0) THEN
            SLEEP = .TRUE.
        END IF
    END IF
...
!     Check circularization time after chain regularization.
    IF (IPHASE == 8) THEN
        ...
        CALL TCIRC(QPERI,ECC,I1,I2,ICIRC,TC)
        IF (TC > 3000.0) THEN
            if(rank == 0)WRITE (6,4)  NAME(I1), TC
...
!     Re-initialize all chaos parameters after expansion or check SLEEP.
    IF (DMR > 0.01 .OR. KSTAR(I) == -1 .OR. SLEEP) THEN
        IF (rank == 0 .AND. DMR > 0.01 .AND. TC > 100.0 .AND. KM < 5) THEN
    ...
    !     Combine the two stars inelastically in case of chaos disruption.
        IF (IDIS > 0 .AND. QPERI < RCOLL) THEN
            R1 = MAX(RADIUS(I1),RADIUS(I2))
            WHICH1 = ' CHAOS  '
            IF (KSTAR(I) == -2) WHICH1 = ' SPIRAL '
            if(rank == 0) &
        ...
        !$$$            CALL CMBODY(2)
            ...
            GO TO 30
        END IF
    ELSE IF (KSTAR(I) == -2) THEN
        I1 = 2*IPAIR - 1
        I2 = I1 + 1
        IF (QPERI < 2.0*MAX(RADIUS(I1),RADIUS(I2))) THEN
        ...
        !     Check Roche radius but skip RESET call (no second EMERGE correction).
            IF (RADIUS(J1) > RL1 .AND. IPHASE /= 7) THEN
                if(rank == 0) &
