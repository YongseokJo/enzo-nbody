...
!       Determine inclination in radians.
    ...
    IF(FAC < -1.D0 .OR. FAC > 1.D0)THEN
        IF(FAC < -1.05D0 .OR. FAC > 1.05D0)THEN
...
!       Skip small EMAX or large pericentre distance (TMDIS increased).
    ...
    IF (KSTARM(IM) >= 0 .AND. EMAX < 0.9) THEN
        IQ = 1
        GO TO 40
    END IF
...
!       Delay Kozai cycle for EMAX > 0.9 and TC > 2000.
    IF (EMAX >= 0.9) THEN
        ...
        IF (TC > 2000 .OR. (ECC < 0.1 .AND. EMAX < 0.95)) THEN
            IQ = 2
            GO TO 40
        END IF
    END IF
...
!       Check termination time for marginal stability criterion.
    IF (NAME(I) == NAMEI .AND. (TIME+TOFF) > TCHECK) THEN
...
!       Activate indicator on small QPERI to prevent collision before EMAX.
    IF (QPERI < 2.0*RM) ICOLL = .TRUE. 

!       Check circularization time near turning point of normal binary.
    IF ((KSTARM(IM) >= 0 .AND. EDAV > 0.0 .AND. &
    (DEDT < 0.0 .OR. ECC > ABS(EMAX-0.00005))) .OR. ICOLL) THEN
        IF ( .NOT. ICOLL) THEN
            CALL HICIRC(QPERI,ECC,I,IG,BODYI,TG,TC,EC,EDT,W)
        END IF
        IF (TC < 7.0D+04 .AND. QPERI < 6.0*RM .AND. ITRY < 4 .OR. &
        TC < 1.0D+04 .AND. QPERI < 5.0*RM .AND. ITRY < 8) THEN
    ...
    !       Define oscillation period (dimensionless time).
        ...
        !       Specify polytropic index for each star (n = 3, 2 or 3/2).
            IF (KSTAR(IK) == 3 .OR. KSTAR(IK) == 5) THEN
                BODI = CM(K,IM)
            ...
            ELSE
                IP = 3
                IF (KSTAR(IK) >= 3) IP = 2
                IF (KSTAR(IK) == 4 .OR. KSTAR(IK) == 6) IP = 3
                IF (KSTAR(IK) == 0) IP = 1
                W(K) = WW(IP)
            END IF
        20 END DO
    ...
    !       Begin chaos/spiral stage if chaos boundary has been crossed.
        ...
        !       Check collision condition to be sure.
            IF (QPERI < RADIUS(I) + RADIUS(IG)) THEN
    ...
    IF (NAME(I) /= NAMEI .OR. DTM <= DT0) THEN
        ...
        !       Specify reduced peri if old criterion < PCRIT/2 (avoids switching).
            IF (PCR < 0.5*PCRIT) THEN
                PCRIT = 0.75*PCRIT
            END IF
            IF (PCRIT < PCR .AND. PERT < 0.01 .AND. &
            ITIME < 20) THEN
                ALPH = 360.0*ZI/TWOPI
...
!       Delay first time if apsidal motion or GR are important (ECC < 0.9).
    IF (ECC < 0.90 .AND. ITIME == 0) THEN
        IF (MIN(TQ,TGR)*TSTAR < TG) THEN
            IQ = 2
            GO TO 40
        END IF
    END IF
...
!       Include check on tidal dissipation for large eccentricity.
    IF (KSTARM(IM) >= 0 .AND. ECC > 0.9) THEN
        TT = ECC/ABS(EDT)
        TQ = MIN(TT,TQ)
...
!       Include safety check to avoid problems.
    ETRY = ECC + EDAV*DT1
    IF (ECC > 0.96 .AND. ETRY > 0.98) THEN
        ETRY = MIN(ETRY,EMAX)
        QPTRY = SEMI*(1.0 - ETRY)
...
!       Ensure frequent termination check inside 5*RM to avoid overshooting.
    IF (QPERI < 5.0*RM .AND. EDAV > 0.0 .AND. KSTARM(IM) >= 0) THEN
...
!       Transform back to KS variables after integration.
    ...
    !       Include rectification of active SPIRAL (suppressed).
        IC = 0
        DO 50 K = 1,NCHAOS
            IF (NAMEC(K) == NZERO - NAMEM(IM)) IC = K
