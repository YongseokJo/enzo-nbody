...
!      DATA RLAGR, RSLAGR, RBLAGR, RPLAGR /NN14 * 0.0/
!      DATA NLAGR, NSLAGR,  /NN14 * 0/
!      DATA MLAGR, MSLAGR, MBLAGR, MPLAGR /NN14 * 0.0/
!      DATA VLAGR, VSLAGR, VBLAGR, VPLAGR /NN98 * 0.0/
!      DATA SLAGR, SSLAGR, SBLAGR, SPLAGR /NN98 * 0.0/
!      DATA ELAGR, ESLAGR, EBLAGR, EPLAGR /NN14 * 0.0/
!      DATA VTAVE, VTBAVE, VTSAVE, VTPAVE  /NN42 * 0.0/
!      DATA MSBLAGR, MSPBLAGR /NN1*0.0, NN1*0.0/
!      DATA NSBLAGR, NSPBLAGR /NN1*0.0, NN1*0.0/
!      DATA EBINL,   EBINBL   /NN*0.0, NN1*0.0/
!      DATA EPBINL,  EPBINBL  /NN*0.0, NN1*0.0/
...
!      DATA C1,C2,C3,CMASS,BMASS,SMASS,PMASS /6*0.0, NTP*0.0/
!      DATA MCPREV, MCSPREV, MCBPREV, MCPPREV /3*0.0, NTP*0.0/
!      DATA NC, NCS, NCB, NCPREV, NCSPREV, NCBPREV /6*0/
!      DATA NCP, NCPPREV, NPK /NTP*0, NTP*0, NTP*0/
!      DATA I,J,K,kk,kks,kkb,kkp /3*0,3*1,NTP*1/
!      DATA TMASS, TSMASS, TBMASS /3*0.0/
...
!     energy
    ...
    MM    = 0
    RH    = 0
!      DATA EPARS,EKIN,EPOT,EBIN,MM,RH /17*0.0/

!**   Flag
    IF (KZ7 == 2 .OR. KZ7 == 3) THEN
        fshell = .true.
    ELSE
        fshell = .false.
    END IF
...
!!$omp reduction(+:C1) reduction(+:C2) reduction(+:C3)
    DO I = 1, NPS
        C1 = C1 + S_M(I)*S_X1(I)
        C2 = C2 + S_M(I)*S_X2(I)
        C3 = C3 + S_M(I)*S_X3(I)
...
!**   Loop all stars
    ...
    !     Check the type of stars (single/binary/merger)
        IF (J > NPS) THEN
            ...
            !     cumulative binary bins for binary fraction
                IF (kk < NN)  NSBLAGR(kk) = NSBLAGR(kk) + 1
        ...
        !     cumulative binary bins for binary fraction
            ...
            END IF
            if (ri <= rc) EBINL(NN)  = EBINL(NN)   - EBI(I)
        ELSE
        !     Singles:
            NCS  = NCS + 1
            mi = S_M(J)
    ...
    !     rotational direction sign
        IF ((vrot1*r(2) - vrot2*r(1)) < 0.0) vrot(I) = - vrot(I)
    !     Get energy
    ...
    !     binary fraction
        IF (kk < NN .AND. kpre < kk .AND. .NOT. fshell) THEN
            IF (NSBLAGR(KK) == 0) THEN
                MSBLAGR(kk) = MSBLAGR(kk-1)
                NSBLAGR(kk) = NSBLAGR(kk-1)
            END IF
            IF (NSPBLAGR(KK) == 0) THEN
                MSPBLAGR(kk) = MSPBLAGR(kk-1)
                NSPBLAGR(kk) = NSPBLAGR(kk-1)
            END IF
            IF (A(1,KK) == 0)  A(:,KK) = A(:,KK-1)
        END IF

    !     half mass radius
        IF (CMASS >= HFMASS .AND. RH == 0.0) RH = ri
...
!     Go to next bin if mass reach the R_lagr limit
    if (kb < NN) THEN
        if ((mass >= ml(kb)) .OR. (kb == NN-1 .AND. n == ntot)) then
        !     update mass
            ml(kb) = mass
        ...
        !     initial next bins
            if (kb < NN) then
                if( .NOT. fshell .AND. nl(kb) == 0) then
...
!     ----------------------------
    ...
    end subroutine lagr_empty_fill
...
!     dispersion
    ...
    IF (ri <= rc) sl(NN,2:7) = sl(NN,2:7) + dr2(1:6)

!     check whether need to increase kk
    if (n-npre == nl(kb)) then
        if (fshell) npre = npre + nl(kb)
        kb = kb + 1
        if ( .NOT. fshell .AND. kb < NN) sl(kb,2:7) = sl(kb-1,2:7)
    end if

    return

    end subroutine lagr_sig_fill
...
!     ----------------------------
    ...
    end subroutine lagr_sig_empty_fill
...
!     ----------------------------
    ...
    end subroutine lagr_sig_average
