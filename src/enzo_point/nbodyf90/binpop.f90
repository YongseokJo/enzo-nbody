...
! NOTE: This routine works for <= 100 per cent binaries as the initial
! set up. It allows choice of an
! initial period distribution and allows eigenevolution of orbital parameters.
! It also MERGES the closest binaries to one star.

! NOTE: A more realistic code for pre-main sequence eigenevolution
! is contained in binpop_mardling.f. One day it will have to be
! included in binpop_pk.f as an aditional pre-ms evoltuion sub-routine.
...
!       Check type of binary mass distribution (NSKIP, IMF2 or split c.m.).
    IF (NSKIP == 0 .OR. KZ(20) >= 2) GO TO 10
    IF (RATIO == 1.0) GO TO 20
...
!     Output collision detection during generation
    OPEN (UNIT=42,STATUS='UNKNOWN',FORM='FORMATTED',FILE='ibcoll.42')

!     For eigen-evolution, output diagnostics for generation parameters
    IF (KZ(42) == 6 .AND. rank == 0) &
...
!       Introduce binary components from relative motion.
    ...
    !             Get thermal distribution if b=1 with e0=0
        ...
        31 IF (ECC0 < 0.0) THEN
            ECC2 = RAN2(KDUM)
            ECC = SQRT(ECC2)
        else if (ecc0 == 20) then
            ecc = RAN2(KDUM)
        
        else if (ecc0 == 30) then
            ecc = 1./0.15 - RAN2(KDUM)/0.1765
            ecc = 1./ecc
        
        else if (ecc0 == 40) then
            PK_e0 = 0.D0
            PK_b = 1.D0
        ...
        ELSE if (ecc0 >= 0.0 .AND. ecc0 <= 1.) then
            ECC = ECC0
        END IF
    ...
    !========= KZ(42) = 0 ========
        ...
        ! semi in pc and then in modell units:
            ...
            semi = semi/RBAR
        !========= KZ(42) = 5 ========
        else if (KZ(42) == 5) then
            exp1 = -10.
        ! Only accept periods logP>1, i.e. longer than 10 days at birth
        ! AND periods shorter than 10^(10) days!
            do while (exp1.LT.-1.6522)
                exp1 = GASDEV_pk(KDUM)
                if (exp1 > 2.2696) exp1=-10.
            end do
        ...
        !$$$C              IF (A0*RAU.GT.1000.0) GO TO 35
        ...
        !$$$              IF(rank.eq.0) then
        !$$$                 WRITE (23,99)  IT, I1, ZMB, E0, ECC, A0*SU, SEMI*SU, TK
        !$$$ 99              FORMAT (12X,'BINARY:   ITER I1 M(ICM)[M*] ECCI ECCC ',
        !$$$     &                'SEMII[R*] SEMIC[R*] P[days]',
        !$$$     &                I4,I12,F12.8,2F7.3,1P,3E15.6)
        !$$$                 CALL FLUSH(23)
        !$$$              end if
        !$$$***-----------
        !$$$          else
            if(rank == 0)write(6,*)' No KZ(42) defined!!!!! '
            STOP
        end if
    ...
    !     Avoid collision at beginning
        ...
        RP0 = (1-ecc)*semi
        IF (RP0 < MAX(RADIUS(I1),RADIUS(I2))) THEN
    ...
    ! Moved below to account for accepted binaries only.
    ...
    !          pkmergedist = 50000.D0 * 4.6523D-3
        J1 = I1 -2*pkmerge
        J2 = I2 -2*pkmerge
        if (semi <= pkmergedist) then
            pkmerge = pkmerge + 1
            ZMBIN = BODY(I1) + BODY(I2)
        ! Added 20.8.96:
            if(rank == 0)then
        ...
        ! Sum binding energy for accepted binaries.
            EBIN0 = EBIN0 + EBpk
            bodypk(J1) = body(I1)
            bodypk(J2) = body(I2)
            do k=1,3
...
! Kroupa: 5.11.96
! Take into account possible initial single stars. Before merging above
! these occupy positions: 2*NBIN+1....N+NBIN (before merging the stars
! in binaries occupy positions 1,2,....2*NBIN).
    pk_sing = 0
...
!       and introduce additional counters for MERGING:
    IF (RATIO < 1.0 .OR. KZ(20) >= 2) THEN
        N = N + NBIN
        NZERO = N
        NTOT = N
        BODYM = ZMASS/FLOAT(N)
        IF (rank == 0 .AND. NSKIP > 0) THEN
...
! CCCCCCCCCC for testing:
    ...
    !        write(6,*)
    !        write(6,*)
    !        write(6,*)' original stars'
    !        do i=1,N
    !           write(6,'(I4,7(F8.3))') i,body(i)*ZMBAR,
    !     +     (X(k,i),xdot(k,i),k=1,3)
    !        end do
    !        write(6,*)
    !        write(6,*)
    !        write(6,*)' remaining binaries'
    !        do i=1,2*NBINpk
    !           write(6,'(I4,7(F8.3))') i,bodypk(i)*ZMBAR,
    !     +     (Xpk(k,i),xdotpk(k,i),k=1,3)
    !        end do
    !        write(6,*)
    !        write(6,*)' merged ones:'
    !        do i=1,pkmerge
    !           write(6,'(I4,7(F8.3))') i,bodymergepk(i)*ZMBAR,
    !     +     (Xmergepk(k,i),xdotmergepk(k,i),k=1,3)
    !        end do
    !        write(6,*)'****************************************'
    !        call flush(6)
    end if
...
! Now overwrite the old vars with the MERGED ones:
    ...
    NBIN0 = NBIN0 - pkmerge
    NBIN = NBINpk
! The stars in remaining binaries:
    do i=1,2*NBIN
        body(i) = bodypk(i)
        do k=1,3
            x(k,i) = xpk(k,i)
...
! Add merged (i.e single) stars to end of the above list of stars in binaries:
    j = 2*NBIN
    do i=1,pkmerge
        j = j+1
...
! Add the initially single stars again at end of the lot:
    do i=1,pk_sing
        j=j+1
        body(j) = s_body(i)
        do k=1,3
...
! Added 11.08.98 with Sverre Aarseth: (body0 needed for
! stellar evolution!)
    if (j /= ntot) then
        if(rank == 0) &
    ...
    !      write(6,*)
    !      write(6,*)
    !      write(6,*)' After merging & re-arranging:'
    !      write(6,*)' N,NBIN0,NTOT: ',N,NBIN0,NTOT
    !      do i=1,N
    !         write(6,'(I3,7(F8.3))')i,body(i)*ZMBAR,
    !     +   (x(k,i),xdot(k,i),k=1,3)
    !      end do
    !      call flush(6)
    end if
...
! Added 20.06.96 write statements:
!       if(rank.eq.0)then
!       write(6,*)
...
! This below is wrong as in ecc formula above constant Rperi was assumed!!!
!* Duquennoy et al. 1992 in "Binaries as tracers of stellar evolution":
!      period = period_initial * DEXP((57.D0/14.D0) *
!     & (ecc*ecc - ecc_initial*ecc_initial))
...
! New semi-major axis and back to model units:
    ...
    return
    end subroutine proto_star_evol
!=======================================================
    FUNCTION gasdev_pk(idum)
    INTEGER :: idum
    REAL*8 :: gasdev_pk
!U    USES ran1
! P.Kroupa: 8.11.96: changed to RAN2
    INTEGER :: iset
