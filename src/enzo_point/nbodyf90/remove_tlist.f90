...
!     Check ghost
    IF(K == -1) THEN
        IF(NGHOSTS > 0) THEN
            L = NXTLIMIT + 1
            2 IF(NXTLST(L) == J) THEN
                IF(L-NXTLIMIT < NGHOSTS) &
                NXTLST(L) =  NXTLST(NXTLIMIT+NGHOSTS)
                NGHOSTS = NGHOSTS - 1
            !     --07/16/14 20:23-lwang-debug--------------------------------------*
            !**** Note:------------------------------------------------------------**
            !$$$               print*,'REMOVE GHOST J',J,'STEP',STEP(J),'NGHOSTS',
            !$$$     &              NGHOSTS,'LIST',NXTLST(NXTLIMIT+1:NXTLIMIT+NGHOSTS)
...
!     Shift ghost particles
    IF(NGHOSTS > 0) NXTLST(NXTLIMIT+1) = NXTLST(NXTLIMIT+NGHOSTS+1)

    CALL shrink_tlist

!     --07/08/14 16:49-lwang-debug--------------------------------------*
!**** Note:------------------------------------------------------------**
!$$$      print*,'REMOVE J',J,'NXTLIMIT',NXTLIMIT,'K',K,'L',LL,
!$$$     &     'NDTK',NDTK(K),'NDTK(K+1)',NDTK(K+1)
!     --07/08/14 16:49-lwang-end----------------------------------------*

    RETURN

    end subroutine remove_tlist
