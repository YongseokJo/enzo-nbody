...
!      DATA  G1,G2,G3,G4  /0.28,1.14,0.010,0.1/
!=========================
...
!     Generate initial mass function (N-NBIN0 singles & 2*NBIN0 binaries).
    ...
    !       Choose between Kroupa et al (M.N. 262, 545) & Eggleton (book).
        IF (KZ(20) == 2 .OR. KZ(20) == 4) THEN
            ZM = 0.08 + (G1*XX**G2 + G3*XX**G4)/(1.0 - XX)**0.58
        ELSE IF (KZ(20) == 3 .OR. KZ(20) == 5) THEN
            ZM = 0.3*XX/(1.0 - XX)**0.55
        ELSE IF (KZ(20) == 6 .OR. KZ(20) == 7 .OR. KZ(20) == 8) THEN
            LM = BODYN
            UM = BODY10
            ZM = IMFBD(XX,LM,UM)
        END IF
    ...
    !       See whether the mass falls within the specified range.
        IF (ZM >= BODYN .AND. ZM <= BODY10) THEN
            BODY(I) = ZM
            ZMASS = ZMASS + BODY(I)
        ELSE IF(ZM < 0 .AND. rank == 0) THEN
        !       Safe check
...
!       Save scaled binary masses in decreasing order for routine BINPOP.
    ...
    !       Adopt 0.6*(m2/m1)^(-0.4) distribution (Kouwenhoven, 2007)
        ...
        IF ((KZ(20) > 3 .AND. KZ(20) <= 5) .OR. KZ(20) == 7) THEN
        ...
        END IF
    !*       Use mass ratio of O stars based on observations (Kiminki & Kobulnicky 2012; Sana et al. 2012; Kobulnicky et al. 2014)
