...
!**** Note:------------------------------------------------------------**
!$$$      print*,'CHFIRR I FIRR',I,FIRR(1:3)
...
!       First subtract all perturber forces with respect to single body #ICH.
    ...
    !       Skip correcting all interactions outside RPERT2.
        IF (RIJ2 > RPERT2) GO TO 30
    !     --03/13/14 21:07-lwang-debug--------------------------------------*
    !**** Note:------------------------------------------------------------**
    !$$$          write(130+rank,*),'J',J,'N',NAME(J),'M',BODY(J),'X',X(1:3,J),
    !$$$     &         'XD',XDOT(1:3,J),'I',I,'M',BODY(I),'X',XI(1:3),'XD',
    !$$$     &         XIDOT(1:3),'T',TIME
    !$$$          if(rank.eq.0) then
    !$$$          end if
    !$$$          if(time.eq.29.047119140625000) then
    !$$$             write(130+rank,*),'j',j,'n',name(j),'xd',xdot(1,j),
    !$$$     &            'npc',npc,'t',time,'x0d',x0dot(1,j),'f',f(1,j),
    !$$$     &            'fd',fdot(1,j)
    !$$$          call flush(130+rank)
    !$$$          end if
    ...
    !**** Note:------------------------------------------------------------**
    !$$$          print*,rank,'CM J1',J1,'X1',X(1:3,J1),'XD',XDOT(1:3,J1),
    !$$$     &         'J2',X(1:3,J1+1),'XD',XDOT(1:3,J1+1),'XI',XI(1:3)
    !     --07/25/14 23:34-lwang-end----------------------------------------*
    !       See whether c.m. approximation applies (skip unperturbed case).
        IF (RIJ2 > CMSEP2*R(JP)**2 .OR. LIST(1,J1) == 0) GO TO 5
    ...
    !     --03/13/14 21:07-lwang-debug--------------------------------------*
    !**** Note:------------------------------------------------------------**
    !$$$          print*,'CHFIRR I FIRR',I,FIRR(1:3),'J',J,'N',NAME(J),
    !$$$     &         'NP',LIST(1,2*(J-N)-1)
    !$$$          if(time.eq.29.047119140625000) then
    !$$$             write(130+rank,*),'j',j,'n',name(j),'xd',xdot(1,j),
    !$$$     &            'npc',npc,'t',time,'x0d',x0dot(1,j),'f',f(1,j),
    ...
    !**** Note:------------------------------------------------------------**
    !$$$          if (rank.eq.0) then
    !$$$          print*,'J',J,'DX',DX(1:3),'DR3I',DR3I,'FIRR',FIRR(1:3),
    !$$$     &         'M',BODY(J),'MI',BODY(I)
    !$$$          call flush(6)
    !$$$          end if
    !$$$          if(rank.eq.0.and.time.gt.23.17335927486419678) stop
...
!       Set perturber distance and calculate force for chain c.m.
    ...
    !       Obtain perturbation on each component.
        ...
        !**** Note:------------------------------------------------------------**
        !$$$              print*,'48 DR2I',DR2I,'IM',IM,'K',K,'KDUM',KDUM
...
!       Add perturbations on the components to the c.m. force & derivative.
    ...
    !       Sum individual perturbations.
        ...
        !**** Note:------------------------------------------------------------**
        !$$$              print*,'80 FPSUM',FPSUM(1:3),'BODYC',BODYC(L),'FP',
        !$$$     &             (FP(L1+K),K=1,3)
...
!**** Note:------------------------------------------------------------**
!$$$      if(rank.eq.0) then
!$$$      print*,'F - CHFIRR I FIRR',I,FIRR(1:3),'J',J,'N',NAME(J)
!$$$      end if
!     --07/25/14 23:22-lwang-end----------------------------------------*

    RETURN

    END SUBROUTINE CHFIRR
