...
!       Specify slow-down criterion with conservative value for N > 4.
    GCRIT = 1.0D-05
    IF (N == 4) GCRIT = 5.0D-06
    IF (N > 4) GCRIT = 1.0D-06
...
!       Obtain maximum step from two-body iteration based on c-functions.
    IF (ICOLL == 0 .AND. ICALL == 0 .AND. STEPS(ISUB) > 0.0D0) THEN
...
!       Check time-step rejuvenation during standard integration.
    IF (MOD(NSTEP1,1000) == 0 .AND. ICALL == 0 .AND. ICOLL == 0) THEN
        CALL CONST(X,V,M,N,ENER0,G0,ALAG)
...
!       Save new step during standard integration for subsequent restart.
    22 IF (ICALL == 0 .AND. ICOLL == 0) THEN
        SAVEIT = STEP
        ICASE = .TRUE.
    !       Reset collision indicator and activate IPREV on failed iteration.
    ELSE IF (ICOLL < 0 .AND. .NOT. KCOLL) THEN
        ICOLL = 0
        IPREV = 1
        ICASE = .TRUE.
        KCOLL = .FALSE.
...
!       Check slow-down procedure for small minimum distance.
    IF (KZ26 >= 2 .AND. ( .NOT. stopB .OR. KSLOW)) then
        RM = 0.0
        DO 25 I = 1,N-1
            IF (RM < RINV(I)) THEN
                RM = RINV(I)
...
!       Include possible emergency stop of slow-down (DERQP; inactive).
    ...
    IF (STEP > 10.0*STEP0 .AND. ICOLL == 0) THEN
        STEP = 10.0*ABS(STEP0)
    ELSE IF (STEP == 0.0D0) THEN
        if(rank == 0) WRITE (6,*) ' Stepsize = 0!', char(7)
        STOP
    END IF

    IF (rank == 0 .AND. KZ30 > 2) THEN
...
!       Switch on search indicator during iteration or just after pericentre.
    IF (ICOLL < 0) ICALL = 1
    IF (RY < SX .AND. NSTEP1 > NEXT) THEN
        IF (ZMI3 < ZMI2 .AND. ZMI2 > ZMI1) THEN
        !       Skip pericentre determination on large perturbation (avoids looping).
            IF (ISYNC == 0 .AND. NCIRC < 25 .AND. GX < PMAX) THEN
                ICALL = 1
                IF (KZ26 >= 2 .AND. KSLOW) THEN
                    GSAVE = GCRIT
                    GCRIT = 0.0
                    NCIRC = NCIRC + 1
                    CALL SLOW
...
!       Restore indicators for further attempts after 100 steps.
    IF (ISYNC > 0 .AND. NSTEP1 > NSTEPX + 100) THEN
        NCIRC = 0
        ISYNC = 0
        IMCIRC = 0
        NSTEPX = NSTEP1
...
!       Check for capture, circularization or collision.
    IF (QPERI < 4.0*MAX(SIZE(K1),SIZE(K2))) THEN
    
    !       Distinguish between tidal energy loss and collision.
        J1 = K1
        J2 = K2
        IF (SIZE(K2) > SIZE(K1)) THEN
            J1 = K2
            J2 = K1
        END IF
    ...
    !       Avoid tidal dissipation for circularized orbit.
        ICIRC = 0
        IF (KZ27 > 0 .AND. EBS < 0.0) THEN
            SEMI = -0.5*M(K1)*M(K2)/EBS
            ECC = 1.0 - QPERI/SEMI
            IF (ECC > 0.0022) ICIRC = 1
        END IF
    ...
    !       Adopt collision criterion of Kochanek (Ap.J. 385, 684, 1992)
        ...
        !       Activate indicator for new chain (terminates at once with N = 2).
            KCASE = 1
            GO TO 10
        ELSE IF (ICIRC > 0 .AND. KZ27 > 0 .AND. ISYNC == 0) THEN
...
!       Check termination or strong perturbation (T > TMAX or GPERT > 0.01).
    ...
    !       Avoid checking after switch (just in case).
        IF (ISW <= 1) THEN
            ...
            IF (KCASE < 0) GO TO 60
        END IF
        IF (ISW == 0 .AND. N == 3) THEN
            IF (RSUM > 4.0*RM) THEN
                CALL CHSTAB(ITERM)
                IF (ITERM < 0) GO TO 70
            END IF
        END IF
        IF (CHTIME < TMAX .AND. STEPS(ISUB) > 0.0D0) GO TO 21
        GO TO 60
    ELSE
    !       Exit chain integration if time limit exceeded or STEPS = 0.
        IF (CHTIME > TMAX) GO TO 60
    !       Terminate on large step number and tiny STEP > 0.
        IF (NSTEP1 > 100000 .AND. STEP < SMALL .AND. STEP > 0.0) THEN
            if(rank == 0) &
...
!       See whether temporary or actual termination (continue if N > 5).
    IF (N > 5 .OR. (KCASE == 0 .AND. STEPS(ISUB) > 0.0D0)) GO TO 100
    IF (KCASE < 0) GO TO 70
    IF (TIMEC > TMAX .AND. RSUM < RMAXC) THEN
        IF (STEPS(ISUB) > 0.0D0 .OR. N > 4) GO TO 100
    END IF
...
!       Activate termination index for routine INTGRT.
    ...
    IF (rank == 0 .AND. KZ30 > 1 .AND. QPERI < 1.0) THEN
...
!       Update current time unless termination and set subsystem index.
    100 IF (ITERM >= 0) TS(ISUB) = T0S(ISUB) + TIMEC
    ISUB = ITERM
