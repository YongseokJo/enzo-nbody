...
!       Read new ETAI, ETAR, ETAU, DTMIN, RMIN (IF > 0 & KSTART = 4 or 5).
    ...
    !       Check modification of integration parameters.
        IF (ETA1 > 0.0) ETAI = ETA1
        IF (ETA2 > 0.0) ETAR = ETA2
        IF (ETA3 > 0.0) ETAU = ETA3
        IF (DTM > 0.0) THEN
            DTMIN = DTM
            SMIN = 2.0*DTM
        END IF
        IF (RM > 0.0) THEN
            ...
            RMIN22 = 4.0*RMIN2
        END IF
        IF (NEWCR > 0) NCRIT = NEWCR
        IF (NNBO > 0) NNBOPT = NNBO
        IF (SMAX0 > 0) THEN
        !     Check SMAX to make sure it have correct value
            IF(SMAX0 > 1) THEN
                ...
                DTNSMAX = 1.0
                99 IF(SMAX0/DTNSMAX <= 0.75) THEN
                    DTNSMAX = 0.5D0*DTNSMAX
                    IF(DTNSMAX > 1E-19) GO TO 99
                END IF
                SMAX = DTNSMAX
            END IF
        END IF
