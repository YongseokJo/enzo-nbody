...
!       ------------------------
    ...
    !       Switch on search indicator during iteration or just after pericentre.
        IF (ICOLL < 0) ICALL = 1
        IF (R(IMIN) < RSTAR .AND. NSTEP4 > NEXT) THEN
    ...
    !       Check the termination criterion.
        IM2 = IND(5)
        IF (R(IM2) > RMAX4 .OR. TIME4 > TMAX) THEN
            IF (R(IMIN) < 0.01*RUNAV) GO TO 50
            IQ = -1
            GO TO 100
        END IF
    ...
    !       Check rare case of zero regularized step.
        IF (ISTEPS < 3 .AND. DS == 0.0D0) DS = 1.0E-2*DSO
        IF (DS == 0.0D0) GO TO 120
    50 END DO

    100 RETURN

!       Make one restart with reduced tolerance.
    120 IF (ISTART > 1) RETURN
    if(rank == 0) &
