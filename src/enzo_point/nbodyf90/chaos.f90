...
!     See whether new case could be an old SPIRAL exchanged in chain.
    ...
    !     Remove tables and re-initialize on large epoch or standard binary.
        IF (TIME - TOSC(IC) > 1.0 .OR. KSTAR(I) == 0) THEN
            II = -I
            CALL SPIRAL(II)
            IC = 0
        END IF
...
!     Increase counter for new chaos and initialize index & variables.
    ...
    !     Ensure next location contains zero core mass (avoids confusion).
        ...
        DO 5 K = 1,4
            EOSC(K,IC) = 0.0D0
        5 END DO
        IF (KZ(9) >= 2 .AND. H(IPAIR) < 0.0) THEN
            CALL BINEV(IPAIR)
        END IF
        IF (NCHAOS > NTMAX) THEN
            if(rank == 0)WRITE (6,6)  NAME(I1), NCHAOS, ECC, QPERI
            6 FORMAT (' FATAL ERROR!    CHAOS    NM NCH E QP ', &
            I6,I4,F8.4,1P,E9.1)
            if(rank == 0)WRITE (6,7)  (NAMEC(K),K=1,NCHAOS)
...
!     Check termination for rare case of wide chaos (minimum 1000 calls).
    ...
    !     ks MPI communication
        ...
        if(rank == 0)WRITE (6,8)  NAME(I1), NAME(I2), NDEC, &
...
!     Save initial eccentricity, binding energy & J0 the first time.
    CJ = ZMU*SQRT(BODY(I))
    IF (IOSC(IC) == 0 .OR. LIST(1,I1) > 0) THEN
        SEMI = -0.5*BODY(I)/H(IPAIR)
        ECC0 = ECC
        EB0(IC) = ZMU*H(IPAIR)
...
!     Employ an emergency procedure for zero energy change (TIDES2 bug).
    IF (DE2(1) == 0.0D0 .OR. DE2(2) == 0.0D0) THEN
        EB = BODYM*H(IPAIR)
...
!     Adopt non-linear dissipation time scale of Kumar & Goodman 1995.
    ...
    IF (NDEC > 10000 .AND. QPERI > 4.0*RM) THEN
        DO 18 K = 1,2
            DE2(K) = 1000.0*DE2(K)
            DE3(K) = 1000.0*DE3(K)
        18 END DO
...
!     Check energy or eccentricity criterion for chaotic case.
    IF (KZ(27) == 2 .AND. (IOSC(IC) == 1 .OR. IOSC(IC) == -1)) THEN
        IF (EDEC(IC) > -(ECRIT(IC) - EB0(IC))) THEN
            IOSC(IC) = 2
            if(rank == 0) &
        ...
        ELSE
            ...
            IF (ECC1 < ECCM) THEN
                ...
                KICKS = KICKS + 1
                IF (KICKS < 3) THEN
                    if(rank == 0)WRITE (6,32)  NDEC, EPS, ECCM, ECC1
...
!     Check for hierarchical configuration on first call.
    34 IF (NDEC <= 1 .AND. SEMI > 0.0 .AND. SEMI < 2.0*RMIN) THEN
        NP1 = LIST(1,I1) + 1
        DO 39 L = 2,NP1
            J = LIST(L,I1)
            RIJ2 = 0.0
        ...
        !     Accept semi-major axis ratio below 25.
            IF (1.0/A1 > 0.04/SEMI .AND. IDIS <= 0) THEN
                RA = SEMI*(1.0 + ECC)
                SR = PMIN/RA
...
!     Check for terminated or escaped chaotic binaries first time.
    ...
    !     Skip during mergers or multiple regularizations (KSTAR not visible).
        IF (NMERGE > 0 .OR. NSUB > 0) GO TO 70
