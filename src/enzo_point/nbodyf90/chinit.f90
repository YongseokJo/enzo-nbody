...
!**** Note:------------------------------------------------------------**
!$$$      kk= 15967
!$$$      print*,rank,'SETSYS KK',kk,'n',name(kk),'x',x(1,kk),
!$$$     &     'xdot',xdot(1,kk),'fdot',fdot(1,kk),'time',time
!$$$      call flush(6)
...
!     --01/03/14 13:06-lwang-debug--------------------------------------*
!**** Note:------------------------------------------------------------**
!$$$      print*,rank,'SUBSYS ICH',ich,'n',name(ich),'x',x(1,ich),
!$$$     &     'xdot',xdot(1,ich),'fdot',fdot(1,ich),'time',time
!$$$      call flush(6)
!$$$      call mpi_barrier(MPI_COMM_WORLD,ierr)
!     --01/03/14 13:06-lwang-end----------------------------------------*
...
!       Check possible switch of reference body on second call from CHAIN.
    IF (TIMEC > 0.0D0 .AND. ICH /= ICH0) THEN
...
!     --01/03/14 13:06-lwang-debug--------------------------------------*
!**** Note:------------------------------------------------------------**
!$$$      print*,rank,'CHFIRR ICH',ich,'n',name(ich),'x',x(1,ich),
!$$$     &     'xdot',xdot(1,ich),'fdot',fdot(1,ich),'time',time
!$$$      call flush(6)
!$$$      call mpi_barrier(MPI_COMM_WORLD,ierr)
!     --01/03/14 13:06-lwang-end----------------------------------------*
...
!**** Note:------------------------------------------------------------**
!$$$               do i=1,ifirst-1
!$$$*            if(name(i).eq.16107) then
!$$$            write(102+rank,*) i,name(i),step(i),t0(i),time,body(i),
!$$$     *           gamma((i+1)/2),list(1,i),name((i+1)/2+N)
!$$$c$$$*               write(100+rank,*) i,name(i),x0(1,i),x0dot(1,i),
!$$$c$$$*     *              f(1,i),fdot(1,i),time
!$$$c$$$            end if
!$$$         end do
!$$$         do j=ifirst,ntot
!$$$*            if(name(j).eq.16107) then
!$$$            write(102+rank,*),j,name(j),'x0',(x0(kk,j),kk=1,3),
!$$$     *        'x0dot',(x0dot(kk,j),kk=1,3),'t0',t0(j),
!$$$     &           'step',step(j),'stepr',stepr(j),
