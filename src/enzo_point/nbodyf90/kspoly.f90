...
!       Obtain the perturbation & first derivative.
    DO 40 L = 2,NNB1
        ...
        !       See whether c.m. approximation applies.
            ...
            (X(3,J) - X(3,I))**2
            K = J - N
            IF (RIJ2 < CMSEP2*R(K)**2 .AND. LIST(1,2*K-1) > 0) THEN
                JDUM = 2*K - 1
                J = JDUM
            END IF
        END IF
...
!       Ensure that c.m. step is not exceeded (H > 0 is OK).
    IF (STEP(I1) > STEP(I) .AND. H(IPAIR) < 0.0) THEN
        DTU = 0.5*DTU
        GO TO 75
    END IF
