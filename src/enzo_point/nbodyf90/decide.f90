...
!       Accept active spiral but ensure some updating to increase pericentre.
    ...
    !       Adopt termination interval 1/2 of predicted t_{circ} (at const R*).
        ...
        IF (ECC < 0.1 .AND. MAX(KSTAR(I1),KSTAR(I2)) <= 1) THEN
            DT1 = 10.0*DT1
        END IF
...
!       See whether to deform binary orbit (skip chaos case).
    ...
    !       Transform to exact apocentre unless done already.
        IF (ABS(TDOT2(IPAIR)) > 1.0D-12) THEN
            IF (R(IPAIR) > SEMI .AND. TDOT2(IPAIR) < 0.0) THEN
                CALL KSAPO(IPAIR)
            END IF
            CALL KSPERI(IPAIR)
            CALL KSAPO(IPAIR)
        !       Check spiral case already at peri.
        ELSE IF (KSTAR(I) == -2 .AND. R(IPAIR) < SEMI .AND. &
            ABS(TDOT2(IPAIR)) < 1.0D-12) THEN
            CALL KSAPO(IPAIR)
        END IF
