...
!       Rename lists containing single regularized components.
    ...
    !     Indicator and index of ipair and last pair
        ...
        DO L = 2, NNB
            IF(IPAIRLIMIT == 0 .AND. LIST(L,J) > I2_IPAIR) &
            IPAIRLIMIT = L
            IF(LIST(L,J) > I2_ILAST) THEN
                ILASTLIMIT = L
                GO TO 20
            END IF
            IF(LIST(L,J) == I1_IPAIR) THEN
                ID_IPAIR = L
                INUM_IPAIR = 1
            END IF
            IF(LIST(L,J) == I2_IPAIR) THEN
                IF(INUM_IPAIR == 1) THEN
                    INUM_IPAIR = 2
                ELSE
                    ID_IPAIR = L
                    INUM_IPAIR = 1
                END IF
            END IF
            IF(LIST(L,J) == I1_ILAST) THEN
                ID_ILAST = L
                INUM_ILAST = 1
            END IF
            IF(LIST(L,J) == I2_ILAST) THEN
                IF(INUM_ILAST == 1) THEN
                    INUM_ILAST = 2
                ELSE
                    ID_ILAST = L
                    INUM_ILAST = 1
        ...
        20 DO L = NNB, 2, -1
            IF(ILCMLIMIT == 0 .AND. LIST(L,J) < ILASTCM) &
            ILCMLIMIT = L
            IF(LIST(L,J) < IPAIRCM) THEN
                IPCMLIMIT = L
                GO TO 30
            END IF
            IF(LIST(L,J) == ILASTCM) ID_ILASTCM = L
            IF(LIST(L,J) == IPAIRCM) THEN
                ID_IPAIRCM = L
            END IF
        END DO
    ...
    !     Check the case ipair member is in list
        30 IF(ID_IPAIR > 0) THEN
        !     Safe check
            IF(ID_IPAIRCM > 0) THEN
        ...
        !     Case both ipair and ilast members are in list
            ...
            !     number in list, don't need to shift other data
                IF(INUM_IPAIR == INUM_ILAST .AND. INUM_IPAIR == 1) THEN
                    IF(LIST(ID_IPAIR,J) == I1_IPAIR) THEN
                        IF(LIST(ID_ILAST,J) == I2_ILAST) THEN
                            LIST(ID_IPAIR,J) = I2_IPAIR
                            LIST(ID_ILAST,J) = I1_ILAST
                            FUPDATE = .true.
                        END IF
                    ELSE IF(LIST(ID_IPAIR,J) == I2_IPAIR) THEN
                        IF(LIST(ID_ILAST,J) == I1_ILAST) THEN
                ...
                ELSE IF(INUM_IPAIR < INUM_ILAST) THEN
            ...
            ELSE
                IF(ILASTLIMIT == 0) ILASTLIMIT = NNB + 1
                IF(INUM_IPAIR == 1) THEN
                    ITMP = 0
                    IF(LIST(ID_IPAIR,J) == I1_IPAIR) THEN
                        ITMP = I1_ILAST
    ...
    !     Check the ipair c.m. is in the list
        ...
        ELSE IF(ID_ILASTCM > 0) THEN
        !     right shift by 1 from LPCMLIMIT
            IF(IPCMLIMIT == 0) IPCMLIMIT = 1
    ...
    !     Update neighbor list in AVX/SSE library
        IF(J >= IFIRST .AND. FUPDATE .AND. TIME /= 0.0D0) THEN
    ...
    !$$$         IF(FUPDATE) THEN
    !$$$            NNB = LIST(1,J)
    !$$$            write(120+rank,*)'IPAIR',IPAIR,'IPAIRCM',IPAIRCM,
    !$$$     &           'ILAST',ILAST,'ILASTCM',ILASTCM,
    !$$$     &           'NAME(IL)',NAME(I1_ILAST),NAME(I2_ILAST),
    !$$$     &           'NAME(IP)',NAME(I1_IPAIR),NAME(I2_IPAIR),
    !$$$     &           'NAME(IPCM)',NAME(IPAIRCM),'NAME(ILCM)',
    !$$$     &           NAME(ILASTCM),'NTOT',NTOT
    !$$$            write(120+rank,*)'NEW',LIST(1:NNB+1,J),FUPDATE
    !$$$            write(120+rank,*)'INUM_ILAST',INUM_ILAST,'INUM_IPAIR',
    !$$$     &           INUM_IPAIR,'ID_ILAST',ID_ILAST,'ID_IPAIR',ID_IPAIR,
    !$$$     &           'ID_ILASTCM',ID_ILASTCM,'ID_IPAIRCM',ID_IPAIRCM
    !$$$            write(120+rank,*)'LIMIT: IPAIR',IPAIRLIMIT,'ILAST',
    !$$$     &           ILASTLIMIT,'IPCM',IPCMLIMIT,'ILCM',ILCMLIMIT,'T',TIME
    !$$$            call flush(120+rank)
    !$$$         END IF
...
!       Add primordial pairs to LISTD (skip new KS pairs or primordials).
    IF (IFLAG == 0 .OR. IABS(JLIST(1) - JLIST(2)) == 1) GO TO 110

!       Check list of disrupted component names.
    NNB = LISTD(1) - 1
    KCOMP = 0
    DO 100 K = 2,NNB+1,2
        IF (LISTD(K) == JLIST(1) .AND. LISTD(K+1) == JLIST(2)) KCOMP = 1
    100 END DO
...
!     Replace ilast c.m. to new position (ipair c.m.)
    130 IF(LISTV(L) == ILASTCM) LISTV(L) = IPAIRCM
    IF(L < LISTV(1)+1) THEN
        L = L + 1
        GO TO 125
    END IF
