...
!       Find the dominant body (JCOMP) and nearest perturber (JMAX).
    DO 10 L = 2,NNB2
        ...
        (X(3,I) - X(3,J))**2
        IF (J1 == I .AND. RIJ2 > 1.0D+04*RMIN2) GO TO 10
        NP = NP + 1
        JLIST(NP) = J
...
!       Specify larger perturbation for optional chain regularization.
    IF ((KZ(30) > 0 .OR. KZ(30) == -1) .AND. NCH == 0) THEN
        GSTAR = 100.0*GMIN
        KCHAIN = 1
    ELSE
        GSTAR = GMIN
...
!**** Note:------------------------------------------------------------**
!$$$      if(i.eq.5099.and.tblock.ge.4.55627441406250000E-002) then
!$$$      print*,rank,'impact 101 RDOT',RDOT,'PERT3',PERT3,'GSTAR',GSTAR,'t'
!$$$     *     ,time
!$$$      call flush(6)
!$$$      end if
!     --03/17/14 21:11-lwang-end----------------------------------------*
    IF (RDOT > 0.0 .OR. PERT3 > 100.0*GSTAR) GO TO 100
...
!       Quit on hyperbolic orbit with large impact parameter.
    IF (ECC1 > 1.0 .AND. PMIN > 50.0*SEMI) GO TO 100

!       Include rectification for non-circular binaries with KSTAR = 10 & 12.
    IF (KZ(27) == 2 .AND. KSTAR(I) >= 10 .AND. KSTAR(I) <= 18) THEN
        IF (ECC > 0.1 .AND. MOD(KSTAR(I),2) == 0) THEN
        ...
        !       Circularize the orbit instantaneously for short TC.
            ...
            !       Deform orbit to circular eccentricity after determining apocentre.
                IF (R(IPAIR) > SEMI .OR. ERR > 1.0D-04) THEN
...
!       Use combined semi-major axis for binary-binary collision.
    ...
    !**** Note:------------------------------------------------------------**
    !$$$          if(I.eq.9011.and.jcomp.eq.10283) then
    !$$$             print*,rank,'I',I,'N',NAME(I),'JCOMP',JCOMP,
    !$$$     &            'N',NAME(JCOMP),'SEMI',1.0/SEMI,'RMIN',0.25/RMIN
    !$$$     ^            ,'APO',APO,'RIJ',RIJ,'SEMI1',SEMI1,'PMIN',PMIN,
    !$$$     &            'FI-J',BODY(I)/RIJ**2,'FJ-I',BODY(J)/RIJ**2,
    !$$$     &            'STEPI',STEP(I),'STEPJ',STEP(JCOMP),'KCHAIN',KCHAIN
    !$$$             call flush(6)
    !$$$          end if
    !     --04/21/14 17:42-lwang-end----------------------------------------*
    !       Do not allow negative or soft cross section
        IF (1.0/SEMI < 0.5/RMIN) GO TO 100
    !       Consider merger for PMIN > SEMI and large semi-major axis ratio.
        IF (PMIN > SEMI .AND. SEMI2 > 20.0*SEMI0) GO TO 30
    !      END IF
...
!       Check separation in case of chain regularization.
    ...
    !     &         PMIN.GT.2.0*RMIN) GO TO 30
        IF (RIJ > MAX(3.0*RGRAV,RMIN) .OR. RSUM > 2.0*RMIN) GO TO 30
        GI = 2.0*BODY(JCOMP)*(RI/RIJ)**3/BODY(I)
    !       Enforce KS orbit using MERGE for high eccentricity if PMIN > 10*RI.
        IF (ECC1 > 0.99 .AND. PMIN > 10.0*RI .AND. &
        PERTM < GMAX) GO TO 40
        IF (KZ(27) > 0 .AND. JCOMP > N) THEN
            IF (SEMI0 < SEMI2) J1 = I1
            RT = 4.0*MAX(RADIUS(J1),RADIUS(J1+1))
        !       Do not allow large distance ratio for nearly synchronous binary.
            IF (SEMI0 > RT .AND. RI > 25.0*SEMI0) GO TO 30
            IF (MIN(SEMI0,SEMI2) < 0.05*RIJ) THEN
                IF (MAX(SEMI0,SEMI2) < 0.1*RIJ) GO TO 30
            END IF
        END IF
    END IF

!       Include special case of strong interraction and large ECC1.
    IF (ECC1 > 0.9 .AND. GAMMA(IPAIR) > 0.01) THEN
        IF (APO < 0.01*RMIN .AND. PMIN < 2.5*APO) GO TO 16
    END IF
...
!     IF ((APO.GT.0.01*RMIN.OR.JCOMP.GT.N).AND.PMIN.GT.APO) GO TO 30
    IF ((APO > 0.01*RMIN .OR. JCOMP > N) .AND. PMIN > 1.5*APO) GO TO 30
!     IF (APO.LE.0.01*RMIN.AND.PMIN.GT.2.0*APO) GO TO 30
    IF ((RIJ > RMIN .AND. SEMI1 > 0.0) .OR. RIJ > 2.0*RMIN) GO TO 100
    IF (PERTM > 100.0*GSTAR) GO TO 30
    16 IF (JCOMP > N .AND. PMIN > 0.1*RMIN) THEN
        IF (PMIN > A0 + SEMI2) GO TO 30
    END IF
    IF (JCOMP > N .AND. PMIN > 4.0*SEMIX .AND. &
    (ECC1 > 0.9 .AND. ECC1 < 1.0)) GO TO 30

!       Check almost stable triples (factor 1.2 is experimental).
    IF (JCOMP <= N .AND. PMIN > 2.5*SEMI) THEN
        CALL HISTAB(IPAIR,JCOMP,PMIN,RSTAB)
        RA = SEMI1*(1.0 + ECC1)
        IF (SEMI1 < 0.0) RA = RIJ
        GI = PERT*(RA/RIJ)**3
    !       Use estimated apocentre perturbation for decision-making.
        IF (PMIN > 1.2*RSTAB) THEN
            IF (GI < 0.05) GO TO 30
        !       Choose chain for critical case of highly eccentric outer orbit.
            IF (ECC1 < 0.95) GO TO 100
        ELSE IF (PMIN > 0.7*RSTAB) THEN
        !       Treat marginally stable triple according to external perturbation.
            IF (GI < 0.05) GO TO 30
            IF (GI < 1.0 .OR. ECC1 < 0.9) GO TO 100
        END IF
        IF (PMIN > 0.6*RSTAB .AND. PMIN < 0.9*RSTAB) GO TO 100
    !       Delay for large distance ratio outside 0.5*RMIN.
        IF (RIJ > MAX(10.0*APO,0.5*RMIN)) GO TO 100
        IF (RIJ > 10.0*APO) GO TO 100
        IF (PMIN > 2.5*APO) GO TO 40
    END IF
    IF (PMIN > 3.0*SEMI .AND. JCOMP <= N) GO TO 40

    IF (JCOMP > N) THEN
        IF (RIJ > 10.0*APO) GO TO 100
    END IF
!       Skip chain if merged binary or chain c.m. (denoted by NAME <= 0).
    IF (NAME(I) <= 0 .OR. NAME(JCOMP) <= 0) GO TO 100

!       Compare with existing subsystem of same type (if any).
    IF (NSUB > 0 .AND. KCHAIN <= 0) THEN
        PERIM = R(IPAIR) + RIJ
        IF (JCOMP > N) PERIM = PERIM + R(JPAIR)
        IGO = 0
...
!**** Note:------------------------------------------------------------**
!$$$      call abort()
!$$$      print*,rank,'IMPACT I1',i1,'n',name(i1),'x',x(1,i1),
!$$$     &     'xdot',xdot(1,i1),'fdot',fdot(1,i1),'time',time
!$$$      call flush(6)
...
!       Include any close single or c.m. perturber (cf. routine SETSYS).
    IF (JMAX > 0 .AND. JMAX /= JCOMP) THEN
        IF(SQRT(RMAX2) < MIN(2.0D0*RSUM,RMIN) &
         .AND. NAME(JMAX) > 0) THEN
            IF (JCOMP > N .AND. JMAX > N) THEN
                JCMAX = 0
            ELSE
                if(rank == 0) THEN
...
!       Check B-B interaction for switch of IPAIR & JPAIR or inert binary.
    IF (KCHAIN > 0 .AND. JCOMP > N) THEN
        ...
        IF (SEMI0 < 4.0*RT .AND. LIST(1,J1) == 0 .OR. &
        MIN(SEMI0,SEMI2) < 0.01*RIJ) THEN
        ...
        !       Check reduction of c.m. index (JPAIR becomes JPAIR - 1 if > IPAIR).
            IF (JPAIR > IPAIR) JCLOSE = JCLOSE - 1
            IF (KZ(26) < 2) THEN
...
!       Include the case of two interacting KS pairs.
    ...
    !       Switch pair indices and rename JCOMP if JPAIR has smaller size.
        IF (STEP(J1) < STEP(I1) .AND. LIST(1,I1) > 0) THEN
            KSPAIR = JPAIR
            JCOMP = I
            KS2 = IPAIR
        ELSE
            KS2 = JPAIR
        END IF
        IF (KZ(27) <= 0 .AND. JPAIR > IPAIR) THEN
            IF (JCLOSE > 0) JCLOSE = JCLOSE - 1
        END IF
...
!       Allow temporary merger of inner part of extremely eccentric orbit.
    RFAC = 10.0*RMIN
    IF (ECC1 > 0.99 .AND. RA > RFAC) THEN
        IF (RIJ < 0.1*SEMI1) RFAC = RA
    END IF
...
!       Increase apocentre tolerance to local scale factor for EB1 < EBS.
    ...
    !       Extend maximum apocentre for massive systems (less perturbers).
        IF (BODY(I) + BODY(JCOMP) > 10.0*BODYM) RFAC = 2.0*RFAC
    END IF
...
!     IF (BODY(I)*SMU.LT.0.4) ZF = 1.5
    ...
    !       Use orbital velocity condition instead of binding energy for planets.
        IF (SEMI1 > 2.0*RMIN) GO TO 100
    ELSE IF (EB > EBH .OR. EB1 > EBS .OR. RA > RFAC) THEN
        GO TO 100
    END IF
...
!       Check tidal capture option (synchronous or evolving binary orbit).
    ...
    !       Do not allow merger if Roche overflow or mass loss during next orbit.
        ...
        IF (KZ(34) > 0 .AND. TM - TIME < STEPX) THEN
            RT = 5.0*MAX(RADIUS(I1),RADIUS(I2))
            IF (A0 < RT .OR. KSTAR(I) > 0) THEN
                CALL TRFLOW(IPAIR,DTR)
                IF ((MOD(KSTAR(I),2) == 1 .AND. DTR < STEPX) .OR. &
                DTR < TK) GO TO 100
            END IF
            IF (JCOMP > N .AND. KSTAR(JCOMP) > 0) THEN
                CALL TRFLOW(JPAIR,DTR)
                IF(MOD(KSTAR(JCOMP),2) == 1 .OR. DTR < STEPX) GOTO 100
            END IF
        END IF
    ...
    !       Ensure SLEEP for circularizing binary with TCIRC > 1.0E+06.
        QPERI = A0*(1.0 - ECC)
        RM = MAX(RADIUS(I1),RADIUS(I2))
        IF (KSTAR(I) == -2 .AND. QPERI > 10.0*RM) THEN
            ICIRC = -1
            CALL INDUCE(IPAIR,JCOMP,EMAX,EMIN,ICIRC,TC,ANGLE,TG,EDAV)
            IF (TC > 1.0D+06) THEN
                if(rank == 0) &
    ...
    !       Delay merger for recently updated standard binary and short TCIRC.
        ...
        IF (KSTAR(I) == 0 .AND. NAME(I) > 0 .AND. DT < TK) THEN
            ICIRC = -1
        ...
        !       Beware possible termination by routine HMDOT using QPERI < 3*RADIUS.
            IF (TC < 2000.0 .AND. ECC > 0.002) GO TO 100
        END IF
        IF (KZ(19) >= 3) THEN
            IF (MIN(TEV(I1),TEV(I2)) < TIME + TK) GO TO 100
        END IF
    !       Skip chaotic binary (KSTAR = -2 is treated below).
        IF (KSTAR(I) == -1 .OR. KSTAR(JCOMP) == -1) GO TO 100
    END IF
...
!       Allow highly eccentric outer orbit (estimated PERT may be excessive).
    IF (ECC1 > 0.98 .AND. RIJ < 0.1*SEMI1) THEN
        PERT = PERT4
        GO TO 40
    END IF
...
!       Do not allow merger in the inner region of perturbed eccentric orbit.
    IF (RIJ < SEMI1 .AND. LIST(1,I1) > 0) THEN
    !       Note: moved down from label 30 with 0.1*SEMI1 replacing 2*PMIN.
        IF (ECC1 > 0.95 .AND. RIJ < 0.1*SEMI1) THEN
            GO TO 100
        END IF
    END IF
...
!       Evaluate the general stability function (Mardling MNRAS, 2008).
    IF (ECC1 < 1.0 .AND. YFAC < 1.02) THEN
        BJ = BODY(JCOMP)
        EOUT = ECC1
    ...
    !**** Note:------------------------------------------------------------**
    !          print*,'NST SEMI',SEMI,'SEMI1',SEMI1,'ECC',ECC,'EOUT',EOUT,
    !     &         'ANGLE',ANGLE,'BODY1',BODY(I1),'BODY(I2)',BJ
    !     --10/08/14 21:40-lwang-end-debug----------------------------------*
        ...
        !       Specify reduced peri if old criterion < PCRIT/2 (avoids switching).
            IF (PCR < 0.5*PCRIT) THEN
                PCRIT = 0.75*PCRIT
            END IF
            IF (PCRIT < YFAC*PCR .AND. PERT < 0.02 .AND. &
            NMTRY < 10) THEN
...
!       Check whether the main perturber dominates the outer component.
    IF (JMAX /= JCOMP) THEN
        ...
        IF (FMAX > (BODY(I) + BODY(JCOMP))/RJMIN2) GO TO 100
    END IF
...
!       Check perturbed stability condition.
    IF (PMIN*(1.0 - PERT) < YFAC*PCRIT) GO TO 100

!       Extend active Roche case up to end of look-up time (bug fix 01/09).
    IF (KSTAR(I) >= 11 .AND. MOD(KSTAR(I),2) /= 0) THEN
        DT = TEV(I) - TIME
        IF (DT < 10.0*STEPX) GO TO 100
    ...
    !       Prevent termination for TC < 2000 in HMDOT but allow small EMAX & DE.
        ...
        !       Accept KSTAR = 10, coasting Roche or small eccentricity.
            IF (MOD(KSTAR(I),2) == 0) THEN
                ...
                END IF
            ELSE
                IF (EMAX > 0.8 .OR. DE > 0.2 .OR. DT < 0.1) GO TO 100
            END IF
        END IF
    END IF

!       Check circularization and dissipation time (exclude Roche stages).
    IF (KZ(27) == 2 .AND. KSTAR(I) < 10 .AND. ECC1 < 1.0 .AND. &
    NAME(I) > 0) THEN
        ECC0 = ECC
        CALL DECIDE(IPAIR,SEMI,ECC,EMAX,EMIN,TG,EDAV,IQ)
        IF (IQ > 0 .OR. IPHASE < 0) GO TO 100
...
!       Perform safety check on radii for case of no circularization.
    IF (KZ(27) == 0) THEN
        IF (SEMI*(1.0 - ECC) < 2.0*MAX(RADIUS(I1),RADIUS(I2))) THEN
            IF (KZ(19) == 0) THEN
                GO TO 100
            END IF
        END IF
...
!       Include rare case of circularizing outer binary.
    IF (KZ(27) == 2 .AND. JCOMP > N .AND. KSTAR(JCOMP) == -2) THEN
        ECC2 = (1.0 - R(JPAIR)/SEMI2)**2 + &
        TDOT2(JPAIR)**2/(BODY(JCOMP)*SEMI2)
        ECCJ = SQRT(ECC2)
...
!       Specify the final critical pericentre using the fudge factor.
    ...
    IF (NAME(I) > N .AND. NAME(JCOMP) > N .AND. ECC1 > 1.0) GO TO 100
!       Skip if #JCOMP is a chain c.m. but allow bound double hierarchy.
    IF (NAME(JCOMP) == 0) GO TO 100
    IF (ECC1 > 1.0 .AND. MIN(NAME(I),NAME(JCOMP)) < 0) GO TO 100
    DO 55 ISUB = 1,NSUB
        IF (NAME(JCOMP) == NAMES(1,ISUB)) GO TO 100
    55 END DO
...
!       Include diagnostics for double hierarchy or optional standard case.
    ...
    !       Note rare case of two hierarchies merging and identify ghost names.
        IF (NAME(I) < 0 .AND. NAME(JCOMP) < 0) THEN
            CALL FINDJ(I1,JI,IM)
            J1 = 2*JPAIR - 1
            CALL FINDJ(J1,JJ,JM)
            if(rank == 0) &
...
!       Check for diagnostic output of quadruples.
    IF (SEMI1 > 0.0 .AND. JCOMP > N .AND. KZ(15) >= 3) THEN
        ZMB = BODY(I) + BODY(JCOMP)
