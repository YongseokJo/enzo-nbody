...
!       Adopt c.m. approximation for small total perturbation.
    ...
    !**** Note:------------------------------------------------------------**
    !$$$      if(time.ge.14.968627929687500.and.name(i).eq.18207) then
    !$$$         print*,'bcmf',rank,'I',I,'n',name(i),'fi',fi(1,i),
    !$$$     &        'fin',firr(1),'s',step(i),'fd',fidot(1,i),
    !$$$     &        'fdn',fd(1),'nb',list(1,i),'pn',list(1,2*(i-n)-1)
    !$$$         call flush(6)
    !$$$      end if
    !     --03/03/14 20:51-lwang-end----------------------------------------*
        call cmfirr_cor(I,I1,FIRR,FD)
    !     --03/30/13 22:34-lwang-debug--------------------------------------*
    !$$$      if (time.ge.0.3.and.name(i).eq.6193) then
    !$$$         write(111+rank,*),'anb',I,name(i),FIRR,FD,TIME,nnb0,list(1,i1)
    !$$$*      if (rank.eq.0.and.nstepr.gt.4314800) then
    !$$$*         write (6,*) name(i), nnb,time, freg(1)
    !$$$* 118     format ('name, nnb, freg, time', 2I6, 1F2.6, 1E10.2)
    !$$$         call flush(111+rank)
    !$$$      end if
    !$$$*         write(111+rank,*) 'nb',I,i1,FIRR,FD,TIME,nnb0,list(1,i1)
...
!**** Note:------------------------------------------------------------**
!$$$      if (time.eq.4.2424011230468750.and.name(i).eq.2602) then
!$$$         write(6,*),rank,'AC I',I,'N',name(i),'FI',FIRR,'FD',FD,'T',TIME
!$$$     &        ,'NB',nnb0
!$$$         call flush(6)
!$$$      end if
...
!     --03/03/14 20:51-lwang-debug--------------------------------------*
!**** Note:------------------------------------------------------------**
!$$$      if(time.eq.0.96801757812500000.and.rank.eq.0) then
!$$$         print*,rank,'I',I,'n',name(i),'fi',fi(1,i),
!$$$     &        'fin',firr(1),'s',step(i),'fd',fidot(1,i),
!$$$     &        'fdn',fd(1),'nb',list(1,i)
!$$$         call flush(6)
!$$$      end if
!     --03/03/14 20:51-lwang-end----------------------------------------*
    DO 80 K = 1,3
...
!**** Note:------------------------------------------------------------**
!$$$      if(rank.eq.0.and.time.ge.4.1667480468750000.and.name(i).eq.1155)
!$$$     &     then
!$$$         print*,rank,'nbint i',i,'n',name(i),'firr',firr(1),'fd',fd(1),
!$$$     &        'step',step(i),'dt0',dt0,'t',time,'x',xi(1),'xd',xidot(1),
!$$$     *        'nb',list(1,i),'step',step(i)
!$$$         call flush(6)
!$$$      end if
    if(dt0 <= 0.1*step(i)) then
    ...
    !$$$         if(name(i).eq.1155) stop
    !$$$         print*,'N',N,'BODY',BODY(I)
    !$$$         do k =2,LIST(1,I)+1
    !$$$            kk =LIST(K,I)
    !$$$            print*, 'I',KK,'N',NAME(KK),'STEP',STEP(KK),'M',BODY(KK)
    !$$$         end do
    end if
...
!       Select discrete value (increased by 2, decreased by 2 or unchanged).
    IF (TTMP > 2.0*STEP(I)) THEN
        IF (DMOD(TIME,2.0*STEP(I)) == 0.0D0) THEN
            TTMP = MIN(2.0*STEP(I),SMAX)
        ELSE
            TTMP = STEP(I)
        END IF
    ELSE IF (TTMP < STEP(I)) THEN
        TTMP = 0.5*STEP(I)
        IF (TTMP > DT0) THEN
            TTMP = 0.5*TTMP
        END IF
...
!$$$      if (time.ge.0.3.and.name(i).eq.6193) then
!$$$      j=i
!$$$         write(105+rank,*),'nb',j,name(j),'x0',(x0(kk,j),kk=1,3),
