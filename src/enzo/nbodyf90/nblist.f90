...
!       Search all single particles & c.m. bodies.
    ...
    DO 10 J = IFIRST,NTOT
        ...
        (X(3,I) - X(3,J))**2
        IF (RIJ2 > RCRIT2) GO TO 10
        IF (RIJ2 > RS2) THEN
        !       Skip velocity test for primordial binaries.
            IF (TIME <= 0.0D0 .AND. J <= 2*NBIN0) GO TO 6
            RIJDOT = 0.0
            DO 4 K = 1,3
                RIJDOT = RIJDOT + &
        ...
        !       Accept member if maximum penetration factor is significant.
            IF (RIJDOT >= VRFAC) GO TO 10
        ELSE
            IF (J == I) GO TO 10
        END IF
    !       Include both components in case of primordial binaries.
        6 IF (J <= 2*NBIN0 .AND. TIME == 0.0D0) THEN
            JPAIR = KVEC(J)
