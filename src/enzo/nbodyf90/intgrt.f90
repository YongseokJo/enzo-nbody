...
!       Update quantized value of STEPM for large N (first time only).
    ...
    IF ( .NOT. LSTEPM .AND. NZERO > 1024) THEN
    ...
    !     Initial nxtlst
        IF(TIME+TOFF == 0.D0) THEN
            ...
            DO I = IFIRST,NTOT
                ...
                END IF
            END DO
            IF(NGHOSTS > 0) NXTLST(NXTLIMIT+1:NXTLIMIT+NGHOSTS) &
            ...
            !     Initial NMINR
                IMINR(1:NTOT) = 1
                DO I = IFIRST,2*NBIN0
                    IF(NAME(I) <= 2*NBIN0) THEN
                        IF(MOD(I,2) == 0 .AND. NAME(I-1) == NAME(I)-1) THEN
                            IMINR(I) = I - 1
                        ELSE IF(MOD(I,2) == 1 &
                             .AND. NAME(I+1) == NAME(I)+1) THEN
                            IMINR(I) = I + 1
                        END IF
                    END IF
                END DO
            ...
            !     At the first time, full dataset need to be stored,
                if(rank == 0) then
                    ...
                    'with time interval ',DTK(KZ(47)+1)
                    #endif
                    IF(KZ(46) >= 3 .AND. KZ(46) <= 4 .AND. SMAX > &
                    DTK(KZ(47)+1)) then
                ...
                if(rank == 0) then
                    IF(KZ(46) == 1 .OR. KZ(46) == 3) THEN
                        call custom_update_file(TTOT,DELTAT)
                    ELSE IF(KZ(46) == 2 .OR. KZ(46) == 4) THEN
        ...
        IF(rank == 0 .AND. TOFF+TIME /= 0.D0) THEN
            IF(KZ(46) == 1 .OR. KZ(46) == 3) THEN
                call custom_update_file(TTOT,DELTAT)
            ELSE IF(KZ(46) == 2 .OR. KZ(46) == 4) THEN
                call custom_update_file(TTOT,DTOUT)
...
!       Search for high velocities after escape or KS/chain termination.
    999 IF (KZ(37) > 0 .AND. (IPHASE == -1 .OR. IPHASE >= 2)) THEN
        CALL HIVEL(0)
    END IF
...
!     When necessary, update all particle data in AVX/SSE library
    IF( .NOT. FNTLOOP .OR. IPHASE < 0 .OR. IPHASE >= 3) THEN
...
!       Redetermine TMIN after main change to catch new small steps after chain.
    ICALL = ICALL + 1
    IF (ICALL == 2) GO TO 999

    call cputim(tttbegin)

!     See whether need to add delay list into NXTLST
    IF(NLSTDELAY(1) > 0) call delay_add_tlist(T0,STEP,DTK)
...
!**** Note: Check time step--------------------------------------------**
    DO L = 1, NXTLEN
        ...
        IF(BODY(J) == 0.D0) THEN
            ...
            call abort()
        END IF
    !         IF(J.EQ.9951) print*,'L',L,'J',J,'T',TIME
    END DO
...
!     Determine MPI NXTLST and Regular block
    ...
    DO L = 1, NXTLEN
        ...
        IF (TIMENW(J) >= T0R(J) + STEPR(J)) THEN
            NREG = NREG + 1
            IREG(NREG) = J
        END IF
        IF (J > N) THEN
            IPAIR = J - N
            IF (LIST(1,2*IPAIR-1) > 0) NSTEPB = NSTEPB + 1
        END IF
    END DO
...
!**** Note: Interupt the simulation -----------------------------------**
    if(time >= dumptime) then
        if(rank == 0) call mydump(1,1)
        #ifdef PARALLEL
...
!      IF (KZ(14).EQ.3.OR.KZ(14).EQ.4) THEN
    IF (KZ(14) == 3 .OR. KZ(14) == 4 .OR. &
    (KZ(14) == 9 .AND. TTMODE == 0) ) THEN
    !          IF (KZ(14).EQ.3.AND.DMOD(TIME,STEPX).EQ.0.0D0) THEN
        IF ((KZ(14) == 3 .OR. KZ(14) == 9) .AND. &
        DMOD(TIME,STEPX) == 0.0D0) THEN
        !** FRenaud
            #else
        !       Check optional integration of cluster guiding centre.
            IF (KZ(14) == 3 .OR. KZ(14) == 4) THEN
                IF (KZ(14) == 3 .AND. DMOD(TIME,STEPX) == 0.0D0) THEN
                    #endif
                    CALL GCINT
                END IF
            !       Include mass loss by gas expulsion (Kroupa et al. MN 321, 699).
                IF (MPDOT > 0.0D0 .AND. TIME + TOFF > TDELAY) THEN
        ...
        !    &                   TIME/STEP(IMIN)
        ! 005     FORMAT (' DANGER!   I NM # TIME STEP T/DT ',
        ...
        !       See whether to advance any close encounters at first new time.
            IF ((TIME > TPREV) .AND. (TIME /= TCRIT)) THEN
                call cputim(tt5)
                CALL SUBINT(IQ,I10)
                call cputim(tt6)
        ...
        !       Check regularization criterion for single particles.
            ...
            !       Search only in prepared list of short-step particles. (R.Sp.)
                ...
                !$$$             WRITE (6,24)  TIME, I, NAME(I), NXTLEN, NSTEPI,
                !$$$     &            STEP(I), STEPR(I), NTOT, NPAIRS
                !$$$ 24          FORMAT (' SMALL STEP!!  T I NAME LEN #I SI SR NT NP',F10.5,
                !$$$     &            3I6,I11,1P,2E10.2,2I11)
                !$$$             CALL FLUSH(6)
                !$$$             STOP
                !$$$  END IF
                    IF (STEP(I) < DTMIN .AND. STEP(I) < DSMIN .AND. I <= N .AND. &
                    I >= IFIRST) THEN
                        DSMIN = STEP(I)
                        ISMIN = I
                    END IF
        ...
        !       See whether dominant body can be regularized.
            ...
            !       Include close encounter search for low-eccentric massive binaries.
                IF (IKS == 0 .AND. NXTLEVEL <= 4 .AND. STEP(ISMIN) < 4.0*DTMIN) &
                THEN
                !       Consider massive single bodies in absence of subsystems.
                    IF (ISMIN <= N .AND. BODY(I) > 2.0*BODYM .AND. NSUB == 0) THEN
                    ...
                    !       Obtain two-body elements and relative perturbation.
                        ...
                        IF (EB < EBH .AND. GI < 0.25 .AND. JMIN >= IFIRST) THEN
                            APO = SEMI*(1.0 + ECC)
                        !     Check eccentricity (cf. max perturbation) and neighbour radius.
                            IF (ECC < 0.25 .AND. APO < 0.02*RS(ISMIN)) THEN
                                if(rank == 0) PRINT*, ' KS TRY: NAM E A EB ', &
        ...
        !       Block-Step Level Diagnostics (R.Sp. 29.Apr. 1993)
            IF(KZ(33) > 0)JHIST(NXTLEN) = JHIST(NXTLEN) + 1
        
            #ifdef DEBUG
        !     --08/27/13 16:38-lwang-debug--------------------------------------*
        !**** Note: Before nbint-----------------------------------------------**
            if(time >= dprintt) then
                do L=1,NXTLEN
                    J = NXTLST(L)
                ...
                !            write(100+rank,*)'LIST',LIST(1:LIST(1,J)+1,J)
                !              call flush(100+rank)
                end if
            end do
            ...
            #ifdef PARALLEL
        !        print*,'rank',rank,'bar count:',ibarcount,'nstepi',nstepi
            call flush(6)
        !        call mpi_barrier(MPI_COMM_WORLD,ierr)
            #endif
        !        if (TCCT.gt.2.0) stop
    ...
    !     Predict chain variables and perturbers at new block-time.
        ...
        !     &          nxtlen,ibarcount
            ...
            do ix = 1,isize
                inum(ix)=inl
                if(ix <= idiff)inum(ix) = inum(ix) + 1
                ista(ix) = irun+1
                if(ista(ix) > nl)inum(ix) = 0
                irun = irun + inum(ix)
            end do
        ...
        !     OpenMP version of irregular force and fdot
            ...
            DO L = istart_lst,iend_lst,I_STEP_LEN
                if(iend_lst-L+1 >= I_STEP_LEN) THEN
    ...
    !       See whether any KS candidates are in the same block.
        ...
        !       Accept same time, otherwise reduce STEP(ICOMP) and/or delay.
            ...
            !**** Note:------------------------------------------------------------**
            !$$$              if(time.ge.0.3) then
            !$$$                 print*,rank,'icomp',icomp,name(icomp),
            !$$$     &            x(1,icomp),xdot(1,icomp),x0(1,icomp),x0dot(i,icomp)
            !$$$              end if
            !     --09/25/13 21:44-lwang-end----------------------------------------*
            ELSE IF (T0(JCOMP) + STEP(JCOMP) < T0(ICOMP)) THEN
    ...
    !**** --Regular force calculation--------------------------------------**
        ...
        !**** Note:------------------------------------------------------------**
        !$$$      if(rank.eq.0) print*,'NREG',NXTLEN,NREG,TIME,NGPUC,tt52-tt51
        !$$$      NGPUC = NGPUC + 1
        !$$$      if(rank.eq.0) then
        !$$$         do i=1,nreg
        !$$$            L=ireg(i)
        !$$$            write(100,*) NREG, Name(L), STEPR(L),TIME
        !$$$         end do
        !$$$      end if
        ...
        !       Block-Step Level Diagnostics (R.Sp. 29.Apr. 1993)
            IF(KZ(33) > 1)JHISTR(NREG) = JHISTR(NREG) + 1
        

            #ifdef PARALLEL
            IF(NREG <= iserreg)THEN
                #endif
            ...
            ELSE
                ...
                istart = ista(rank+1)
                iend = ista(rank+1) + inum(rank+1) - 1
            !$$$      if (ixxxx.le.10000000.and.time.gt.1.33.and.rank.eq.0) then
            ...
            !        Distribute variables into private vectors again T3E (R.Sp.)
                ...
                !$$$      if (ixxxx.le.10000000.and.time.gt.1.33) then
        ...
        !$$$         call cputim(tt333)
            ...
            NBLCKR = NBLCKR + 1
        !$$$*
        !$$$c$$$**!$omp parallel do if(NREG.GE.ithread) private(L,I,K)
        !$$$c$$$         DO L = 1,NREG
        !$$$c$$$            I = IREG(L)
        !$$$c$$$*
        !$$$c$$$            LIST(1:LMAX,I) = IMPI(1:LMAX,L)
        !$$$c$$$*
        !$$$c$$$*       Check minimum neighbor sphere since last output
        !$$$c$$$**!$omp critical
        !$$$c$$$            IF(LIST(1,I).GT.0)RSMIN = MIN(RSMIN,RS(I))
        !$$$c$$$**!$omp end critical
        !$$$c$$$*
        !$$$c$$$         END DO
    ...
    !     custom output
        call cputim(ttiout1)
        IF(KZ(46) > 0 .AND. DMOD(TIME,DTOUT) == 0) THEN
    ...
    !$$$      J = 230
    !$$$      K=K_STEP(STEP(J),DTK)
    !$$$      DO L = NDTK(K+1)+1, NDTK(K)
    !$$$c      DO L = 1,NXTLIMIT
    !$$$         IF(NXTLST(L).EQ.J) THEN
    !$$$            print*,'J',J,'N',NAME(J),'STEP',STEP(J),'T',TIME,'NXTLEVEL',
    !$$$     &           NXTLEVEL,'L',L,'DTK',DTK(K),'K',K,'IPHASE',IPHASE
    !$$$C            GO TO 1022
    !$$$         end if
    !$$$      END DO
    !$$$      print*,'J',J,'N',NAME(J),'NOT FIND STEP',STEP(J),'K',K,'NXTLEVEL',
    !$$$     &     NXTLEVEL,'IPHASE',IPHASE
    !$$$      call flush(6)
    !$$$C      call abort()
    !$$$ 1022 continue
    ...
    !    &                                            BODY(I),T0(I))
        ...
        !     Allow large quantized interval with internal iteration.
            IF (DMOD(TIME,0.25D0) == 0.0D0) THEN
                #ifndef TT
            ! omp parallel do private(J)
                #endif
                DO 65 J = ITAIL0,NTTOT
                    IF (TIMENW(J) <= TIME) THEN
                        CALL NTINT(J)
                    END IF
                65 END DO
                #ifndef TT
    ...
    !       Perform optional check on high-velocity particles at major times.
        IF (KZ(37) > 0 .AND. LISTV(1) > 0) THEN
            call cputim(tttsa)
            IF (DMOD(TIME,STEPM) == 0.0D0 .AND. NREG > 0) THEN
                CALL SHRINK(TMIN)
                IF (LISTV(1) > 0) THEN
                    CALL HIVEL(-1)
                END IF
    ...
    !       Check optional mass loss time.
        ...
        !       Delay until time commensurate with 1000-year step (new polynomials).
            IF (NREG /= 0 .AND. TIME > TMDOT &
             .AND. DMOD(TIME,STEPX) == 0.0D0) THEN
            !$$$          IF (TIME.GT.TMDOT.AND.DMOD(TIME,STEPX).EQ.0.0D0) THEN
                ...
                IF (IPHASE < 0 .OR. IPHASE == 1 .OR. IPHASE == 2) GO TO 999
            END IF
        END IF
    
    !       Advance counters and check timer & optional COMMON save (NSUB = 0).
        NTIMER = NTIMER + NXTLEN
        IF (NTIMER < NMAX) GO TO 1
        NTIMER = 0
        NSTEPS = NSTEPS + NMAX
    
        IF (NSTEPS >= 100*NMAX .AND. NSUB == 0) THEN
            NSTEPS = 0
            IF (KZ(1) > 1) CALL MYDUMP(1,1)
        END IF
    ...
    !     Include facility for termination of run (create dummy file STOP).
        IF(rank == 0)THEN
            ...
            IF (IO == 0) THEN
                CLOSE (99)
                IF (NSUB == 0 .AND. rank == 0) WRITE (6,90)
