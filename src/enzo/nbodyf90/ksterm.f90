...
!       Retain final KS variables for explicit restart at merge termination.
    IF (TIME <= TBLOCK .AND. IPHASE == 6) THEN
        HM(NMERGE) = H(IPAIR)
        DO 6 K = 1,4
...
!       Check optional diagnostic output for disrupted new hard binary.
    IF (KZ(8) == 0) GO TO 10
    IF (LIST(2,I1+1) /= 0 .OR. H(IPAIR) > 0.0) GO TO 10
    IF (GAMMA(IPAIR) > 0.5 .AND. JCOMP > 0 .OR. IPHASE == 7) THEN
        IF (JCOMP == 0 .OR. IPHASE == 7) JCOMP = I1
        K = 0
        IF (JCOMP > N) THEN
            J2 = 2*(JCOMP - N)
            K = LIST(2,J2)
    ...
    !**** Note:------------------------------------------------------------**
    !$$$          if(rank.eq.0.and.name(icm).eq.2567) call abort()
    !$$$          if(rank.eq.0.and.name(icm).eq.11569)
    !$$$     &         print*,'IFIRST',IFIRST,'N',NAME(IFIRST),'IPAIR',IPAIR,'N',
    !$$$     &         ,NAME(2*IPAIR-1),'LIST',LIST(1:6,I1),
    !$$$     &     'N',(NAME(LIST(2+K,I1)),K=1,5)
...
!       Copy all neighbours in the case of merger.
    IF (IPHASE == 6) THEN
        ...
        NBP = NNB - 1
    END IF
    IF (RS(ICM) > -100.0*BODY(ICM)/H(IPAIR)) A0 = 1.0
...
!       Select neighbours for components inside the modified c.m. sphere.
    ...
    !       Ensure that at least the predicted neighbour number is reached.
        IF (RIJ2 < RS2 .OR. L + NBP > NNB1 + NNB) THEN
            NNB1 = NNB1 + 1
            ILIST(NNB1) = J
        END IF
    25 END DO
...
!       Check that there is space for adding dominant component later.
    IF (NNB1 >= NNBMAX .AND. IPHASE /= 6) THEN
        RS2 = 0.9*RS2
        GO TO 20
    END IF
...
!       Include removal of the circularization name NAMEC from chaos table.
    IF (KSTAR(ICM) >= 10 .AND. NCHAOS > 0 .AND. IPHASE /= 6) THEN
...
!      CALL repair_tlist(ICOMP,JCOMP,ICM,2)
!     --04/15/14 21:40-lwang-debug--------------------------------------*
!**** Note:------------------------------------------------------------**
!$$$          if(rank.eq.0)
!$$$     &     print*,'NEW IFIRST',IFIRST,'N',NAME(IFIRST),'IPAIR',IPAIR,
!$$$     &     'N',NAME(2*IPAIR-1)
!     --04/15/14 21:40-lwang-end----------------------------------------*
!       Check replacing of single KS component by corresponding c.m.
    70 IF (LIST(2,ICOMP) < ICOMP) THEN
        J2 = LIST(2,ICOMP)
        J = KVEC(J2) + N
        DO 80 L = 2,NNB1
            IF (L < NNB1 .AND. LIST(L+1,ICOMP) < J) THEN
...
!       Form new force polynomials (skip triple, quad, merge & chain).
    ...
    !**** Note:------------------------------------------------------------**
    !$$$      if(time.ge.12.920379638671875) then
    !$$$         do i=ifirst,ntot
    !$$$            if(name(i).eq.560) then
    !$$$           print*,rank,'fp1',i,name(i),x0(1,i),x0dot(1,i),
    !$$$     *      step(i),t0(i),f(1,i),fdot(1,i),time
    !$$$           end if
    !$$$        end do
    !$$$      end if
    ...
    !$$$         do i=ifirst,ntot
    !$$$            if(name(i).eq.560) then
    !$$$           print*,rank,'fp2',i,name(i),x0(1,i),x0dot(1,i),
    !$$$     *      step(i),t0(i),f(1,i),fdot(1,i),time
    !$$$           end if
    !$$$        end do
    !$$$      end if
...
!**** Note:------------------------------------------------------------**
!$$$      if (rank.eq.0) then
!$$$         print*,'N',name(ICOMP),name(JCOMP),'S',stepr(icomp),step(icomp)
!$$$     &        ,stepr(jcomp),step(jcomp),time,'t0',t0(Icomp),t0(jcomp),
!$$$     &        'iphase',iphase
!$$$         call flush(6)
!$$$         if(stepr(jcomp).ge.100) then
!$$$            call abort()
!$$$         end if
!$$$      end if
!$$$      j = 15967
!$$$      print*,rank,'ksterm ',j,'n',name(j),'x',x(1,j),'xdot',xdot(1,j),
!$$$     &     'fdot',fdot(1,j),'time',time,'iphase',iphase
!$$$      call flush(6)
!$$$      call mpi_barrier(MPI_COMM_WORLD,ierr)
!$$$      if (time.ge.0.73070144653320313) then
!$$$         itest = 0
!$$$         print*,body(itest)
!$$$      end if
