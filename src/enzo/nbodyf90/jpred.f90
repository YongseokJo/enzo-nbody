...
!**** Note:------------------------------------------------------------**
!$$$      if(ttime.le.
!$$$     &     29.047119140625000.and.name(i).eq.12503) then
!$$$        write(120+rank,*),'t',ttime,'tpr',tpred(i),'xd',xdot(1,2*(i-n)),
!$$$     &        xdot(1,2*(i-n)-1),'x0d',x0dot(1,i),'x0',x0(1,i),
!$$$     &        'f',f(1,i),'fd',fdot(1,i),'nb',list(1,2*(i-n)-1),
!$$$     &        'U',U(1,i-n),'UD',UDOT(1,i-n),'i1',i-n,'t0',t0(i-n)
!$$$         call flush(120+rank)
!$$$         if(ttime.ge.29.047092579805405.and.
!$$$     &        tpred(i).ge.29.047090694399618) call abort()
!$$$      end if
!     --03/13/14 21:22-lwang-end----------------------------------------*
    IF(I < ifirst) return
!     --03/15/14 20:05-lwang-debug--------------------------------------*
!**** Note:------------------------------------------------------------**
!$$$      if(x0(1,I).ge.1000.and.body(i).gt.0) call abort()
!     --03/15/14 20:05-lwang-end----------------------------------------*
    IF (TPRED(I) == TIME) THEN
        IF (TPRED(I) == T0(I) .AND. X(1,I) /= X0(1,I)) THEN
            X(1:3,I) = X0(1:3,I)
    ...
    !**** Note:------------------------------------------------------------**
    !$$$         IF(I.EQ.12195) THEN
    !$$$            print*,rank,'JP I',I,'N',NAME(I),'X',X(1,I),'X1',X(1,J1),
    !$$$     &           'X2',X(1,J2),'T',TIME,'TPR',TPRED(I),'T0',T0(I),
    !$$$     &           'x0',x0(1,i)
    !$$$            call flush(6)
    !$$$         end if
