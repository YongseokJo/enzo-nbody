...
int ReceiveFromEnzo(std::vector<Particle*> &particle) {
	...
	***************************************************/
	...
	FixNumNeighbor = std::min((int) std::floor((NNB+newNNB-1)/2.0), FixNumNeighbor0);
...
#define no_star_formation_location_test
...
#ifdef star_formation_location_test
			new_particle.push_back(particle[NNB+i]);
#endif
		}
	...
	*/
	...
	fprintf(stderr, "NBODY+    : original NNB      = %d (+%d)\n", NNB-newNNB, newNNB);
	fprintf(stderr, "NBODY+    : newly updated NNB = %d\n", NNB, newNNB);
