...
*       Replace #JCOMP by arbitrary body in case it is the only neighbour.
      write(6,*) 'error-related outputs - SY'
      write(6,*) 'ICOMP = ',ICOMP,'JCOMP = ',JCOMP

      NNB = LIST(1,ICOMP)
      write(6,*) 'NNB of ICOMP = ',NNB
      write(6,*) 'fist neighbor number = ',LIST(2,ICOMP)
      call flush(6)
...
*     Exchange NXTLST index also
          ...
          ELSE
             write(6,*) 'exchange_tlist called at KSREG'
             flush(6)
...
*      Update the tlist array in intgrt
      IF(IPHASE.NE.8.AND.time+TOFF.ne.0.0D0) then
         ...
         write(6,*) 'shrink_tlist called at KSREG'
         flush(6)
         call shrink_tlist
      end if
*
