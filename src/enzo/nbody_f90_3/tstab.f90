...
!       Distinguish between absolute and approximate stability.
    ITERM = 0
    IF (PMIN1 > YFAC*PCRIT .OR. KZ(19) == 0 .OR. KZ(27) == 0) THEN
        TMDIS(NMERGE+1) = 1.0D+10
    ELSE IF (NAME(I) < 0 .OR. NAME(JCOMP) < 0) THEN
    !       Skip double hierarchy (RESET2 uses standard stability criterion).
        ITERM = 1
    ELSE IF (PMIN1 > 0.8*YFAC*PCRIT .OR. &
        (ECC1 > 0.99 .AND. ECC1 < 1.0)) THEN
    ...
    !       Note that large NK implies termination by other means.
        IF (ECC1 < 1.0 .AND. NK <= 1) ITERM = 1
    !       Modify PCRIT so it becomes < SEMI1*(1 - ECC1)*(1 - 2*PERT).
        IF (PCRIT < PMIN1) PCRIT = 0.999*PMIN1/YFAC
    ELSE IF (ECC1 < 1.0) THEN
