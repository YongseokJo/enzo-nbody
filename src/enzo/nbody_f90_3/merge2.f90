...
!       Select deepest level as primary in case of two hierarchies.
    IF (NAME(N+KSPAIR) < 0 .AND. NAME(JCOMP) < 0) THEN
    !       Note possibility of a quartet (or even quintet) and triple.
        IF (NAME(JCOMP) < NAME(N+KSPAIR)) THEN
            K = KSPAIR
            KSPAIR = JCOMP - N
            JCOMP = N + K
        END IF
...
!       Check for [[B,S],S] quartet or [[B,B],S] quintuplet.
    IF (NAME(JCOMP) > 0 .AND. NAME(JCOMP) <= NZERO) THEN
        CALL FINDJ(I,JG,IM)
        IF (KZ(19) >= 3 .AND. KZ(27) < 3) THEN
            JLIST(1) = I1
            JLIST(2) = I2
        ...
        END IF
        ZM = (BODY(I) + BODY(JCOMP))*SMU
        IF (rank == 0 .AND. JG <= N .AND. JCOMP <= N) THEN
