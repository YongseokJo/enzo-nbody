...
!$$$      if(rank.eq.0) then
!$$$      do J = ifirst,ntot
!$$$         rij2=body(j)*(xdot(1,j)**2+xdot(2,j)**2+xdot(3,j)**2)
!$$$         if(rij2.ge.5E-2) then
!$$$            print*,'J',J,'N',name(j),'F',F(1:3,j),'FD',FDOT(1:3,j),
!$$$     &           'm*v2',rij2,'T0',T0(j),'STEP',STEP(J),'m',body(j)
!$$$            call flush(6)
!$$$         end if
!$$$      end do
!$$$      end if
...
!     Obtain c.m. & angular momentum integrals and Z-moment of inertia.
    ...
    !     Update the maximum single body mass but skip compact subsystems.
        IF (NSUB == 0) BODY1 = MAX(BODY1,BODY(I))
    20 END DO
...
!     Subtract the kinetic energy of c.m. due to possible cloud effects.
    IF (KZ(13) > 0) ZKIN = ZKIN - 0.5*ZMASS*CMRDOT(4)**2
...
!     Determine average neighbour number and smallest neighbour sphere.
    ...
    DO 40 I = IFIRST,NTOT
        NNB = NNB + LIST(1,I)
        IF (LIST(1,I) > 0) RS0 = MIN(RS0,RS(I))
    40 END DO
...
!     Check optional determination of regularization parameters.
    ...
    !     Use harmonic mean to reduce fluctuations (avoid initial value).
        IF (TIME > 0.0D0) RMIN = SQRT(RMIN0*RMIN)
    ...
    !     Adopt central velocity as upper limit (avoids large kick velocities).
        IF (2.0*ZKIN/ZMASS > VC**2) ECLOSE = 2.0*VC**2
        IF (Q > 0.5) THEN
            ECLOSE = 0.5*ECLOSE/Q
            KSMAG = 0
        END IF
...
!     Test of Makino for large steps using STEPJ.
    IF (TIME <= 0.0D0) STEPJ = 0.01*(60000.0/FLOAT(N))**0.3333
...
!     Redefine the crossing time for 3D cluster orbit or Plummer model.
    IF ((KZ(14) == 3 .OR. KZ(14) == 4) .AND. ZKIN > 0.0) THEN
...
!     57 FORMAT(' IPE=',I4,' ttfrc=',f9.3)
    ...
    !     Print energy diagnostics & KS parameters.
        ICR = INT(TTOT/TCR0)
    !     New (Aug.1998) by P.Kroupa: (time also in Myr)
...
!     See whether standard output is due.
    ...
    !     Include optional diagnostics for the hardest binary below ECLOSE.
        IF (KZ(9) == 1 .OR. KZ(9) == 3) THEN
            HP = 0.0
            IP = 0
            DO 60 IPAIR = 1,NPAIRS
            !     Skip outer (ghost) binary of quadruple system.
                IF (H(IPAIR) < HP .AND. BODY(N+IPAIR) > 0.0D0) THEN
                    HP = H(IPAIR)
                    IP = IPAIR
                END IF
            60 END DO
            IF (IP > 0 .AND. HP < -ECLOSE) THEN
                I1 = 2*IP - 1
                I2 = I1 + 1
...
!     Save COMMON after energy check (skip TRIPLE, QUAD, CHAIN).
    TDUMP = TIME
    IF (KZ(2) >= 1 .AND. NSUB == 0) CALL MYDUMP(1,2)

!     Check termination criteria (TIME > TCRIT & N <= NCRIT).

    IF (TTOT*TSCALE > TCRITp .OR. TTOT > TCRIT - 20.0*DTMIN &
     .OR. N <= NCRIT) THEN
...
!     Check optional truncation of time.
    IF (KZ(35) > 0 .AND. TIME >= DTOFF) THEN
        CALL OFFSET(DTOFF)
    END IF
