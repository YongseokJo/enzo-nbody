...
!     allocate all nbody common variables
...
!----communication-with-ENZO-YS------------------------------------------*
    IF ((IEPHASE == 2) .OR. (IEPHASE == 3)) THEN

        SHUFFLECNT = 0

        DO 29 IP = 1,NTOT

            IF (NAME(IP) > N) GO TO 23
            ...
            DO JP = 1,N
                IF (IE == EID(JP)) THEN
                    ...
                    END DO
                    if (JP <= 3) write(0,*) 'enzo id matches!'
