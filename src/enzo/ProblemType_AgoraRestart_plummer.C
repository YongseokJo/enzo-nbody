...
public:
    ...
    HierarchyEntry &TopGrid, TopGridData &MetaData)
  {
    ...
    this->InitializeParticles(TopGrid.GridData, TopGrid, MetaData); //by Ahram Lee_test
			  ...
			  TopGridData &MetaData)
  {
    ...
    int nDisk, nNSC, nParticles, nBulge, nHalo; // by Ahram Lee
    ...
    // loading NSC particle by Ahram Lee
    nNSC = nlines("nsc.dat");
    if(debug) fprintf(stderr, "InitializeParticles: Number of NSC Particles %"ISYM"\n", nNSC);
    nHalo = nlines("halo.dat");
    if(debug) fprintf(stderr, "InitializeParticles: Number of Halo Particles %"ISYM"\n", nHalo);
    nParticles = nBulge + nDisk + nHalo + nNSC;
    ...
    this->ReadParticlesFromFile(
      Number, Type, Position, Velocity, Mass,
      //"nsc.dat", PARTICLE_TYPE_STAR, count, dx);
      "nsc.dat", PARTICLE_TYPE_MUST_REFINE, count, dx);
    this->ReadParticlesFromFile(
      Number, Type, Position, Velocity, Mass,
...
// by Ahram Lee
// Reading one vcirc.dat file for newnsc simulation(211001)
// Reading two vcirc.dat files
  void ReadInVcircData(void)
  {
    ...
    //fclose(fptr);
  //}
    // Read vcirc.dat file for NSC gas
    // by Ahram Lee
  ...
  float InterpolateVcircTable(FLOAT radius)
  {
    ...
    // Determine vcirc value for agora gas
    if (i == 0)
