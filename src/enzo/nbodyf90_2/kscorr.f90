...
!C      ITP = 0
!C      IF(GAMMA(IPAIR).GT.1.0D-04) ITP = 1
!       Perform iteration with or without re-calculating perturbation.
    ...
    !C*       Choose between old and optional new perturbation (first iteration).
    !C          IF (ITER.EQ.ITP) THEN
    !C*       Transform to improved coordinates & velocities.
    !C             I1 = 2*IPAIR - 1
    !C             call kstran(I1,UI,UIDOT,XI,VI)
    !C             NNB0 = LIST(1,I1)
    !C*       Re-calculate the perturbing force & derivative.
    !C             call KSPERT(I1,NNB0,XI,VI,FP,FD)
    !C          END IF
