...
!       Determine impact to each high-velocity particle for large STEPR.
    ...
    !!$omp parallel do private(J,NNB1,K,RV,VV,RIJ,DT,IT,R2,FJ2,FI2)
        ...
        !       Form relative quantities for large regular time-steps.
            IF (STEPR(J) < DTM .OR. J == I) GO TO 10
        !       Check whether body #I is already a neighbour (accept LIST > I).
            NNB1 = LIST(1,J) + 1
            DO 1 K = 2,NNB1
                IF (LIST(K,J) > I) GO TO 2
                IF (LIST(K,J) == I) GO TO 10
            1 END DO
            2 RV = 0.0
            VV = 0.0
            DO 3 K = 1,3
        ...
        !       Compare force at minimum distance with total force on body #J (> 0).
            FI2 = (BODY(I)/R2)**2
            IF (FI2 < 0.04*FJ2 .OR. BODY(J) == 0.0D0) GO TO 10
        
        !       See whether the regular time-step can be shortened.
            IF (T0R(J) + 0.5*STEPR(J) > TMIN .AND. IT < 5) THEN
            !                  if(rank.eq.0)
