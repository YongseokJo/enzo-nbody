...
!     Check ghost
    ...
    !**** Note:------------------------------------------------------------**
    !$$$         print*,'ADD GHOST J',J,'STEP',STEP(J),' NGHOSTS', NGHOSTS
...
!     Update smallest step level indicator
    IF(NDTMAX < K) NDTMAX = K
    IF(NDTMIN > K) NDTMIN = K
!     Safe check
    IF(NXTLIMIT >= NMAX) THEN
...
!     Increase nxtlst ending point
    NXTLIMIT = NXTLIMIT + 1
    IF(NGHOSTS > 0) NXTLST(NXTLIMIT+NGHOSTS) = NXTLST(NXTLIMIT)
...
!**** Note:------------------------------------------------------------**
!$$$      print*,'ADD J',J,'STEP',STEP(J),'NXTLIMIT',NXTLIMIT,'K',K,
!$$$     &     'NDTMAX',NDTMAX
!     --07/08/14 16:48-lwang-end----------------------------------------*
    RETURN

    end subroutine add_tlist
