...
!      endif
    ...
    IF (ABS(DMSUN) < 1.0D-12 .AND. .NOT. COALS) THEN
    !         if(rank.eq.0)
...
!       Distinguish between coalescence and surviving binary.
    ...
    !       Note no initialization of neighbours done in the chain case.
        call xbpredall
        DO 50 J = IFIRST,NTOT
            IF (J /= I1 .AND. J /= I2 .AND. J /= I3 .AND. J /= I4) THEN
                RIJ2 = (X(1,J) - XCM(1))**2 + &
                (X(2,J) - XCM(2))**2 + &
                (X(3,J) - XCM(3))**2
    ...
    !       Flag hyperbolic chain CE.
        ...
        KSTAR(I2) = KW2
        IPHASE = 0
        IF (ECC <= 0.001D0) NCE = NCE + 1
    !       Include possible NS or BH kick.
        IF (KW1 >= 10 .AND. KW1 <= 14) THEN
