...
!     Reset TBH after restart.
    IF (TBH == 0.D0 .AND. TIME+TOFF /= 0.D0 .AND. KSTART > 1) &
...
!     IF not reach next output time, return
    IF (TBH > TIME+TOFF) RETURN

!     Output labels
    IF(IBH == -1 .AND. KSTART == 1 .AND. rank == 0) then
...
!     Search for massive black hole
    IF(IBH == -1) IBH = NIMBH
    IF(IBH >= 1 .AND. NAME(IBH) == NIMBH) GO TO 1

!     Search index of IMBH
    IF(ABS(N-IBH) > ABS(IFIRST-IBH)) THEN
        IBH = 0
        DO K = 1,N
            IF (NAME(K) == NIMBH) THEN
                IBH = K
                GO TO 1
            END IF
        END Do
...
!     Output data
    1 IF(IBH > 0) THEN
        ...
        !$$$            write(120,*),'TIME',TIME,'IBL',IBL,'X',X(1,IBL),
        !$$$     &           'XD',XDOT(1,IBL)*VSTAR,'M',BODY(IBL)*ZMBAR,
        !$$$     &              'F',F(1,IBL),'FD',FDOT(1,IBL),
        !$$$     &              'STEP',STEP(IBL),'STEPR',STEPR(IBL),
        !$$$     &              'FI',FI(1,IBL),'FID',FIDOT(1,IBL),
        !$$$     &              'T0',T0(IBL),'T0R',T0R(IBL),'NB',LIST(1,IBL),
        !$$$     &              'FR',FR(1,IBL),'FRD',FRDOT(1,IBL)
        !     &           'XBH',X(1,IBH),'XDBH',XDOT(1,IBH)*VSTAR,
        !     &           'X2',X(1,IBH+1),'XD2',XDOT(1,IBH+1)*VSTAR,
        !     &           'MBH',BODY(IBH)*ZMBAR,'M2',BODY(IBH+1)*ZMBAR
        !$$$            call flush(120)
        !            END IF
        !$$$            IF(XDOT(1,IBL)*VSTAR.GT.100) call abort()
