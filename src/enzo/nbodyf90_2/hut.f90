...
!     Include step reduction for large de/dt or primary spin rate.
    IT = 0
    1 IF (ABS(udot(1))*dtau > 0.01*MAX(es0,0.01d0)) THEN
        dtau = 0.5D0*dtau
        nsteps = 2*nsteps
        IT = IT + 1
        IF (IT < 10) GO TO 1
    ...
    2 IF (ABS(udot(2))*dtau > 0.01*u(2) .AND. u(2) > 0.1) THEN
        dtau = 0.5D0*dtau
        nsteps = 2*nsteps
        IT = IT + 1
        IF (IT < 10) GO TO 2
...
!     Treat large eccentricity carefully for rapid spin change.
    IF (es0 > 0.9995 .AND. ABS(udot(2))*dtau > &
    0.01*u(2) .AND. u(2) > 0.1) THEN
        dtau = 0.5D0*dtau
        nsteps = 2*nsteps
        IT = IT + 1
    END IF
...
!      END IF
    ...
    !     Note there are 4 calls to deriv2 when u(1) may go negative.
        IF (u(1) < 0.002 .OR. u(1) > 0.99999) then
        !     Enforce circularization by adopting e=0.00199 and copying spins.
            IF (u(1) < 0.0) WRITE (6,4) I, nsteps, u(1)
...
!     DATA IC /0/
    ...
    if(e <= 0.0 .OR. e >= 1.0 .OR. oa < 0.0)then
        udot(1) = 0.0
        udot(2) = 0.0
        udot(3) = 0.0
    else
    ...
    if (e < 0.002 .AND. semi < 0.0d0) semi=semi1
