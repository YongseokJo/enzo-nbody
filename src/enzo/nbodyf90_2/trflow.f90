...
!       Compare scaled Roche radii when choosing the primary.
    IF (RADIUS(J1)/RL1 < RADIUS(J2)/RL2) THEN
        J1 = I1 + 1
        J2 = I1
        RL1 = RL2
    END IF
...
! 9      FORMAT(' TRFLOW WARNING! ',3I6,I4,2F10.3)
!     ENDIF
...
!       Exit with large interval if semi-major axis is negative.
    ...
    ELSEIF(KSTAR(I) > 0 .AND. MOD(KSTAR(I),2) /= 0)THEN
        IF(TEV(I) > 9.9D+09) TEV(I) = TEV0(J1)
        DTR = TEV(I) - TIME
        GOTO 210
    ENDIF
    ...
    IF(KW <= 1 .OR. KW == 7)THEN
        ...
        ! Find the overflow time using binary chopping.
            ...
            ! Stop when 0.1% accuracy is achieved.
                ...
                !                  ENDIF
                ENDIF
                IF(ABS(RR - RLS) <= 0.001*RR .OR. IT > 25) GOTO 200
                THIGH = T
                RHIGH = RR
            ENDIF
            GOTO 10
...
! lobe before helium ignition.
    ...
    IF(KW <= 3)THEN
        ...
        ! then for the overflow time.
            ...
            30 IT = IT + 1
            DEL = RGBF(M1,LUM) - RLS
            IF(IT == 25 .AND. ABS(DEL) > 0.1)THEN
            !               if(rank.eq.0)then
            !               WRITE(38,*)' TRFLOW KW3: ',rgbf(m1,lum),rls,lum,lums(3)
            !               end if
            ENDIF
            IF(ABS(DEL/RLS) <= TOL .OR. IT == 25) GOTO 40
            DER = RGBDF(M1,LUM)
            LUM = LUM - DEL/DER
            GOTO 30
            40 CONTINUE
        ...
        ! any further until it has actually become a type 4.
            IF(M0 < ZPARS(2) .OR. TN <= TSCLS(2)) GOTO 200
        ENDIF
    ENDIF
...
! The star now has only until the end of the AGB phase to fill its lobe.
    ...
    ! then for the overflow time.
        ...
        ENDIF
        IF(ABS(DEL/RLS) <= TOL .OR. IT == 25) GOTO 80
    ...
    ! The star has until the end of the Helium GB to fill its lobe.
        ...
        ! Find the overflow luminosity using binary chopping.
            ...
            ! Stop when 0.1% accuracy is achieved.
                IT = IT + 1
                IF(IT == 25)THEN
                    IF(ABS(RR-RLS) > 0.1)THEN
                    !                     if(rank.eq.0)WRITE(38,*)' TRFLOW KW9: ',rr,rls,t,tm
                    ENDIF
                ENDIF
                IF(ABS(RR - RLS) <= 0.001*RR .OR. IT > 25) GOTO 95
                THIGH = LUM
                RHIGH = RR
            ENDIF
            GOTO 90
    ...
    ELSE
        ...
        ENDIF
        DTR = MAX(DTR,0.D0)
        IF(TEV(J1) < TEV0(J1))THEN
            if(rank == 0) WRITE (6,101)J1,KSTAR(J1),TEV(J1)+TOFF,TTOT
            101 FORMAT(' TRFLOW WARNING! TEV<TEV0 ',I6,I4,2F10.3)
        ENDIF
    ENDIF

    210 RETURN
    END SUBROUTINE TRFLOW
!**
