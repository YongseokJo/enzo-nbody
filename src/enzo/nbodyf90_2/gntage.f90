...
! Next we check that we don't have a GB star for M => Mfgb
    ...
    ! Use the bisection method to find m0
        ...
        do 10 j = 1,jmax
            ...
            aj*mcagbf(mmid) - mc
            if(fmid < 0.d0) m0 = mmid
            if(abs(dm) < macc .OR. ABS(fmid) < tiny) goto 20
        10 END DO
        20 continue
    ...
    ! Use the bisection method to find m0
        ...
        do 70 j = 1,jmax
            ...
            fmid = mcy - mc
            if(fmid < 0.d0) m0 = mmid
            if(abs(dm) < macc .OR. ABS(fmid) < tiny) goto 80
        70 END DO
        80 continue
