...
!       Skip rectification for small eccentricity or large perturbation.
    ...
    ECC = SQRT(ECC2)
    IF (ECC <= 0.01) GO TO 50
    IF (GAMMA(IPAIR) > 0.1) GO TO 50

!       Include Roche rectification for large ECC (additional to roche.f).
    IF (KSTAR(I) > 10 .AND. ECC > 0.2) THEN
        QP = SEMI*(1.0 - ECC)
        ICIRC = -1
    ...
    DB = ZMU*(HI - H(IPAIR))
    IF (ABS(DB) > 1.0D-08) THEN
        SEMI = -0.5*BODY(N+IPAIR)/H(IPAIR)
        RA = R(IPAIR)/SEMI
        IF (SEMI < 0.0) RA = R(IPAIR)
    !          if(rank.eq.0)
