...
!       Define global indices with body #I1 being most evolved.
    IF(KSTAR(J1) >= KSTAR(J2))THEN
        I1 = J1
        I2 = J2
        IF(KSTAR(J1) <= 1 .AND. BODY(J1) < BODY(J2))THEN
            I1 = J2
            I2 = J1
        ENDIF
    ELSE
...
!       Check for planetary systems - defined as HeWDs - and low-mass WDs!
    IF(K1 == 10 .AND. M1 < 0.05)THEN
        ICASE = K2
        IF(K2 <= 1)THEN
            ICASE = 1
            AGE1 = 0.D0
        ENDIF
    ELSEIF(K1 >= 11 .AND. M1 < 0.15 .AND. ICASE == 6)THEN
        ICASE = 1
    ENDIF
    IF(K2 == 10 .AND. M2 < 0.05)THEN
        ICASE = K1
        IF(K1 <= 1)THEN
            ICASE = 1
            AGE2 = 0.D0
...
!      Form binding energy of secondary and orbital kinetic energy.
    ...
    IF (ZKE + EB > 0.0 .AND. RIJ < 0.5*(RADIUS(I1)+RADIUS(I2)) .AND. &
    MAX(K1,K2) < 2) THEN
        ZM = ABS(ZKE/EB)
        FAC = 1.0/SQRT(ZM)
...
!       Evaluate apparent age and other parameters.
    ...
    ELSEIF(ICASE == 3 .OR. ICASE == 6 .OR. ICASE == 9)THEN
        MC3 = M1
    ...
    !       Ensure that a new WD has the initial mass set correctly.
        M03 = M3
        DT = 1.0d+04
    ELSEIF(ICASE == 13 .OR. ICASE == 14)THEN
    ...
    !       unless the less evolved star is a WD, NS or BH.
        IF(K2 < 10)THEN
            ...
            DM = M2
            NTZ = NTZ + 1
            if(rank == 0)WRITE (6,2)  K1, K2, NAME(I1), NAME(I2)
    ...
    !       This should not be reached.
        if(rank == 0)WRITE(6,*)' ERROR MIX: ICASE NOT CAUGHT!!!'
        if(rank == 0)WRITE(6,*)' K1 K2 -> K3 ',K2,K2,KW
        STOP
    ENDIF
...
!       Determine consistent stellar type and specify mass loss.
    IF(KW <= 14)THEN
        ...
        IF(KW /= KW0)then
            DM = M1 + M2 - M3
            if(rank == 0)WRITE (6,5)  NAME(I1), NAME(I2), KW0, KW, DM
...
!       Check for blue straggler formation (TM < TPHYS & KSTAR <= 1).
    IF(TMS3 < TPHYS .AND. KSTAR(I1) <= 1)THEN
        if(rank == 0)WRITE (6,8)  NAME(I1), M3, TMS3, TPHYS, AGE3
...
!       Open unit #13 the first time.
    IF(rank == 0 .AND. FIRST .AND. IQCOLL /= 3 .AND. KSTART == 1)THEN
