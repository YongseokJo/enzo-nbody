...
!       Quit if active ROCHE or mass-losing star.
    ITERM = 1
    IF(KSTARM(IM) == 11 .OR. KSTARM(IM) == 13)THEN
        GO TO 30
    END IF
    IF (M1 >= 10.0 .OR. (KSTAR(I1) > 1 .AND. KSTAR(I1) < 10))THEN
        GO TO 30
    END IF
    IF (M2 >= 10.0 .OR. (KSTAR(I2) > 1 .AND. KSTAR(I2) < 10))THEN
        GO TO 30
    END IF
...
!       Identify the most magnetically active star (#J2).
    IF (RADIUS(I1) >= RADIUS(I2)) THEN
        IF ((KSTAR(I1) == 1 .AND. M1 > 1.25) .OR. &
        (KSTAR(I1) == 0 .AND. M1 < 0.35)) THEN
            J2 = I2
        ELSE
            J2 = I1
        END IF
    ELSE
        IF ((KSTAR(I2) == 1 .AND. M2 > 1.25) .OR. &
        (KSTAR(I2) == 0 .AND. M2 < 0.35)) THEN
            J2 = I1
        ELSE
            J2 = I2
        END IF
...
!       Suppress magnetic braking for massive MS/low-mass or evolved star.
    IF (((M2 > 1.25 .OR. M2 < 0.35) .AND. KSTAR(J2) <= 2) .OR. &
    KSTAR(J2) >= 10) THEN
        ADOT = 0.0
        TBR = 1.0D+06
    END IF
...
!       Include safety test on new semi-major axis.
    IF (ABS(SEMI1) < RADIUS(J2) .OR. SEMI1 < 0.0) THEN
        SEMI1 = RADIUS(J2)
    END IF
...
!       Update radius for MS stars to current time.
    ...
    IF(KSTAR(I2) <= 1)THEN
        ...
        ENDIF
    ENDIF
    IF(KSTAR(I1) >= 10 .AND. TM0 > 0.D0) TEV0(I1) = TIME
    IF(KSTAR(I2) >= 10 .AND. TM0 > 0.D0) TEV0(I2) = TIME
...
!       Check for Roche mass transfer at every stage (AM CVn formation).
    IF (RADIUS(J1) > RL1 .AND. KZ(34) > 0) THEN
