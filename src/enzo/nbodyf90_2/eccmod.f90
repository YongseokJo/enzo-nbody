...
!       Quit for tidal dissipation or circular orbit (TMDIS set by IMPACT).
    IF (KSTARM(IM) == -2 .OR. KSTARM(IM) >= 10) THEN
        GO TO 50
    END IF
...
!       Skip on hyperbolic outer orbit, double merger or circularized orbit.
    IF (H(IPAIR) > 0.0 .OR. NAME(I) < -2*NZERO .OR. &
    KSTARM(IM) == 10) THEN
...
!       Delay for long growth times or aged active SPIRAL.
    IF ((KSTARM(IM) >= 0 .AND. TG > 20.0) .OR. IQ > 0 .OR. &
    (KSTARM(IM) == -2 .AND. MAX(ECC,EMAX) < 0.9) .OR. &
    (KSTARM(IM) < 0 .AND. ECC < 0.1)) THEN
        RM = MAX(RADIUS(I1),RADIUS(IGHOST),1.0D-20)
        IDELAY = IDELAY + 1
        IF (EMAX > 0.99 .AND. MOD(IDELAY,10) == 0) THEN
            ALPH = 360.0*ZI/TWOPI
        ...
        DT = 10.0
        IF (LIST(1,I1) > 0 .AND. SEMI*(1.0-ECC) < 5.0*RM) THEN
            DT = 1.0
        END IF
...
!       Include diagnostics every 5000th time.
    IF (ITIME == 1 .OR. MOD(ITIME,5000) == 0) THEN
...
!       Include termination for expected short circularization time.
    IF (KSTARM(IM) >= 0 .AND. PMIN < 3.0*RM) THEN
        ZID = 360.0*ZI/TWOPI
