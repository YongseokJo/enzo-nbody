...
! omp& private(ipair,icm,i1,i2,eb,semi,tk,vi,ui,tp,imod,K)
    ...
    !     Transform any unperturbed hard binary to apocentre and set time-step.
        ...
        IF (LIST(1,I1) == 0 .AND. EB < EBH) THEN
    ...
    !     Estimate an appropriate KS slow-down index for G < GMIN.
        IMOD = 1
        IF (LIST(1,I1) == 0 .AND. SEMI > 0.0) THEN
            TK = TWOPI*SEMI*SQRT(SEMI/BODY(ICM))
    ...
    !     Diagnostics
    !         if(rank.eq.0) then
    !            write(6,60) IPAIR, NAME(ICM), NAME(I1),NAME(I2),STEP(ICM),
    !     &           STEP(I1),LIST(1,ICM),LIST(1,I1),GAMMA(IPAIR),IMOD
    ! 60         format('KS PAIR: IPAIR',I10,' NAME(ICM,I1,I2)',3I10,
