...
!       Obtain reliable semi-major axis for small pericentre or large EB.
    ...
    !       Evaluate semi-major axis of K1 & K2 from non-singular variables.
        ...
        ERR = (a1 - SEMI)/SEMI
        IF (ABS(ERR) > 1.0E-05 .AND. IWARN < 10) THEN
            IWARN = IWARN + 1
            ZK = 1.0
            DO I = 1,N-1
                ZK = MAX(KSCH(I),ZK)
            END DO
            if(rank == 0)WRITE (6,4)  NSTEP1, ZK, g1, RM, SEMI, ERR
...
!       Adopt safety measure to avoid negative argument (Seppo Mikkola).
    ...
    IT = IT + 1
    IF (ABS(DT - DT1) > 1.0D-06*ABS(DT) .AND. IT < 25) GO TO 10

!       Accumulate the integral of dt/R.
    SUM2 = SUM2 + 2.0*MC(i1)*MC(i1+1)*Y

    IF (IT > 15 .AND. IWARN < 50) THEN
        IWARN = IWARN + 1
        ECC = SQRT((1.0 - RM/a1)**2 + ETA**2/(MB*a1))
        if(rank == 0)WRITE (6,15) IT,KSCH(I1),ECC,RM,a1,DT-DT1,DTAU
...
!       Combine the two terms and ensure positive value.
    ...
    IF (DTAU < 0.0) THEN
        ...
        IF (IWARN < 100) THEN
            IWARN = IWARN + 1
            if(rank == 0)WRITE (6,16)KSCH(i1),RM/A1,SUM1*DT0,SUM2,DTAU
