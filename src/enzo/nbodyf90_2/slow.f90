...
!       Perform perturbation check if slow-down not active.
    ...
    !       Check carefully two possible binaries (eccentricity effect).
        ...
        !       Switch to #I2 if second binary is smaller or first pair is not bound.
            IF (SEMI2 > 0.0) THEN
                IF (SEMI2 < SEMI .OR. SEMI < 0.0) THEN
                    I1 = I2
                    SEMI = SEMI2
                    RM = RINV(I2)
                END IF
            !       Exit in case of two hyperbolic two-body motions.
            ELSE IF (SEMI < 0.0) THEN
                GO TO 100
            END IF
        END IF
...
!       Determine chain index for slow-down binary (otherwise from above).
    IF (KSLOW .AND. N > 3) THEN
        DO 20 I = 1,N-1
            IF (KSCH(I) > 1.0D0) i1 = i
        20 END DO
    !       See whether a closer particle pair is present (factor of 2).
        ...
        DO 60 I = 1,N-1
            RI2 = 1.0/RINV(I)
            IF (I /= I1 .AND. R1 < 0.5*RI2) THEN
                I2 = I
                IF (R1 < 0.1*SEMI .AND. RI2 < 0.1*SEMI) THEN
                    GO TO 100
                END IF
            !       Compare closest separation with current slow-down binary.
                IF (RI2 < 0.5*SEMI .AND. R1 > 0.5*SEMI) THEN
                !       Evaluate semi-major axis directly (cf. small RI2 in EREL).
                    ...
                    SEMI = 1.0D0/SEMI
                    GO TO 80
                ELSE IF (RI2 < MIN(SEMI,R1) .AND. R1 > 0.5*SEMI) THEN
                !       Determine second binary by regular expression (R1 not too small).
                    ...
                    IF (SEMI2 > 0.0 .AND. SEMI2 < SEMI) THEN
                        KSNEW = 1.0D0
                        GO TO 90
                    ELSE
...
!       Form modified mass factors.
    ...
    do i = 1,n-1
        ...
        if (i > 1) TKK(i-1) = tkk(i-1) + dt12
        if (i < n-1) TKK(i+1) = tkk(i+1) + dt12
        if (i > 1 .AND. i < n-1) TK2(i) = -2.0d0*dt12
    end do
    TK2(0) = 0.0
    TK2(N) = 0.0
