...
#endif  // end FAST_SIB
{
	...
	for (StartGrid = 0; StartGrid < NumberOfGrids; StartGrid += GRIDS_PER_LOOP) {
		...
		/* Finally, receive the data and process it. */
	fprintf(stdout,"Proc:%d, 4-3\n", MyProcessorNumber); // by YS

		CommunicationReceiveHandler();
		fprintf(stdout,"4-4"); // by YS
	...
	if (traceMPI) 
		fprintf(tracePtr, "PrepareDensityFieldNoStar: P(%"ISYM"): PGMF1 (send)\n", 
				MyProcessorNumber);

	TIME_MSG("PrepareGravitatingMassFieldNoStar1");
	LCAPERF_START("PrepareGravitatingMassFieldNoStar1");
	for (StartGrid = 0; StartGrid < NumberOfGrids; StartGrid += GRIDS_PER_LOOP) {
		EndGrid = min(StartGrid + GRIDS_PER_LOOP, NumberOfGrids);
		...
		/* Finally, receive the data and process it. */
		fprintf(stdout,"4-3No"); // by YS

		CommunicationReceiveHandler();
		fprintf(stdout,"4-4No"); // by YS

	} // ENDFOR grid batches
	LCAPERF_STOP("PrepareGravitatingMassFieldNoStar1");
	...
	if (traceMPI) 
		fprintf(tracePtr, "PrepareDensityFieldNoStar: P(%"ISYM"): PGMF2 (receive)\n", 
				MyProcessorNumber);

	TIME_MSG("PrepareGravitatingMassFieldNoStar2");
	LCAPERF_START("PrepareGravitatingMassFieldNoStar2a");
	for (StartGrid = 0; StartGrid < NumberOfGrids; StartGrid += GRIDS_PER_LOOP) {
		EndGrid = min(StartGrid + GRIDS_PER_LOOP, NumberOfGrids);
		...
		fprintf(stdout,"4-6No"); // by YS
		CommunicationReceiveHandler();
#endif /* BITWISE_IDENTICALITY */
		fprintf(stdout,"4-7No"); // by YS

	} // ENDFOR grid batches
	LCAPERF_STOP("PrepareGravitatingMassFieldNoStar2a");
	...
	/************************************************************************/
	LCAPERF_START("PrepareGravitatingMassFieldNoStar2b");
	for (StartGrid = 0; StartGrid < NumberOfGrids; StartGrid += GRIDS_PER_LOOP) {
		EndGrid = min(StartGrid + GRIDS_PER_LOOP, NumberOfGrids);
	...
	LCAPERF_STOP("PrepareGravitatingMassFieldNoStar2b");

	fprintf(stdout,"4-8No"); // by YS
#endif
