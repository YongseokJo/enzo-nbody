...
int UpdateParticlePositions(grid *Grid)
{
  ...
  LCAPERF_START("UpdateParticlePositions");
	...
	// By YS, EnzoNbody
	// We have to make sure that the subject particles are not updated via the old processes.
	//
  ...
  if (Grid->UpdateParticleVelocity(0.5*dt) == FAIL) {
	...
	// By YS, EnzoNbody
	// Here, soem functions should be added to update the positions and velocity of
	// the subject particles.
