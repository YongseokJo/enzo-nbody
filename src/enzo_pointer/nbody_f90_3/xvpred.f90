...
!$$$       if(name(i1).eq.6537) then
!$$$        write(123+rank,*) i1,name(i1),(x(k,i1),k=1,3),(xdot(k,i1),k=1,3)
!$$$     *        ,(x0(k,i1),k=1,3),(x0dot(k,i1),k=1,3),t0(i1),step(i1)
!$$$     &        ,(f(k,i1),k=1,3),(fdot(k,i1),k=1,3),time,tvpred(i1)
!$$$      end if
...
!       Use high order for special singles (NNB < 0) or c.m. bodies (I1 > N).
    IF (NNB < 0 .OR. (NNB == 0 .AND. I1 > N)) GO TO 20

!       Adopt low order for standard single particles (I1 <= N & NNB = 0).
    IF (NNB == 0) THEN
        IF(TVPRED(I1) == TIME) GO TO 40
        I = I1
        S = TIME - T0(I)
...
!       Predict all neighbours to order FDOT.
    ...
    !      if(time.ge.3.3985595703125000.and.name(i1).eq.6537) then
    !$$$       if(name(i1).eq.6537) then
    !$$$         write(123+rank,*) i,name(i),(x(k,i),k=1,3),(xdot(k,i),k=1,3)
    !$$$     *        ,(x0(k,i),k=1,3),(x0dot(k,i),k=1,3),t0(i),step(i)
    !$$$     &        ,(f(k,i),k=1,3),(fdot(k,i),k=1,3),time,tvpred(i)
    !$$$      end if
    !     --09/25/13 19:31-lwang-end----------------------------------------*
        IF(TVPRED(I) == TIME) GO TO 5
        S = TIME - T0(I)
...
!      IF ((DT.EQ.0.0D0.AND.IPHASE.LT.4).OR.BODY(I).EQ.0.0D0) GO TO 35
    IF (BODY(I) == 0.0D0) GO TO 35
    IF (DT == 0.0D0 .AND. IPHASE < 4) THEN
        IF (X(1,I) == X0(1,I) .AND. XDOT(1,I) == X0DOT(1,I)) go to 35
        X(1,I) = X0(1,I)
        X(2,I) = X0(2,I)
        X(3,I) = X0(3,I)
        XDOT(1,I) = X0DOT(1,I)
...
!     --09/26/13 14:51-lwang-end----------------------------------------*
    ...
    !**** Note:------------------------------------------------------------**
    !$$$       if(time.ge.12.920379638671875.and.name(i).eq.560) then
    !$$$          print*,rank,'xv',a1,a2,a3,a4,dt,f3dotk,f2dotk,f1dotk,fk,time
    !$$$          call flush(6)
    !$$$       end if
