...
!       Count all escapers.
    ...
    DO 60 I = IFIRST,NTOT
        ...
        !** FlorentR - skip contribution of stars that have been identified as unbound in first pass
            IF (NUNBOUND(J) == 0) POTI = POTI + BODY(J)/SQRT(RIJ2)
        !** FRenaud
            #else
...
!** FlorentR - iterative scheme and output
    ...
    IF(NITER < 5) THEN
        IF(ABS(NESC2-NOLDESC2) > NESC2/10.0) THEN
