...
!$$$#ifdef GPU
!$$$      IGPUDM = .true.
!$$$      call gpupot_send(rank,NTOT-IFIRST+1,BODY(IFIRST),X(1,IFIRST))
!$$$#endif
!$$$      NCNUM = 0
...
!       Include braking or merger termination at time for Roche overflow.
    IF (I > N .AND. NAME(I) < 0 .AND. KZ(34) > 0) THEN
        KSPAIR = I - N
        ...
        ! 900             FORMAT('BRAKE2 MERGER TERM:    K* NAME(ICM) Time[NB] ',
        !     &                'R12[NB] ',I4,I12,1P,E26.17,E15.6)
        !                 CALL FLUSH(14)
        !             END IF
            JCOMP = 0
            IPHASE = 7
...
!       Check for hierarchy containing NS/BH (same TEV for inner components).
    IF (I < IFIRST .AND. KZ(28) > 0) THEN
        IP = KVEC(I)
        IF (NAME(N+IP) < 0 .AND. KSTAR(I) >= 13) THEN
...
!     Check possible Roche overflow condition (skip any ghost c.m.).
    IF (I > N .AND. KZ(34) > 0 .AND. BODY(I) > 0.0D0 .AND. &
    KSTAR(I) >= 10) THEN
        IPAIR = I - N
    !       Consider odd types (11, 13, etc.) for Roche updating or treatment.
        IF (MOD(KSTAR(I),2) /= 0) THEN
        !     Do enery correction before particle index change
        !             call encorr_mdot(NPNUM,NDMLIST,DMLIST,IGPUDM,
        !     &            IGPUSWITCH)
            CALL ROCHE(IPAIR)
        ...
        !                  IGPUSWITCH = .true.
        ! endif
            IF (IQCOLL /= 0 .OR. IPHASE < 0) GO TO 1
        !       Ensure that look-up time is increased in case of no mass transfer.
            IF (TEV(I) <= TIME) THEN
                IF (MOD(KSTAR(I),2) /= 0) KSTAR(I) = KSTAR(I) + 1
        ...
        !       Include other cases (even types outside 10 R_sun).
            ...
            !                  IGPUSWITCH = .true.
            ! endif
                IF (TEV(I) <= TIME) TEV(I) = 1.000002d0*TIME
            ELSE
                IF(KZ(34) == 1)THEN
                    CALL SYNCH(IPAIR)
                ELSE
...
!       Check for special treatment of possible hierarchical system (IHDOT).
    ...
    !       Include Roche consistency check (single components not permitted).
        IF (KSTAR(ICM) > 10 .AND. MOD(KSTAR(ICM),2) == 1) THEN
            IF(BODY(I) == 0.0 .OR. BODY(I2) == 0.0)THEN
                TEV(I) = TIME + MIN(0.1D0,TEV(I)-TEV0(I))
                TEV(I2) = TEV(I)
                GOTO 1
            ENDIF
            IWARN = IWARN + 1
            IF (rank == 0 .AND. MOD(IWARN,100) == 0) THEN
...
!       Determine relevant indices in case of merger (zero mass done below).
    ...
    !       Determine index of mass-losing star for inner or outer binary.
        IF (IGHOST <= N) THEN
            IF (TEV(IGHOST) < TEV(I)) I = IGHOST
            IF (rank == 0 .AND. I == IGHOST) WRITE(6,906) I, NAME(I), &
        ...
        !       Compare look-up times of #I and IGHOST <= N on first KS component.
            ...
            END IF
        END IF
    ELSE IF (BODY(I) <= 0.0D0 .OR. NAME(I) == 0) THEN
    ...
    !       Define KS index and set indicator on successful identification.
        ...
        !       Allow an extra amount for MS stars (depending on TEV - TEV0).
            IF(KSTAR(I) <= 1 .AND. (TEV(I)-TEV0(I))*TSTAR < 10.0)THEN
...
!       Skip any c.m. particles after increasing look-up time.
    ...
    !    &                       TTOT, TM - TEV(I)
    ! 10            FORMAT(' WARNING!    MDOT:    I NAM K* T D(TEV) ',
    !    &                               2I6,3I4,F9.2,1P,E10.2)
    !            END IF
    !         ENDIF
        IF (KSTAR(I) < 10) THEN
            TEV(I) = 1.0d+10
        ELSE IF (KZ(34) > 0) THEN
            IPAIR = I - N
            CALL TRFLOW(IPAIR,DTR)
            DTR = MAX(DTR,0.0D0)
            IF (DTR < STEP(I)) THEN
...
!       component of a hierachy and if so set the mass accretion flag.
    ...
    !       Check for zero c.m. mass (i.e. NAME < 0 & > -NZERO in [[B,S],[B,S]]).
        ...
        IF(ECC2 < 1.0)THEN
            ...
            ISWAP = 0
            IF(KSTAR(JX(1)) <= 1 .AND. &
            (KSTAR(JX(2)) == 5 .OR. KSTAR(JX(2)) == 6))THEN
                ISWAP = 1
            ELSEIF(RXL2 > RXL1)THEN
                ISWAP = 1
            ENDIF
            IF(ISWAP > 0)THEN
                J1 = JX(1)
    ...
    !       Ensure that type change occurs at time TEV.
        IF(KW /= KSTAR(I) .OR. DABS(M1-M0)/M0 > 0.05D0)THEN
            IF (rank == 0 .AND. KW >= 13) THEN
    ...
    !       Evaluate mass loss due to stellar wind.
        RLPERI = 0.D0
        IF(KACC > 1) RLPERI = ROL(K)*(1.D0 - ECC)
    ...
    !       Check for unequal evolution times in a recently formed binary.
        IF(K == 2)THEN
            IF(DTX(1) == 0.D0 .OR. DTX(2) == 0.D0) DT = DTX(2)
        ENDIF
    ...
    !       Restrict the time-step for extreme conditions (JH 05/09).
        ...
        !       Tidal synchronisation (only operates on circular orbits).
            Q = MASS(3-K)/MASS(K)
            IF(K == 1) DJT = 0.D0
            IF(KZ(34) == 2 .AND. DT > 0.D0 .AND. ECC <= 0.01D0)THEN
                IF((KW <= 9 .AND. RAD(K) >= 0.01D0*ROL(K)) .OR. &
                (KW >= 10 .AND. Q >= 1.D0))THEN
                
                    CALL BSETID(kw,mass(k),massc(k),menv(k),rad(k), &
                    radc(k),renv(k),lumin(k),ospin(k),k2str(k), &
                    q,sep,ecc,oorb,delet,dspin,eqspin,djti)
                
                    IF(ABS(DSPIN) > TINY)THEN
                        DSPIN0 = DSPIN
                        IF(DSPIN >= 0.D0)THEN
...
!       gravitational radiation for close binary systems.
    ...
    !       Limit orbital angular momentum change to 2%.
        ...
        IF(ABS(DJORB) > TINY .AND. (DTXMIN > 0.D0 .OR. DIFF == 0.D0))THEN
            DTGR = 0.02D0*JORB/ABS(DJORB)
            IF(DELET > TINY .AND. ECC > 0.0011D0)THEN
    ...
    !       Set indicator for mass loss correction.
        ...
        !       Check rejuvenation of MS, HG or HE star.
            IF(KW <= 2 .OR. KW == 7)THEN
                ...
                CALL STAR(KW,M0,M1,TM,TN,TSCLS,LUMS,GB,ZPARS)
                IF(KW == 2)THEN
                    IF(GB(9) < MASSC(K) .OR. M10 > ZPARS(3))THEN
                        M0 = M10
                    ELSE
    ...
    !       Check for blue straggler formation (TM < TPHYS).
        IF(DMR > TINY .AND. KW <= 1 .AND. NAME(I) /= IBLUE)THEN
            TPHYS2 = (TEV0(I)+TOFF)*TSTAR - EPOCH0
            IF(TM0(K) > TPHYS2 .AND. TM < 0.98D0*TPHYS2)THEN
                J = JX(1)
                IF(I == J .AND. KACC == 2) J = JX(2)
                if(rank == 0) &
    ...
    !       Obtain stellar parameters at current epoch.
        ...
        IF(rank == 0 .AND. KW == 14)PRINT*,' MDOT: T,M0,MC,AGE=', &
        TTOT,M0,MC,AGE
    
    !       Check for change of CO WD to ONe on rapid accretion (21/11/08).
        IF(DMR > TINY .AND. KW == 11)THEN
            DME = 2.08D-03*(1.D0/(1.D0 + ZPARS(11)))*RM
            IF(ABS(DMA(K)) > 0.4d0*DME) KW = 12
        ENDIF
    ...
    !       Check mass-loss treatment for inner binary components of merger.
        IF (IHDOT > 0) THEN
            IF((KW == KSTAR(I) .AND. AGE >= 0.99*TN) .OR. &
            (KSTAR(I) <= 6 .AND. KW > 6) .OR. &
    ...
    !       Define KS index & Roche indicator and update core mass (chaos only).
        IF (I < IFIRST .AND. IHDOT == 0) THEN
            KSPAIR = KVEC(I)
            IF ( .NOT. ICORR) THEN
                IF (NAME(N+KSPAIR) > 0) ITRY = 2
            END IF
            IF (KZ(27) > 1) THEN
                IC = 0
            !       Search existing chaotic binaries (assign next index if none found).
                DO 30 KK = 1,NCHAOS
                    IF (NAMEC(KK) == NAME(N+KSPAIR)) IC = KK
                30 END DO
                IF (IC == 0) IC = NCHAOS + 1
    ...
    !       Include special procedures for KS components.
        IF (I < IFIRST .AND. ICORR .AND. IHDOT == 0) THEN
        !       Distinguish between KS pair and merger configuration.
            ...
            !       Terminate for large mass loss or soft binary and re-determine index.
                IF ((DMR > 0.2 .AND. R(KSPAIR) > RMIN) .OR. &
                (DM > 0.0 .AND. H(KSPAIR) + DM/SEMI > -ECLOSE .AND. &
                KSTAR(N+KSPAIR) >= 0) .OR. &
            !    &            (KW.NE.KSTAR(I).AND.KW.GE.13)) THEN
                (KW /= KSTAR(I) .AND. &
                (KW >= 13 .OR. (KW >= 11 .AND. KZ(25) >= 1) .OR. &
                (KW == 10 .AND. KZ(25) > 1)))) THEN
            ...
            !       Adopt KS treatment for single outer component or terminate merger.
                IF (I == 2*KSPAIR .AND. NAME(I) <= NZERO .AND. &
                NAME(N+KSPAIR) > -2*NZERO .AND. DM > 0.0D0 .AND. &
            !    &             H(KSPAIR) + DM/SEMI.LT.-ECLOSE.AND.KW.LT.13) THEN
                H(KSPAIR) + DM/SEMI < -ECLOSE .AND. &
                (KW < 10 .OR. (KW < 11 .AND. KZ(25) < 2) .OR. &
                (KW < 13 .AND. KZ(25) < 1))) THEN
                    CALL HCORR(I,DM,RNEW)
                ELSEIF(DM > 0.D0)THEN
                    IPHASE = 7
                    IPOLY = -1
    ...
    !       Check for end of blue straggler evolution (allow 5% extra).
        IF (M1 > 1.05*TURN .AND. KSTAR(I) == 1 .AND. KW == 2) THEN
            if(rank == 0) &
    ...
    !       Perform neighbour force corrections on significant mass loss (no WD).
        ...
        !       Update the mass loss counters for types > 2.
            ...
            ELSEIF(KW == 4)THEN
                ZMHE = ZMHE + DMSUN
            ELSEIF(KW == 5 .OR. KW == 6)THEN
                ZMRS = ZMRS + DMSUN
            ELSEIF(KW >= 7 .AND. KW <= 9)THEN
                ZMNH = ZMNH + DMSUN
            ELSEIF(KW >= 10 .AND. KW <= 12)THEN
                ZMWD = ZMWD + DMSUN
            ELSEIF(KW == 13 .OR. KW == 15)THEN
                ZMSN = ZMSN + DMSUN
            ELSEIF(KW == 14)THEN
                ZMBH = ZMBH + DMSUN
            ENDIF
        ...
        !            IF (rank.eq.0.and.KZ(19).GT.4) THEN
        !               WRITE(6,36) TTOT, I, NAME(I), KSTAR(I), KW, DMSUN,
        !     &              BODY(I)*ZMBAR, ZMDOT
        !  36           FORMAT(' MDOT:      Time[NB]',1P,E15.6,0P,' I',I8,
        !     &              ' NAME',I8,' K*0',I3,' K*N',I3,' DM[M*]',F7.2,
        !     &              ' M1[M*]',F7.1,' DMTOT[M*]',F12.2)
        !            END IF
        ...
        !**** Note:------------------------------------------------------------**
        !$$$      if(time.ge.3.5373535156250000) then
        !$$$      write(112+rank,*),j,name(j),'x',(x(kk,j),kk=1,3),
        !$$$     *        'xdot',(xdot(kk,j),kk=1,3),'x0',(x0(kk,j),kk=1,3),
        !$$$     *        'x0dot',(x0dot(kk,j),kk=1,3),'t0',t0(j),'step',step(j),
        ...
        !$$$     *        'body',body(j),'dt',time-t0(j),'time',time
        !$$$      end if
        ...
        !       Define logical variable to control optional WD kicks (avoids NaN).
            IKICK = .FALSE.
            IF (KZ(25) == 1 .AND. (KW == 10 .OR. KW == 11)) IKICK = .TRUE. 
            IF (KZ(25) == 2 .AND. KW == 12) IKICK = .TRUE. 
        !       Ensure all NS/BH are assigned a kick (might depend on DM).
            IF (KW == 13 .OR. KW == 14) IKICK = .TRUE. 
        
        !$$$       IF(rank.eq.0.and.KW.GE.13.AND.KW.LE.15) then
        !$$$          write(6,*), 'MDOT K*',KW,' K*0',KSTAR(I),' NAME',NAME(I),
        !$$$     &         ' IKICK',IKICK,' M0',M0,' MC',MC,' AGE',AGE,' TN',TN,
        !$$$     &         ' DM',DM,' DMSUN',DMSUN,' TIME',TIME
        !$$$       end if
        ...
        !$$$      if(time.ge.3.5373535156250000) then
        !$$$      il=3173
        !$$$      print*,rank,'m3',il,name(il),d3(1,il),d2r(1,il),d3r(1,il),time
        !$$$      print*,rank,'ff',i,dmsun,kw,ikick,iphase
        !$$$      call flush(6)
        !$$$      end if
        !     --09/27/13 10:39-lwang-end----------------------------------------*
                   
        !       Perform total force & energy corrections (delay dF if DMSUN > 0.1).
            IF (DMSUN <= 0.05 .AND. (KW < 10 .OR. .NOT. IKICK)) THEN
            !$$$          IF(NPNUM.GT.0) THEN
            !$$$             IF(NDMLIST(NPNUM).EQ.I) THEN
            !$$$                DMLIST(NPNUM) = DMLIST(NPNUM) + DM
            !$$$             ELSE
            !$$$                NPNUM = NPNUM + 1
            !$$$                NDMLIST(NPNUM) = I
            !$$$                DMLIST(NPNUM) = DM
            ...
            !**** Note:------------------------------------------------------------**
            !$$$  if(rank.eq.0) THEN
            !$$$  write(130,*) 1,MDOT_C,NAME(I),DMSUN,KW,TIME,
            !$$$  &                (tttfib-tttfia)*60
            !$$$  END if
            ...
            !$$$          END IF
                ...
                CALL FCORR(I,DM,KW)
                call cputim(tttfib)
            !$$$  if(NCNUM.GT.0.AND.DCLIST(NCNUM).EQ.I) THEN
            !$$$  DCLIST(NCNUM) = DCLIST(NCNUM) + DM
            !$$$  else
            !$$$  NCNUM = NCNUM + 1
            !$$$  NDCLIST(NCNUM) = I
            !$$$  DCLIST(NCNUM) = DM
            ...
            !**** Note:------------------------------------------------------------**
            !$$$  if(rank.eq.0) THEN
            !$$$  write(130,*) 0,MDOT_C,NAME(I),DMSUN,KW,TIME,
            !$$$  &                (tttfib-tttfia)*60
            !$$$  end if
            !     --11/10/13 15:36-lwang-end----------------------------------------*
            END IF
        ...
        !**** Note:------------------------------------------------------------**
        !$$$       IF (KW.GE.10.AND.KW.LE.15.OR.IKICK) THEN
        !$$$          X0DOT(1:3,I) = XDOT(1:3,I)
        !$$$          X0(1:3,I) = X(1:3,I)
        !$$$          call fpoly1(I,I,0)
        !$$$          call fpoly2(I,I,0)
        !$$$          call steps(I,I)
        !     Reduce the steps to make safe
        !$$$          IF(T0(I)+0.5D0*STEP(I).GE.TBLOCK) THEN
        !$$$             STEP(I) = 0.5D0*STEP(I)
        !$$$             IF(T0(I)+0.5D0*STEP(I).GE.TBLOCK) STEP(I) = 0.5D0*STEP(I)
        !$$$          END IF
        !$$$          DO JK = 2, LIST(1,I)+1
        !$$$             JJ = LIST(JK,I)
        !$$$             IF(T0(JJ)+0.5D0*STEP(JJ).GE.TBLOCK) THEN
        !$$$                STEP(JJ) = 0.5D0*STEP(JJ)
        !$$$                IF(T0(JJ)+0.5D0*STEP(JJ).GE.TBLOCK) THEN
        !$$$                   STEP(JJ) = 0.5D0*STEP(JJ)
        !$$$                END IF
        !$$$             END IF
        ...
        !**** Note:------------------------------------------------------------**
        !$$$          print*,'I KICK',I,'N',NAME(I),'T0',T0(I),'STEP',STEP(I)
        !$$$          call flush(6)
        !$$$          print*,rank,'KICK I',I,'N',NAME(I),'STEP',STEP(I),'T0',T0(I),
        !$$$     &         'STEPR',STEPR(I),'T0R',T0R(I),'T',TIME
        !$$$          call flush(6)
        ...
        !$$$*
        !$$$*     --09/06/13 16:21-lwang-template-----------------------------------*
        !$$$***** Note: Do full prediction before fpoly1----------------------------*
        !$$$*               call xbpredall
        !$$$*     --09/06/13 16:21-lwang-end----------------------------------------*
        ...
        !$$$c$$$         do jk=2,list(1,j)+1
        !$$$c$$$            jj=list(jk,j)
        !$$$c$$$      write(117+rank,*),jj,name(jj),'x',(x(kk,jj),kk=1,3),
        !$$$c$$$     *        'xdot',(xdot(kk,jj),kk=1,3),'x0',(x0(kk,jj),kk=1,3),
        !$$$c$$$     *        'x0dot',(x0dot(kk,jj),kk=1,3),'t0',t0(jj),'step',step(jj),
        !$$$c$$$     *        'f',(f(kk,jj),kk=1,3),'fdot',(fdot(kk,jj),kk=1,3),
        !$$$c$$$     *        'd0',(d0(kk,jj),kk=1,3),'d1',(d1(kk,jj),kk=1,3),
        !$$$c$$$     *        'd2',(d2(kk,jj),kk=1,3),'d3',(d3(kk,jj),kk=1,3),
        !$$$c$$$     *        'd0r',(d0r(kk,jj),kk=1,3),'d1r',(d1r(kk,jj),kk=1,3),
        !$$$c$$$     *        'd2r',(d2r(kk,jj),kk=1,3),'d3r',(d3r(kk,jj),kk=1,3),
        !$$$c$$$     *        'body',body(jj),'dt',time-t0(jj),'time',time
        !$$$c$$$      end do
        !$$$c$$$      end if
        !$$$*     --09/27/13 10:39-lwang-end----------------------------------------*
        ...
        !$$$   45             CONTINUE
        !$$$                  CALL FPOLY1_KS(J,J,0)
        !$$$                  CALL FPOLY2_KS(J,J,0)
        !$$$                  IPOLY = -1
        !$$$   50          CONTINUE
    ...
    !$$$      if(time.ge.3.5373535156250000) then
    !$$$      il=3173
    !$$$      print*,rank,'m4',il,name(il),d3(1,il),d2r(1,il),d3r(1,il),time
    !$$$      call flush(6)
    ...
    !       Update event counters for types > 2.
        IF(KW /= KW0)THEN
            ...
            ELSEIF(KW == 5)THEN
                NRS = NRS + 1
            ELSEIF(KW >= 7 .AND. KW <= 9 .AND. KW0 <= 6)THEN
                NNH = NNH + 1
            ELSEIF(KW >= 10 .AND. KW <= 12 .AND. KW0 <= 9)THEN
                NWD = NWD + 1
            ELSEIF((KW == 13 .OR. KW == 15) .AND. KW0 <= 12)THEN
                NSN = NSN + 1
            ELSEIF(KW == 14 .AND. KW0 <= 13)THEN
                NBH = NBH + 1
            ENDIF
        ENDIF
    ...
    !       Ensure that the star does not spin up beyond break-up.
        ...
        IF(JSPIN(K) > JSPBRU) JSPIN(K) = JSPBRU
    ...
    !        IF(I.EQ.IGHOST.OR.BODY(I).LE.0.0) RM0 = M1
        ...
        IF(IHDOT == 2)THEN
            TEV(I0) = TEV(I)
            IF(NAME(IGHOST) < 0 .OR. NAME(IGHOST) > NZERO)THEN
                TEV(IGHOST) = TEV(I)
            ENDIF
        ENDIF
    ...
    !       See if former KS pair can be regularized again.
        IF (KS > 0) THEN
            ...
            !       Restore current time to prevent small subsequent steps.
                TIME = TBLOCK
                IF ((KW == 13 .OR. KW == 14) .AND. H(NPAIRS) < 0.0) THEN
                ...
                END IF
                IF(KW >= 10 .AND. H(NPAIRS) < 0.0)THEN
                    IF(KZ(9) >= 3)THEN
                        CALL DEGEN(NPAIRS,NPAIRS,3)
                    ELSE
                        J1 = 2*NPAIRS - 1
                        J2 = J1 + 1
                        IF(KSTAR(J1) >= 10 .AND. KSTAR(J2) >= 10)THEN
                            NDD = NDD + 1
                        ENDIF
                    ENDIF
                ENDIF
    ...
    !       Note any formation of black holes or TZ object.
        IF(KW == 14 .AND. KSTAR(I) < 14 .AND. I <= N)THEN
            if(rank == 0) &
    ...
    !       Perform consistency check on massive WD (skip HMDOT treatment).
        IF(I <= N)THEN
            IF((KSTAR(I) >= 10 .AND. KSTAR(I) <= 12) .AND. IHDOT == 0)THEN
                IF(rank == 0 .AND. BODY(I)*ZMBAR > MCH)THEN
    ...
    !       Include stellar radius check in case of NaN.
        IF (RADIUS(I) >= 0.0 .AND. RADIUS(I) < 1.0) GO TO 105
        if(rank == 0) &
    ...
    !       Check for chaotic binary.
        IF(ITRY > 0 .AND. IGHOST == 0)THEN
            II = I
            I = N + KSPAIR
            IPAIR = KSPAIR
            IF(KSTAR(I) < 0)THEN
                IF(KW >= 10 .AND. DMR > 0.0)THEN
                    if(rank == 0) &
                ...
                IF(DMR < 0.0) GOTO 1
            ENDIF
            IF(KW >= 10 .AND. ICORR)THEN
                IF(KZ(9) >= 3)THEN
                    CALL DEGEN(IPAIR,IPAIR,3)
                ELSE
                    J1 = 2*IPAIR - 1
                    J2 = J1 + 1
                    IF(KSTAR(J1) >= 10 .AND. KSTAR(J2) >= 10) NDD = NDD + 1
                ENDIF
            ENDIF
            ITRY = 0
        ENDIF
...
!       orbit change owing to gravitational radiation, mass loss or tides.
    ...
    !        IF(H(IPAIR).LT.0.D0.AND.NAME(ICM).GT.0)THEN
        ...
        !       Include optional look-up time control for compact object binaries.
            ...
            IF(KSTAR(I) > 0 .AND. KZ(34) > 0)THEN
            !       Ensure optional updating of orbit before possible Roche test (1/2013).
                IF (KZ(34) == 1 .AND. KSTAR(I) > 0 .AND. &
                MOD(KSTAR(I),2) == 0)THEN
                    CALL SYNCH(IPAIR)
                    IF(IQCOLL /= 0 .OR. IPHASE < 0) GO TO 1
                ENDIF
                JPAIR = -IPAIR
                ...
                ELSE
                    TEV(I) = TIME + DTR
                    IF(KSTAR(I) > 10 .AND. MOD(KSTAR(I),2) == 1)THEN
                        J1 = 2*IPAIR - 1
                        J2 = J1 + 1
...
!       Update any merged circularizing binary using TMDIS.
    103 IMERGE = 1
    104 IF (NMERGE > 0) THEN
        IF (KSTARM(IMERGE) < 0 .AND. TMDIS(IMERGE) < TIME) THEN
            KSPAIR = 0
        !       Determine the KS index from c.m. identification.
            DO 106 I = N+1,NTOT
                IF (NAME(I) == NAMEM(IMERGE)) THEN
                    KSPAIR = I - N
                END IF
            106 END DO
            IF (KSPAIR > 0) THEN
