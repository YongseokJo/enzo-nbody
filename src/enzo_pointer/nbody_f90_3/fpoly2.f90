...
!**** Note:------------------------------------------------------------**
!$$$      if(time.ge.0.7307) then
!$$$         do I = ifirst,Ntot
!$$$            write(105+rank,*) 'i',i,'n',name(i),'x',x(1,i),
!$$$     &           'xd',xdot(1,i),'f',f(1,i),'fdot',fdot(1,i),
!$$$     &           'step',step(i),'t0',t0(i),'t',time
!$$$         end do
!$$$         call flush(105+rank)
!$$$      end if
...
!       Loop over all bodies, pair #ICOMP & JCOMP, c.m. or one single body.
    ...
    !       Sum over c.m. instead of regularized neighbours since F not known.
        ...
        !       Ensure that all neighbours are considered.
            IF (RIJ2 > RCRIT2 .AND. J /= NAMEJ) GO TO 60
        !       Distant bodies do not contribute significantly to F2DOT & F3DOT.
        
            IF (KCASE > 0) THEN
                IF (J > JCOMP .OR. KCASE == 1) GO TO 30
            END IF
            IF (J == I) GO TO 60
        
        !       See whether F & FDOT extrapolation is required.
            IF (KCASE == 0 .AND. TIME > 0.0D0) THEN
                IF (T0(J) < TIME) GO TO 30
