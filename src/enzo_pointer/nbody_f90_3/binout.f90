...
!       Obtain relevant distributions of KS binaries.
    DO 50 J = 1,NPAIRS
        I = N + J
        IF (H(J) >= 0.0 .OR. BODY(I) == 0.0D0) GO TO 50
        J1 = 2*J - 1
        J2 = 2*J
    
    !       Count original & exchanged binaries and core members.
        IF (IABS(NAME(J1) - NAME(J2)) == 1) THEN
            IF (NAME(J1) <= 2*NBIN0) JOR = JOR + 1
        ELSE
            IF (MIN(NAME(J1),NAME(J2)) <= 2*NBIN0) JEX = JEX + 1
        END IF
    ...
    !       Perform consistency check in case of over-writing.
        IF (LIST(2,J2) /= -1 .AND. LIST(2,J2) /= 0) THEN
            if(rank == 0) &
    ...
    !       Reset c.m. Roche flag to standard type for non-zero eccentricity.
        IF (KSTAR(I) >= 10 .AND. ECC2 > 4.0D-06) KSTAR(I) = 0
        IF (KSTAR(I) >= 10) K10 = K10 + 1
    50 END DO
...
!       Print eccentricity & energy distributions.
    ...
    IF (DISP > 0.0D0) DISP = SQRT(DISP/FLOAT(NPAIRS))
    EMAX = SQRT(EMAX)
    if(rank == 0) &
...
!       Form the net energy gain in binary interactions (also in events.f).
    ...
    if(rank == 0) WRITE (6,90)  (E(J),J=1,10), ETOT, DETOT, DEGRAV
