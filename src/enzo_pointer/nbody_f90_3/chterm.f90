...
!       Save dominant pair (note change from MIN(R2) to M/R2; SJA 08/2009).
    ...
    !       Determine indices for second closest pair.
        ...
        !       Avoid choosing close pair I1-I2.
            IF (J1 == I1 .OR. J1 == I2) GO TO 2
            DO 1 J2 = J1+1,NCH
                IF (J2 == I1 .OR. J2 == I2) GO TO 1
                IF (R2(J1,J2) < RX1 .AND. R2(J1,J2) > RX0) THEN
                    RX1 = R2(J1,J2)
                    I3 = J1
                    I4 = J2
                END IF
    ...
    !       Identify remaining single particle(s) by exclusion.
        DO 3 I = 1,NCH
            IF (I == I1 .OR. I == I2 .OR. I == I3 .OR. I == I4) GO TO 3
            IF (I5 == 0) THEN
                I5 = I
            ELSE
                I6 = I
...
!       Identify current global indices by searching all particles.
    ...
    DO 10 J = IFIRST,NTOT
        DO 5 L = 1,NCH
            IF (NAME(J) == JLIST(L+5)) THEN
                JLIST(L) = J
                IF (BODY(J) > 0.0D0) ICM = J
            END IF
        5 END DO
    10 END DO
...
!       Set TIMEC for collision to preserve TIME2 after T0S = TIME in SUBSYS.
    IF (ITERM < 0.0) TIMEC = T0S(ISUB) + TIMEC - TIME
...
!       Print diagnostics for high-velocity chain escaper after COLL or COAL.
    IF (rank == 0 .AND. ECH > 0.5*BODYM*16.0*ECLOSE) THEN
...
!       Search all neighbour lists for splitting chain c.m. into components.
    ...
    DO 50 J = IFIRST,NTOT
        NNB2 = LIST(1,J) + 1
        DO 45 L = 2,NNB2
            IF (LIST(L,J) > ICM) GO TO 50
        !       Form list of all ICM without one ghost and predict X & XDOT to FDOT.
            IF (LIST(L,J) == ICM) THEN
                DO 42 K = 2,NNB2
                    IF (LIST(K,J) == JG) GO TO 50
                42 END DO
            !       Skip subsystem members.
                DO 44 K = 1,NCH
                    IF (J == JLIST(K)) GO TO 50
                44 END DO
...
!       Check for stellar collision (only needs coordinates & velocities).
    ...
    !       See whether re-labelling is required (indices I1 - I4 still local).
        IF (R2(I1,I4) < R2(I1,I3) .OR. R2(I3,I4) < R2(I1,I3)) THEN
            IF (R2(I1,I4) < R2(I3,I4)) THEN
...
!       Exclude the dominant interaction for c.m. approximation (large FDOT).
    IF (MIN(R2(I1,I3),R2(I2,I4)) > CMSEP2*R2(I1,I2)) THEN
        JLIST(1) = JLIST(3)
        JLIST(2) = JLIST(4)
        NNB = 2
        IF (NCH == 3) JLIST(2) = MAX(IFIRST + 4,JLIST(1) + 1)
    ELSE
        NNB = NCH
    END IF
...
!       See whether a second binary is present (NCH >= 4 & RB < RMIN).
    KS2 = 0
    IF (NCH >= 4 .AND. R2(I3,I4) < RMIN2) THEN
    !       Accept second KS pair if well separated (> 2) from smallest binary.
        IF (R2(I3,I4) < 0.25*MIN(R2(I1,I3),R2(I2,I4))) THEN
            IF (MAX(JLIST(3),JLIST(4)) <= N) THEN
                KS2 = 1
            END IF
        END IF
    END IF
...
!       Perform KS regularization of dominant components (ICOMP < JCOMP).
    ...
    !       Include optional rectification by standard KS procedure.
        ...
        !       Rectify KS elements of dominant and weakly perturbed binary.
            IF (EB1 > 0.9 .AND. GI < GMAX) THEN
                IF (rank == 0 .AND. ABS(ERR) > 1.0D-08) THEN
...
!       Check for high-velocity ejection with outwards hyperbolic motion.
    ...
    IF (NCH == 3) THEN
        ...
        IF (HI > 0.0 .AND. RDOT > 0.0 .AND. KZ(37) > 0) THEN
            CALL HIVEL(I3)
        END IF
    END IF
...
!       Initialize second KS pair if separation is small.
    ...
    !       Re-determine the global indices after updating in KSREG.
        DO 80 I = IFIRST,N
            IF (NAME(I) == NAME3) I3 = I
            IF (NAME(I) == NAME4) THEN
                I4 = I
                IF (I3 > 0) GO TO 85
            END IF
        80 END DO
...
!       Replace any old perturbers that have been exchanged in KSREG.
    IF (LISTC(1) > 0 .AND. LISTC(2) < IFIRST) THEN
        DO 92 J = IFIRST,N
            IF (NAME(J) == NAMC2) THEN
                LISTC(2) = J
            ELSE IF (LISTC(1) > 1 .AND. NAME(J) == NAMC3) THEN
                LISTC(3) = J
            END IF
        92 END DO
    END IF
...
!       Check for subsystem at last COMMON dump (no restart with NSUB > 0).
    IF (NSUB == 0 .AND. KZ(2) > 1) THEN
        IF (TIME - TDUMP < TIMEC) THEN
            TDUMP = TIME
            CALL MYDUMP(1,2)
        END IF
    END IF
...
!  100 IF (ITERM.GE.0) TPREV = TIME - 16.0*DT8
    100 IF (ITERM >= 0) TPREV = TIME
!     --03/07/14 23:16-lwang-end----------------------------------------*
!     --03/03/14 19:59-lwang-debug--------------------------------------*
!**** Note:------------------------------------------------------------**
!$$$      call adjust
!$$$      print*,rank,'nstepi',nstepi,'t',time
!$$$      call flush(6)
!$$$      stop
!     --03/03/14 19:59-lwang-end----------------------------------------*
    RETURN

    END SUBROUTINE CHTERM
