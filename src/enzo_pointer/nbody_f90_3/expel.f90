...
!       Distinguish between coalescence and surviving binary.
    ...
    !       Check for TZ object formed by CE evolution.
        IF(KSTAR(I2) >= 13 .AND. KW1 >= 13)THEN
            NTZ = NTZ + 1
            if(rank == 0)WRITE (6,5)  KW1, M1, M2
    ...
    !       Check common envelope condition again after circularization (09/08).
        IF (ECC0 > 0.001D0 .AND. ECC <= 0.001D0) THEN
            Q1 = M1/M2
            Q2 = 1.D0/Q1
            RL1 = RL(Q1)
            RL2 = RL(Q2)
            IF (R1 > RL1*SEP .OR. R2 > RL2*SEP) THEN
                ITER = ITER + 1
                IF (ITER == 1) THEN
                    if(rank == 0)WRITE (6,8)  RL1*SEP, RL2*SEP
    ...
    !       Ensure rare case of massless remnant will escape at next output.
        IF (KW1 == 15 .OR. KW2 == 15) THEN
            CALL KSTERM
            IPHASE = -1
            I = IFIRST
            IF (BODY(I) > 0.0D0) I = IFIRST + 1
            T0(I) = TADJ + DTADJ
        !        remove from NXTLST
        ...
        !       Provide diagnostic output if binary survives the kick.
            ...
            IF (H(IPAIR) < 0.0) THEN
                ...
                if(rank == 0)WRITE (6,35)  KW1, R(IPAIR)/SEMI, SEMI*SU, &
            ...
            !       Include safety test on no membership just in case (not sure!).
                IF (IC == 0) THEN
                    ...
                    IF (NCHAOS > NTMAX) THEN
                        if(rank == 0)WRITE (6,38) NAME(I1),NCHAOS,ECC
        ...
        !       Include prediction to end of current block-time before new KS.
            TIME = TBLOCK
            IF (T0(I) < TIME .AND. DM > 0.0D0) THEN
                CALL XVPRED(I,-2)
                T0(I) = TIME
            !     remove from NXTLST
                call delay_remove_tlist(I,STEP,DTK)
        ...
        !       Avoid negative radial velocity at pericentre (bug fix 12/08).
            IF (TDOT2(IPAIR) < 0.0D0) TDOT2(IPAIR) = 0.0
...
!       Include cleaning of CHAOS arrays (cf. KSTERM for current COAL).
    ...
    KK = 0
    DO 55 I = N+1,NTOT
        IF (NAME(I) == NAMEC(L)) KK = 1
    55 END DO
    NCH1 = NCHAOS
!       Perform removal of NAMEC on failed search.
    IF (KK == 0 .AND. KSTAR(N+IPAIR) < 0) THEN
        II = -L
        CALL SPIRAL(II)
    END IF
    IC = IC + 1
!       Consider the same location again after array update.
    IF (NCHAOS < NCH1) L = L - 1
    IF (IC < NCHAOS+1) GO TO 52
