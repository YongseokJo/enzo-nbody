...
!       Check hierarchical stability of triple using simple criterion.
    ...
    !       Check binary escape.
        ...
        !       Employ parabolic escape criterion (reduce if RI > RSUM/2 & RDOT > 0).
            IF (RI > RESC .AND. RDOT > 0.0) THEN
                IF (RDOT**2 < 2.0*MASS/RI) THEN
                    IESC = 0
                END IF
            ELSE
                IESC = 0
        ...
        ELSE IF (IESC > 0 .AND. JESC == 0) THEN
        ...
        !       Check approximate escape criterion outside RESC (allow RX > 2*RESC).
            IF (RM > RESC .AND. RDOT > 0.0) THEN
                IF (RDOT**2 < 2.0*MASS/RI) THEN
                    ...
                    RX = -MASS/ER
                    IF (ER < 0.0 .AND. RX < 2.0*RESC) THEN
                        IESC = 0
                    END IF
                END IF
            ELSE
