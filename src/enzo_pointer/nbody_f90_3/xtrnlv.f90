...
!       Skip external potential during scaling (parameters not defined).
    IF (FIRST .AND. TIME == 0.0D0) THEN
        FIRST = .FALSE.
        GO TO 30
    !       Treat all cases uniformly (but note Coliolis term in FIRR).
        #ifdef TT
    !** FlorentR - case of tidal tensor
    ELSE IF ((KZ(14) <= 4 .OR. KZ(14) == 9) .AND. I2 /= I1) THEN
    !** FRenaud
        #else
    ELSE IF (KZ(14) <= 4 .AND. I2 /= I1) THEN
        #endif
        VIR = 0.0
        DO 5 I = I1,I2
            IF(TIME > 0) CALL JPRED(I,TIME,TIME)
            DO 2 K = 1,3
    ...
    !       Quit for pure 3D tidal field (case of full N summation).
        #ifdef TT
    !** FlorentR - case of tidal tensor
        IF (KZ(14) == 3 .OR. KZ(14) == 9) GO TO 40
