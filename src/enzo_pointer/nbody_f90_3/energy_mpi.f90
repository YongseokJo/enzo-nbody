...
!       Calculate the potential energy.
    ...
    !$$$      DO IJ = istart, iend, 1024
    !$$$         if (iend-IJ+1.lt.1024) then
    !$$$            NN = iend-ij+1
    !$$$         else
    !$$$            NN = 1024
    !$$$         end if
    !$$$      CALL GPUPOT(rank,ij,NN,NNT,BODY(IFIRST),X(1,IFIRST)
    !$$$     &        ,phidbl(Ij+ifirst-1))
    !$$$      end do
    !**** Note:Finish once-------------------------------------------------**
    !$$$      CALL GPUPOT(rank,NN,NNT,NPLIST(Istart),BODY(IFIRST),X(1,IFIRST),
    ...
    ! omp parallel do private(I,JMIN,IPAIR,POTI,POTJ,A1,A2,A3,A4)
        ...
        !     POTI contains potential at particles position to be stored later (R.Sp.)
            ...
            DO 30 J = 1,N
                IF (J == I .OR. J == 2*IPAIR-1 .OR. J == 2*IPAIR .OR. &
                BODY(J) == 0.0D0 .OR. BODY(I) == 0.0D0)  GO TO 30
                A1 = X(1,I) - X(1,J)
                A2 = X(2,I) - X(2,J)
                A3 = X(3,I) - X(3,J)
    ...
    !$$$         end do
    !**** Note:Finish once-------------------------------------------------**
    !$$$         CALL GPUPOT(rank,NNT,NNT,NPLIST(ifirst),BODY(IFIRST),
    !$$$     *        X(1,IFIRST),phidbl(ifirst))
        CALL GPUPOT(rank,1,NNT,NNT,BODY(IFIRST),X(1,IFIRST) &
        ,phidbl(ifirst))
    !$$$         print*, 'phi: ',phidbl(ifirst:ifirst+5)
