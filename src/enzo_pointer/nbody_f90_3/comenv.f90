...
!     ECIRC = EORBI/(1.D0 - ECC*ECC)
    ...
    ! Remove all envelope mass if below 0.05 m_sun.
        ...
        !           CC = M1*MC1/(LAMB1*R1) + ALPHA1*M1*M2/(SEP*EFAC)
            ...
            IF(DM <= 0.0 .OR. DM > (M1-MC1))THEN
                if(rank == 0) WRITE(*,*) ' WARNING: CE DM ',DM,M1,MC1
            ENDIF
            M1 = MAX(M1-DM,MC1)
...
! If the secondary star is also giant-like add its envelopes's energy.
    ...
    IF(KW2 >= 2 .AND. KW2 <= 9 .AND. KW2 /= 7)THEN
        IF(M2 > MC2)THEN
            MENVD = MENV/(M2-MC2)
            RZAMS = RZAMSF(M02)
...
! If the secondary is on the main sequence see if it fills its Roche lobe.
    ...
    !**
        ...
        RL1 = RL(Q1)
        RL2 = RL(Q2)
        IF(RC1/RL1 >= RCC2/RL2)THEN
            IF(RC1 > RL1*SEPF)THEN
                COEL = .TRUE.
                SEPL = RC1/RL1
            ENDIF
        ELSE
            IF(RCC2 > RL2*SEPF)THEN
                COEL = .TRUE.
                SEPL = RCC2/RL2
            ENDIF
        ENDIF
        ...
        ! then see if the resulting core will survive or change form.
            ...
            IF(KW1 == 6 .AND. (KW2 == 6 .OR. KW2 >= 11))THEN
                CALL dgcore(KW1,KW2,KW,MC1,MC2,MC3,EBINDF)
            ENDIF
            IF(KW1 <= 3 .AND. M01 <= ZPARS(2))THEN
                IF((KW2 >= 2 .AND. KW2 <= 3 .AND. M02 <= ZPARS(2)) .OR. &
                KW2 == 10)THEN
                    CALL dgcore(KW1,KW2,KW,MC1,MC2,MC3,EBINDF)
                    IF(KW >= 10)THEN
                        KW1 = KW
...
! Newton-Raphson iteration.
    ...
    !        IF(ABS(DELY/MF**(1.D0+XX)).LE.1.0D-02) GOTO 20
        IF(ABS(DELY/MF) <= 1.0D-03) GOTO 20
    ...
    ! Set the masses and separation.
        ...
        20 IF(MC22 == 0.D0) MF = MAX(MF,MC1+M2)
        M2 = 0.D0
        M1 = MF
        KW2 = 15
    ...
    ! Combine the core masses.
        ...
        ELSEIF(KW == 4 .OR. MC2 > 0.D0 .OR. KW /= KW1)THEN
            IF(KW == 4) AJ1 = (FAGE1*MC1 + FAGE2*MC22)/(MC1 + MC22)
            MC1 = MC1 + MC2
            MC2 = 0.D0
...
! Include collision criterion at the end (note RC1 = 0 for main sequence).
    IF (RC1 == 0.0D0) THEN
        IF (R1 + RCC2 >= SEP*(1.0 - ECC)) THEN
            COEL = .TRUE.
        END IF
    ELSE
        IF (RC1 + RCC2 >= SEP*(1.0 - ECC)) THEN
            COEL = .TRUE.
        END IF
    END IF

    RETURN
    END SUBROUTINE COMENV
!**
