...
!       Provide diagnostics for exchange (membership may have changed).
    IF (NAME12 /= NAMEC(I1) + NAMEC(I2) .AND. &
    NAME34 /= NAMEC(I1) + NAMEC(I2) .AND. EB < 0.0) THEN
        ISUB = ISYS(5)
        TCH = T0S(ISUB) + TIMEC
        if(rank == 0) &
...
!       Include output of strong interaction from chaos case.
    IF (IEND == 2 .AND. PMIN < 2.0*SEMI .AND. DB > 0.1) THEN
