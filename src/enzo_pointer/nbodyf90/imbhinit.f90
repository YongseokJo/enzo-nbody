...
!     Printing initial massive black hole data
    IF(rank == 0) write(6,1) NIMBH,BIMBH,XIMBH(1:3), &
    VIMBH(1:3)
...
!     Search index of massive black hole
    ...
    if(IBH == 0) then
        if(rank == 0) print*,'BH not found! N',NIMBH
        call flush(6)
        call abort
    end if
...
!     Print massive black hole data
    IF(rank == 0) write(6,1) IBH,NIMBH,BIMBH,X(1:3,IBH)*RBAR, &
    XDOT(1:3,IBH)*VSTAR
