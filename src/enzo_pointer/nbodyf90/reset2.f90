...
!       Produce info on quintuplet or sextuplet (NAMEG has outer c.m. name).
    ...
    !       Determine previous merger index with NAME greater by 2*NZERO.
        JM = 1
        DO 3 K = 1,NMERGE
            IF (NAMEM(K) == NAME(I) + 2*NZERO) JM = K
        3 END DO
    !       Use previous merger index to identify ghost from earlier level.
        DO 4 J = IFIRST,NTOT
            IF (NAME(J) == NAMEG(JM)) JG = J
        4 END DO
        IF (JG > N) WHICH1 = ' SEXTUPLET '
        if(rank == 0) &
...
!       Check presence of [[B,S],S] quartet or [[B,B],S] quintuplet.
    IF (NAMEG(IMERGE) > 0 .AND. NAMEG(IMERGE) <= NZERO) THEN
        I1 = 2*IPAIR - 1
        I2 = 2*IPAIR
        CALL FINDJ(I1,JG,IM)
        ZM = BODY(I)*SMU
        IF (NAME(I2) <= NZERO) THEN
            if(rank == 0) &
...
!       Include diagnostics for double triple ([[B,S],[B,S]]).
    ...
    !       Determine original merger indices of the two inner binaries.
        DO 16 K = 1,NMERGE
            IF (NAMEM(K) == NAME(I) + 2*NZERO) IM = K
            IF (NAMEM(K) == NAMEG(JG)) JM = K
        16 END DO
...
!       Check optional diagnostics for hierarchy.
    IF ((KZ(18) == 1 .OR. KZ(18) == 3) .AND. KSTAR(IMERGE) <= 20) THEN
        CALL HIARCH(IPAIR)
    END IF
...
!       Find correct location of ghost particle using identification name.
    ...
    JCOMP1 = JCOMP
    DO 10 I = 1,NTOT
        IF (BODY(I) == 0.0D0 .AND. NAME(I) == NAMEG(IMERGE)) JCOMP1 = I
    10 END DO
...
!      call xbpredall
    DO 46 L = 2,NNB1
        ...
        42 END DO
        IF (RIJ2 < RS2 .AND. J /= JCOMP1 .AND. J <= N) THEN
            T0(J) = TIME
            DO 45 K = 1,3
                X0DOT(K,J) = XDOT(K,J)
                X0(K,J) = X(K,J)
...
!       Check Roche look-up time.
    IF (KSTAR(ICM) > 0 .AND. KSTAR(ICM) <= 10) THEN
        K = ICM - N
        CALL TRFLOW(K,DTR)
