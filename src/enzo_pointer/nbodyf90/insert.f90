...
!       Determine correct index by comparing neighbouring KBLIST members.
    ...
    !$$$          IF(LSTAR.EQ.0) THEN
    !$$$             do k=1,NNTB
    !$$$             print*,'DEBUG',k,KBLIST(k),T0(KBLIST(K))+STEP(KBLIST(K))
    !$$$             end do
    !$$$             print*,'DEBUG, I',I,'TI',TI,'LI',LI,T0(LI)+STEP(LI)
    !$$$             call flush(6)
    !$$$          end if
    !     --12/30/13 20:23-lwang-end----------------------------------------*
        ...
        IF (J == I) THEN
            ...
            GO TO 50
        END IF
        IF (TI < T0(J) + STEP(J) .AND. LSTAR > 1) GO TO 20
        LSTAR = LSTAR + 1
