...
!          --------------------------
!      For \Gamma=(H-E)/L and Y(ntime)=time
    ...
    do  j=1,10                ! Jmax (10 -> efficient,  4 -> short steps)
        ...
        do i=1,n
            ...
            if( .NOT. kl)then
                ...
                is=i+n/2
                if(is > nhalf2)is=i-(n/2)
                dyis=dabs(dy(is))
                if(i == n)dyis=1/(abs(ya(i))+abs(ta)) ! for time
                if(konv)then
                    test=dabs( (y(i)-ta)*dyis )
                    if(test > eps) konv= .FALSE. 
                end if
                if( .NOT. gr)then
                    fv=dabs(w)*dyis
                    if(fs < fv) fs=fv
        ...
        if(fs /= 0.d0)then
            ...
            if(fy bad)then
                h=h*fy
                jti=jti+1
                if(jti > 10)then !Seppo's suggestion Oct/06 (5 before).
                    h=0.0
                    do i=1,n
                        y(i)=ya(i)
                    end do
        ...
        !     if(fy.gt.4.0)fy=4. ! factor 10 may be too large
            if(fy > 2.0)fy=2.  ! factor 10 may be too large
            h=h*fy
            return
        end if
        d(3)=4.d0
