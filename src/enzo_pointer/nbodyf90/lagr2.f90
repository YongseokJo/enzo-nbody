...
!       Apply the mass test in same proportion for two Plummer spheres.
    ...
    DO 10 I = IFIRST,NTOT
        IF (KZ(5) == 1) THEN
            IF (NAME(I) > NAM1 .AND. NAME(I) <= NZERO) GO TO 10
        ELSE IF (KZ(5) == 2) THEN
            IF ((NAME(I) > NAM1 .AND. NAME(I) <= N1) .OR. &
            (NAME(I) > NAM2 .AND. NAME(I) <= NZERO)) GO TO 10
        END IF
        ZM1 = ZM1 + BODY(I)
        NP = NP + 1
...
!       Improve choice of NAM1 using relative deviation (max 10 tries).
    ...
    IF (ABS(DM) > 0.001 .AND. ITER <= 10) THEN
        ITER = ITER + 1
        IF (ABS(DM) > 0.05) DM = DM*0.05/ABS(DM)
...
!       Ensure at least 1% in the most massive bin.
    ...
    IF (NP0 < NPCRIT .AND. ITRY < 10) THEN
        ZMX0 = 0.9*ZMX0
        GO TO 32
    END IF
...
!       Treat the low-mass particles in the same way (exclude cm bodies).
    ...
    DO 50 I = IFIRST,N
        IF (KZ(5) == 1) THEN
            IF (NAME(I) <= NAM1) GO TO 50
        ELSE IF (KZ(5) == 2) THEN
            IF (NAME(I) <= NAM1 .OR. &
            (NAME(I) > N1 .AND. NAME(I) <= NAM2)) GO TO 50
        END IF
        ZM2 = ZM2 + BODY(I)
        NP = NP + 1
        R2(NP) = (X(1,I) - C(1))**2 + (X(2,I) - C(2))**2 + &
