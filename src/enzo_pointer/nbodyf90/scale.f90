...
!       (if start model data were read from other source do NOT scale)(R.Sp.)
    ...
    !     Assume Mass use the unit of M_sun
        IF(KZ(22) == 10) ZMBAR = ZMASS/FLOAT(N)
...
!**** Note:------------------------------------------------------------**
    ...
    !       Sum the total energy of pairs.
        ...
        IF (NBIN0 > 0) THEN
            ...
            END DO
        ! 11  FORMAT(1X,I5,1P,12D15.5)
        ! 12  FORMAT(1X,I5,1P,12D15.5)
        END IF
    END IF
...
!       Allow two optional ways of skipping standard velocity scaling.
    IF (KZ(22) <= 1 .AND. (KZ(5) == 2 .OR. KZ(5) == 3)) THEN
        QV = SQRT(Q*VIR/ZKIN)
    ...
    !     Rescale velocities to new masses for two Plummer spheres.
        IF (KZ(5) == 2) THEN
            ...
            ETOT = E0
            Q = ZKIN/POT
            if(rank == 0) WRITE (6,59)  E0, ZKIN/POT
            59 FORMAT (/,12X,'UNSCALED ENERGY    E =',F10.6, &
            '  Q =',F6.2)
        ELSE
            IF (KZ(5) == 3) E0 = ZKIN - POT
            if(rank == 0) WRITE (6,54)  E0, ZKIN, POT
...
!      IF (KZ(14).EQ.0.AND.KZ(23).LE.2) RTIDE = 1.D8
...
!       Print half-mass relaxation time & equilibrium crossing time.
    ...
    if(rank == 0)WRITE (6,95)  TRH, TCR, 2.0*RSCALE/VC
