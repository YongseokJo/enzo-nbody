...
!     Include case of chain chaos without identified NAMEC.
    ...
    !     Search for component names (set in CHAOS2).
        DO 5 K = 1,NCHAOS
            IF (NAMEC(K) == NAME(I1) .OR. NAMEC(K) == NAME(I2)) IC = K
        5 END DO
    
    !     Include safety check just in case.
        IF (rank == 0 .AND. IC == 0) THEN
...
!     Define oscillation period (dimensionless time) and damping constants.
    ...
    !     Specify polytropic index for each star (n = 3, 2 or 3/2).
        IF (KSTAR(IK) == 3 .OR. KSTAR(IK) == 5 .OR. &
        KSTAR(IK) == 6 .OR. KSTAR(IK) == 9) THEN
        ...
        ELSE
            ...
            IP = 3
            IF (KSTAR(IK) >= 3) IP = 2
            IF (KSTAR(IK) == 4 .OR. KSTAR(IK) == 7) IP = 3
            IF (KSTAR(IK) == 8) IP = 3
            IF (KSTAR(IK) == 0) IP = 1
            W(K) = WW(IP)
            Q(K) = QQ(IP)
            rg2(k)= 0.21
            IF (KSTAR(IK) <= 2 .OR. KSTAR(IK) == 7) rg2(k) = 0.1
            IF (KSTAR(IK) == 4) rg2(k)= 0.1*(1.0 - CM(K,IC)/BODY(IK))
        END IF
...
!     Convert from angular momentum to omega (denoted spin1 & spin2).
    IF (KSTAR(J1) <= 2 .OR. (KSTAR(J1) >= 7 .AND. KSTAR(J1) /= 9)) THEN
        SPIN1 = SPIN(J1)/(rg2(1)*BODY(J1)*RADIUS(J1)**2)
    ELSE
        KW = KSTAR(J1)
        M0 = BODY0(J1)*SMU
        MC1 = CM(1,IC)*SMU
        IF (MC1 <= 0.0D0 .OR. MC1 > M0) THEN
            MC1 = 0.3 + 0.1*FLOAT(KW - 3)
            IF(KW == 9) MC1 = MIN(0.3D0,0.95*M0)
            CM(1,IC) = MC1/ZMBAR
        END IF
        ZDUM = 2.0D0
        ...
        0.21*MC1/SMU*RC1**2)
    END IF
    IF (KSTAR(J2) <= 2 .OR. (KSTAR(J2) >= 7 .AND. KSTAR(J2) /= 9)) THEN
    ...
    ELSE
        ...
        M0 = BODY0(J2)*SMU
        MC2 = CM(2,IC)*SMU
        IF (MC2 <= 1.0D-10 .OR. MC2 > M0) THEN
            MC2 = 0.3 + 0.1*FLOAT(KW - 3)
            IF(KW == 9) MC2 = MIN(0.3D0,0.95*M0)
            CM(2,IC) = MC2/ZMBAR
        END IF
        ZDUM = 2.0D0
    ...
    IF (IONE == 0) THEN
        IONE = IONE + 1
        IF (ABS(SPIN1) > 0.D0 .AND. ABS(SPIN2) > 0.D0) THEN
...
!     Include extra steps for long intervals and/or AGB type 5 or 6.
    IF (TIME - TIME0 > 0.1) nstep = nstep + 20
    IF (KSTAR(J1) == 5 .OR. KSTAR(J1) == 6) nstep = nstep + 10
...
!     Check circularization time after merger or large interval.
    ...
    !     Ensure careful integration with DT=0.1*TC for rapid evolution.
        IF (TC < 10.0 .AND. (KSTAR(J1) > 1 .OR. ECC > 0.1)) THEN
    ...
    IONE = IONE + 1
    IF (rank == 0 .AND. IONE <= 2 .OR. MOD(IONE,10000) == 0) THEN
...
!     Ensure that no overshooting takes place.
    IF (spin10 < 1.0 .AND. spin1 > 1.0) THEN
    ...
    !         END IF
        ...
        IWARN = IWARN + 1
    END IF
    IF (spin20 < 1.0 .AND. spin2 > 1.0) THEN
    !         IF (rank.eq.0.and.MOD(IWARN,100).EQ.0) THEN
...
!     Ensure apocentre or arbitrary phase is replaced by pericentre.
    ...
    IF (R(IPAIR) > SEMI .OR. ERR > 1.0E-05) THEN
...
!     Form critical mass ratio using approximate core mass.
    IF(KSTAR(J1) == 2)THEN
        QC = 4.D0
    ELSE IF(KSTAR(J1) >= 3 .AND. KSTAR(J1) <= 6)THEN
        ZPARS7 = 0.3
        ZCMC = CM(1,IC)
        IF (J1 == I2) ZCMC = CM(2,IC)
    ...
    ELSE IF(KSTAR(J1) == 8 .OR. KSTAR(J1) == 9)THEN
        QC = 0.784D0
    ELSE
        QC = 1000.0
    ENDIF
...
!     Enforce termination at constant angular momentum on Roche condition.
    ...
    !     ks MPI communication ECOLL EGRAV KSTAR
        ...
        call ksparmpi(K_store,K_real8,K_EGRAV,0,0,DETMP)
        call ksparmpi(K_store,K_int,K_KSTAR,I,0,KSTAR(I))
        if(rank == 0)WRITE (6,21)  ES0, ECC, AF/SEMI, DTR, SEMI, AF
    ...
    !     ks MPI communication SPIN
        call ksparmpi(K_store,K_real8,K_SPIN,J1,0,SPIN(J1))
        DOM = (spin1 - meanmotion)/meanmotion
        if(rank == 0)WRITE (6,26)  NAME(J1), Q1, QC, DOM
...
!     Initialize KS for perturbed motion (cf. backwards step in KSPERI).
    ...
    !     Ensure unperturbed motion for small GAMMA.
        IF (GAMMA(IPAIR) < 1.0D-10 .AND. ES0 < 0.95) THEN
            LIST(1,I1) = 0
        END IF
        IF (GAMMA(IPAIR) < 5.0D-07 .AND. ES0 < 0.3) THEN
            LIST(1,I1) = 0
        END IF
        CALL RESOLV(IPAIR,1)
        IMOD = KSLOW(IPAIR)
...
!     Rectify the orbit to yield consistent variables (only at end).
    ...
    !     Deform the orbit to small eccentricity (H = const).
        IF (ECC > ECCM) THEN
            ...
            if(rank == 0)WRITE (6,22)  ECC, ECCM, TIME-TIME0, SEMI, &
    ...
    !     Determine indices for primary & secondary star (donor & accretor).
        IF (BODY(I1)/RADIUS(I1)**3 < BODY(I2)/RADIUS(I2)**3) THEN
            J1 = I1
            J2 = I2
        ELSE
            J2 = I1
...
!     Include occasional diagnostics of spiral evolution (every output).
    IF (ABS(TIME - TADJ) < STEP(I1)) THEN
        IF (RP1 < 3.0 .AND. (ECC < 0.1 .OR. KSTAR(J1) >= 3)) THEN
            NP = LIST(1,I1)
            TK = DAYS*SEMI*SQRT(SEMI/BODY(I))
            if(rank == 0) &
...
!     Include enforced circularization for difficult cases.
    IF ((ECC < 0.6 .AND. GAMMA(IPAIR) < 2.0D-07) .OR. &
    (ECC < 0.8 .AND. GAMMA(IPAIR) < 1.0D-08) .OR. &
    (ECC < 0.95 .AND. GAMMA(IPAIR) < 1.0D-09)) THEN
        ICIRC = 0
        CALL TCIRC(QPERI,ECC,I1,I2,ICIRC,TC)
        IF (TC > 200.0 .OR. (TC > 10 .AND. ECC < 0.7)) THEN
            CALL KSPERI(IPAIR)
            CALL KSAPO(IPAIR)
            CALL DEFORM(IPAIR,ECC,ECCM)
            KSTAR(I) = 10
...
!     Set standard binary on large ECC and TC if small GAMMA (large omeq).
    IF (ECC > 0.95 .AND. GAMMA(IPAIR) < 1.0D-09) THEN
        ICIRC = 0
        CALL TCIRC(QPERI,ECC,I1,I2,ICIRC,TC)
        IF (TC > 1.0D+04) THEN
            KSTAR(I) = 0
        !     ks MPI communication KSTAR
            call ksparmpi(K_store,K_int,K_KSTAR,I,0,KSTAR(I))
            IREM = IC
            if(rank == 0)WRITE (6,29)  NAME(I1), LIST(1,I1), ECC, TC
        ...
        IF (NMERGE > 0) THEN
            DO 48 JPAIR = 1,NPAIRS
                IF (NAME(N+JPAIR) < 0) THEN
                    IF (NAMEC(J) == NZERO + NAME(2*JPAIR-1) .OR. &
                    NAMEC(J) == NZERO + NAME(2*JPAIR) .OR. &
                    NAMEC(J) < -2*NZERO .OR. &
                    NAMEC(J) == NAME(2*JPAIR)) THEN
                    !                     if(rank.eq.0)
