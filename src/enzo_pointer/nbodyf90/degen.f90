...
!       Skip output for start and end of merger.
    IF (IPHASE == 6 .OR. IPHASE == 7) GO TO 50
    IF (ICASE == 7) GO TO 40
...
!       See whether KS binaries contain any degenerate stars.
    ...
    DO 1 IPAIR = I1,I2
        J2 = 2*IPAIR
        J1 = J2 - 1
        IF (KSTAR(J1) > 9 .OR. KSTAR(J2) > 9) THEN
            NB = NB + 1
        END IF
    1 END DO

!       Open unit #4 the first time.
    IF (NB > 0 .AND. FIRST .AND. KSTART == 1) THEN
...
!       Form binding energy and central distance for degenerate stars.
    ...
    DO 20 IPAIR = I1,I2
        J2 = 2*IPAIR
        J1 = J2 - 1
        IF (KSTAR(J1) < 10 .AND. KSTAR(J2) < 10) GO TO 20
        JPAIR = JPAIR + 1
        ICM = N + IPAIR
    ...
    !       Avoid division by zero for merged or synchronous ghost binary.
        ...
        !       Set zero eccentricity after common envelope stage (still large R).
            IF (R(IPAIR) > 2.0*SEMI(JPAIR)) THEN
                ECC(JPAIR) = 0.d0
            END IF
    ...
    !          RATIO = MIN(RATIO,99.9d0)
        IF (SEMI(JPAIR) < 0.0 .AND. RATIO > 5.0) GO TO 20
        SEMI(JPAIR) = SEMI(JPAIR)*RBAR*AU
        IF (IPRINT == 0 .AND. ICASE == 0) THEN
...
!       Search for neutron stars at main output. (seem never used)
    IF (ICASE == 0) THEN
        DO 30 J = 1,N
            ...
            !                  CALL FLUSH(33)
            !                  end if
            ELSE IF (KSTAR(J) > 13) THEN
                IF (J < IFIRST) THEN
                    JCM = N + KVEC(J)
                    IF (NAME(JCM) < 0) GO TO 30
                END IF
                IF (FIRST3) THEN
