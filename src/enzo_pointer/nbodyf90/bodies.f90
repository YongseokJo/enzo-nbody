...
!       Optional search for soft binaries (frequency NFIX with KZ(6) = 4).
    IF (KZ(6) == 0) GO TO 70
    IF (KZ(6) == 2) GO TO 50
    IF (KZ(6) == 3 .AND. NPRINT /= 1) GO TO 50
    IF (KZ(6) == 4 .AND. NPRINT /= 1) GO TO 70
    SIMAX = 0.01*TCR

    DO 40 I = IFIRST,NTOT
        IF (STEP(I) > SIMAX) GO TO 40
        JMIN = 0
        RJMIN2 = RSCALE**2
        NNB = LIST(1,I)
        DO 30 L = 1,NNB
            J = LIST(L+1,I)
            IF (STEP(J) > SIMAX) GO TO 30
            A1 = X(1,I) - X(1,J)
            A2 = X(2,I) - X(2,J)
            A3 = X(3,I) - X(3,J)
            RIJ2 = A1**2 + A2**2 + A3**2
...
!       Output of regularized binaries (frequency NFIX with KZ(6) = 4).
    50 DO 60 JPAIR = 1,NPAIRS
        IF (H(JPAIR) >= 0.0) GO TO 60
        I = 2*JPAIR - 1
        ICM = N + JPAIR
        JMIN = I + 1
        IF (BODY(I) <= 0.0D0) GO TO 60
