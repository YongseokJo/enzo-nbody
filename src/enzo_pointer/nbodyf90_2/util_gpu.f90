...
!     calculate the pot, a & jerk on the GPU
    ...
    !     Save new neighbour list on intermediate vector for regint
        ...
        !$$$#ifdef SIMD
        !$$$            CALL IRR_SIMD_SET_LIST(IDI,LISTGP(1,II))
        !$$$#endif
        END DO
        #else
    ...
    !$$$         CALL IRR_SIMD_FIRR_VEC(TIME,NI,IREG(L),FI_HOST,FID_HOST)
    !     --04/26/14 11:13-lwang-debug--------------------------------------*
    !**** Note:------------------------------------------------------------**
    !$$$         write(6,*) 'L L',IREG(L),'N',NAME(IREG(L)),'FI',FI_HOST(1:3,1),
    !$$$     &        'FD',FID_HOST(1:3,1)
    !$$$         call flush(6)
    !     --04/26/14 11:13-lwang-end----------------------------------------*
    !$$$#else
    !     Evaluate current irregular forces
    !$$$         CALL regint_gpu(ni,L,ireg,FI_HOST,FID_HOST,LISTGP)
    !     --04/26/14 11:13-lwang-debug--------------------------------------*
    !**** Note:------------------------------------------------------------**
    !$$$         write(6,*) 'H L',IREG(L),'N',NAME(IREG(L)),'FI',FI_HOST(1:3,1),
    !$$$     &        'FD',FID_HOST(1:3,1)
    !$$$         call flush(6)
    !$$$         STOP
    !     --04/26/14 11:13-lwang-end----------------------------------------*
    ...
    !!$omp parallel do if(ni.GE.ithread) private(II,I)
        ...
        !**** Note:------------------------------------------------------------**
        !$$$            IF(NAME(I).EQ.8317.AND.TIME.EQ.1.32592773437500000) THEN
        !$$$               write(102+rank,*) 'I',I,'N',NAME(I),'FR',GPU_A(1,II),
        !$$$     &              'FRD',GPU_JERK(1,II),'FI',FI_HOST(1,II),
        !$$$     &              'FID',FID_HOST(1,II),'T',TIME
        !$$$               DO III = IFIRST,NTOT
        !$$$                  write(102+rank,*) 'III',III,'N',NAME(III),'X',X(1,III)
        !$$$     &                 ,'XD',XDOT(1,III),'T',TIME,'F',F(1,III),
        !$$$     &                 'FD',FDOT(1,III),'X0',X0(1,III),'X0D',
        !$$$     ^                 X0DOT(1,III)
        !$$$               END DO
        !$$$               call flush(102+rank)
        !$$$            END IF
        ...
        !$$$            IF(ICFLAG.GT.0.AND..NOT.FLAG_P) THEN
    ...
    END SUBROUTINE CALC_REG_ON_GPU
