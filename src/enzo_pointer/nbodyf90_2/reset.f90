...
!       Locate current position in the merger table.
    IMERGE = 0
    DO 1 K = 1,NMERGE
        IF (NAMEM(K) == NAME(I)) IMERGE = K
    1 END DO

!       Check optional diagnostics for hierarchy.
    IF ((KZ(18) == 1 .OR. KZ(18) == 3) .AND. KSTARM(IMERGE) <= 20) THEN
        CALL HIARCH(IPAIR)
    END IF
...
!       Find the nearest neighbour and reduce steps of active perturbers.
    ...
    !       Identify the closest perturber for dominant force calculation.
        IF (RIJ2 < RJMIN2 .AND. J /= JCOMP) THEN
            RJMIN2 = RIJ2
            JMIN = J
        END IF
...
!       Note that ghost may be saved in an old binary c.m. (search NTOT).
    DO 10 I = 1,NTOT
        IF (BODY(I) == 0.0D0 .AND. NAME(I) == NAMEG(IMERGE)) JCOMP = I
    10 END DO

!       Regularize two-body configuration if JCOMP cannot be identified.
    IF (JCOMP == JCOMP1) THEN
...
!       Ensure that any rare case of missing second component is included.
    DO 30 I = 1,NTOT
        NNB1 = LIST(1,I) + 1
        DO 25 L = 2,NNB1
            IF (LIST(L,I) > ICOMP) GO TO 30
        
        !       Now see whether #JCOMP has already been added.
            DO 22 K = L,NNB1
                IF (LIST(K,I) == JCOMP) GO TO 30
            22 END DO
...
!       Update look-up times & radii and check possible Roche condition.
    ...
    !       Reduce evolution time-scale after any delay during ghost stage.
        IF (TEV(2*JPAIR-1) < TIME + 0.01*TCR) TEV(2*JPAIR-1) = TIME
        IF (TEV(2*JPAIR) < TIME + 0.01*TCR) TEV(2*JPAIR) = TIME
        IF (KSTAR(JCOMP1) > 0 .AND. KSTAR(JCOMP1) <= 10) THEN
...
!       Check Roche look-up time.
    IF (KSTAR(NTOT) >= 10 .AND. KSTAR(NTOT) <= 20) THEN
    !       Reduce evolution time-scale after any delay during ghost stage.
        IF (TEV(2*NPAIRS-1) < TIME + 0.01*TCR) TEV(2*NPAIRS-1) = TIME
...
!       Examine merger list for possible escapers (retain up to 3 levels).
    DO 90 L = 1,NMERGE
        DO 85 J = 1,NPAIRS
            IF (NAMEM(L) == NAME(N+J) .OR. &
            NAMEM(L) == NAME(N+J) + 2*NZERO .OR. &
            NAMEM(L) == NAME(N+J) + 4*NZERO) GO TO 90
        85 END DO
