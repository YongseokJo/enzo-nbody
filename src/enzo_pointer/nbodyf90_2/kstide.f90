...
!       Distinguish between sequential circularization, standard chaos & GR.
    ...
    !       Skip if penetration is not significant (prevents frequent calls).
        IF (ABS(QPERI - ZF*R1) < 0.01*QPERI) GO TO 50
    ...
    !       Update KS variables for CHAOS or TERMINATED CHAOS (H > 0).
        IF (KSTAR(I) == -1 .OR. H(IPAIR) > 0.0) THEN
...
!       Print first and last energy change and update indicator.
    IF (KZ(27) == 1 .AND. (KSTAR(I) == 0 .OR. KSTAR(I) == 9)) THEN
        P = DAYS*SEMI1*SQRT(SEMI1/BODY(I))
        IF (KSTAR(I) == 0 .AND. ECC1 > ECCM) THEN
            WHICH1 = 'NEW CIRC'
            KSTAR(I) = 9
        !     ks MPI communication TEV
            call ksparmpi(K_store,K_int,K_KSTAR,I,0,KSTAR(I))
...
!       Form new perturber list after significant energy loss.
    NP0 = LIST(1,I1)
    IF (ABS(SEMI1/SEMI) < 0.5) THEN
        CALL KSLIST(IPAIR)
    !       Ensure that single perturber differs from #I itself.
        IF (NP0 == 0 .AND. LIST(1,I1) > 0) THEN
            IF (LIST(2,I1) == I) THEN
                LIST(2,I1) = IFIRST
            END IF
        END IF
...
!       Ensure chaos treatment at every pericentre by perturbed motion.
    IF (KZ(27) == 2 .AND. LIST(1,I1) == 0) THEN
        LIST(1,I1) = 1
        LIST(2,I1) = N
        NP0 = 1
    END IF
...
!       Produce diagnostic output for interesting new case (first time).
    IF (ECC > 0.99 .AND. ABS(ECC - ECC1) > 0.01 .AND. IONE == 0) THEN
        if(rank == 0) &
        ...
        IONE = IONE + 1
    END IF
    IF (TIME + TOFF > TTIDE + DTADJ) IONE = 0

!       Check for hierarchical configuration with eccentric inner binary.
    IF (KZ(27) == 2 .AND. ECC > 0.95 .AND. HI < 0.0) THEN
        NP1 = LIST(1,I1) + 1
        DO 30 L = 2,NP1
            J = LIST(L,I1)
        !              CALL JPRED(J,TIME,TIME)
...
!       Count any hyperbolic captures.
    IF (SEMI < 0.0 .AND. SEMI1 > 0.0) THEN
        NTIDE = NTIDE + 1
        QPS = QPERI/R1
        if(rank == 0) &
...
!       Record diagnostics for new synchronous orbit and activate indicator.
    IF (ECC > ECCM .AND. ECC1 < ECCM .AND. KZ(27) <= 2) THEN
        NSYNC = NSYNC + 1
...
!       See whether a low-eccentricity synchronous state has been reached.
    ...
    IF (ABS(SEMI1) < 1.5*RCOLL .AND. ECC < ECCM .AND. &
    KSTAR(I) < 10) THEN
        KSTAR(I) = 10
    !     ks MPI communication TEV
        call ksparmpi(K_store,K_int,K_KSTAR,I,0,KSTAR(I))
...
!       Include warning in case of eccentricity increase (PT only).
    ...
    IF (KZ(27) == 1 .AND. ECC2 > MAX(ECC,ECCM)+1.0D-04) THEN
        if(rank == 0) &
...
!       Reduce radius by DR/R to delay dissipation for small energy loss.
    IF (KZ(27) == 1 .AND. DE(1)+DE(2) < 1.0D-07*ZMU*ABS(HI)) THEN
        J1 = I1
