...
!       Exit if physical radius is smaller than Roche radius.
    ...
    !       other and Roche occurs in the meantime!
        ...
        !            DMA(K) = MIN(DMA(K),0.8D0*DMR(3-K))
            ...
            IF(KW <= 2 .OR. KW == 7)THEN
                ...
                IF(KW == 2)THEN
                    IF(GB(9) < MASSC(K) .OR. M01 > ZPARS(3))THEN
                        MASS0(K) = M01
                    ELSE
    ...
    !       Define first/second Roche overflow stage and increase event counter.
        IF (KSTAR(I) <= 10 .OR. &
        (KSTAR(I) > 10 .AND. MOD(KSTAR(I),2) == 0)) THEN
            KSTAR(I) = KSTAR(I) + 1
            NRO = NRO + 1
            INEW = 1
            if(rank == 0) &
            ...
            IF(rank == 0 .AND. FIRST .AND. KSTART == 1)THEN
    ...
    !       (Note: use old orbital expressions)
        IF (SEP < 10.0) THEN
            ...
            DTM = MIN(DTGR,DTM)
            IF (KSTAR(J1) <= 1 .AND. &
            MASS(1) > 0.35 .AND. MASS(1) < 1.25) THEN
...
! Adopt Kelvin-Helmholz time from the modified classical expression.
    ...
    DO 15 K = 1,2
        ...
        IF(KSTAR(J) <= 1 .OR. KSTAR(J) == 7 .OR. KSTAR(J) >= 10)THEN
            TKH(K) = TKH(K)*MASS(K)
        ELSE
...
! Identify special cases.
    ...
    IF(KSTAR(J1) == 2)THEN
        QC = 4.D0
    ELSEIF(KSTAR(J1) == 3 .OR. KSTAR(J1) == 5 .OR. KSTAR(J1) == 6)THEN
    ...
    ELSEIF(KSTAR(J1) == 8 .OR. KSTAR(J1) == 9)THEN
        QC = 0.784D0
    ELSE
        QC = 3.D0
    ENDIF

    IF(KSTAR(J1) == 0 .AND. Q(1) > 0.695)THEN
    ...
    ! evolution. The result is always a single star.
        ...
        ! otherwise it will become younger still.
            IF(MASS(2) < 0.35 .OR. MASS(2) > 1.25)THEN
        ...
        ! Check for planets or low-mass WDs.
            ...
            IF((KSTAR(J2) == 10 .AND. MASS(2) < 0.05D0) .OR. &
            (KSTAR(J2) >= 11 .AND. MASS(2) < 0.5D0))THEN
                KST = KSTAR(J1)
                MASS(1) = MASS(2) + DM2
                MASS(2) = 0.D0
            ELSE
        ...
        ! The neutron star or black hole simply accretes at the Eddington rate.
            ...
            IF(DM2 < DM1) SUPEDD = .TRUE. 
            MASS(2) = MASS(2) + DM2
        ENDIF
        COALS = .TRUE.
        IF(MASS(2) > 0.D0) MASS(1) = 0.D0
        KW1 = KSTAR(J2)
        GOTO 60
    ELSEIF(((ABS(ABS(2*KSTAR(J1)-11)-3) == 2 .OR. KSTAR(J1) == 9). &
        AND.(Q(1) > QC .OR. RADX(1) <= RADC(1))) .OR. &
        (KSTAR(J1) == 2 .AND. Q(1) > QC) .OR. &
        (KSTAR(J1) == 4 .AND. Q(1) > QC))THEN
    ...
    ! Next step should be made without changing the time.
        ...
        IF(COALS)THEN
            GOTO 60
        ELSE
            IF(KSTAR(J1) < 13 .AND. (KW1 == 13 .OR. KW1 == 14))THEN
                JKICK = J1
                KWK = KW1
            ENDIF
            IF (KSTAR(J2) < 13 .AND. (KW2 == 13 .OR. KW2 == 14))THEN
                JKICK = J2
                KWK = KW2
            ENDIF
            IF(KSTAR(J1) < 10 .AND. KW1 >= 10 .AND. KW1 <= 12)THEN
                JKICK = J1
                KWK = KW1
            ENDIF
            IF(KSTAR(J2) < 10 .AND. KW2 >= 10 .AND. KW2 <= 12)THEN
                JKICK = J2
                KWK = KW2
            ENDIF
            KSTAR(J1) = KW1
    ...
    ELSEIF(KSTAR(J1) >= 10 .AND. KSTAR(J1) <= 12 .AND. Q(1) >= 0.628)THEN
    
    ! Dynamic transfer from a white dwarf.  Secondary will have KW > 9.
    
        TAUM = SQRT(TKH(1)*TDYN)
        DM1 = MASS(1)
        IF(EDDFAC < 10.D0)THEN
            DM2 = MIN(DME*TAUM/TB,DM1)
            IF(DM2 < DM1) SUPEDD = .TRUE. 
        ELSE
            DM2 = DM1
        ...
        ! is enough to destroy the star.
            ...
            KSTAR(J2) = 15
            MASS(2) = 0.D0
        ELSEIF(KSTAR(J1) == 10 .OR. KSTAR(J2) == 10)THEN
    ...
    !       See whether Chandrasekhar's mass is exceeded (SN).
        IF(KSTAR(J2) <= 11 .AND. MASS(2) > MCH)THEN
            if(rank == 0) &
    ...
    ! to the dynamical rate for all others.
        ...
        IF(KSTAR(J1) >= 2 .AND. KSTAR(J1) <= 9 .AND. KSTAR(J1) /= 7)THEN
            DM1 = MIN(DM1,MASS(1)*TB/TKH(1))
        ELSEIF(RAD(1) > 10.D0*ROL(1) .OR. (KSTAR(J1) <= 1 .AND. &
            KSTAR(J2) <= 1 .AND. Q(1) > QC))THEN
    ...
    ! be overly restrictive for long lived phases.
        ...
        IF(ITER <= 100) DTM = MIN(DTM,DTMI(1),DTMI(2))
    ...
    !       Decide between accreted mass by secondary and/or system mass loss.
        ...
        ! or SAGB star unless the primary is also a helium star.
            ...
            ELSE
                ...
                IF(DMCHK > 0.D0 .AND. DM2/MASS(2) > 1.0D-04)THEN
                ...
                ENDIF
            ENDIF
        ELSEIF(KSTAR(J1) <= 6 .AND. &
            (KSTAR(J2) >= 10 .AND. KSTAR(J2) <= 12))THEN
        
        ! White dwarf secondary.
        
            IF(DM1/TB < 2.71D-07)THEN
                IF(DM1/TB < 1.03D-07)THEN
        ...
        IF(KST >= 10 .AND. KST <= 12)THEN
            MT2 = MASS(2) + KM*(DM22 - DMS(2))
            IF(KSTAR(J1) <= 10 .AND. KST == 10 .AND. MT2 >= 0.7)THEN
            ...
            ! it is destroyed in a possible Type 1a SN.
                ...
                COALS = .TRUE.
                GOTO 60
            ELSEIF(KSTAR(J1) <= 10 .AND. KST >= 11)THEN
        ...
        ! convert to ONe (C. Tout 21/11/08).
            ...
            IF(KST == 11 .AND. DM22 > 0.4D0*DME/EDDFAC) KST = 12
            IF((KST == 10 .OR. KST == 11) .AND. MT2 >= MCH)THEN
    ...
    ! the orbital angular momentum.
        ...
        IF(SUPEDD .OR. NOVAE .OR. GAMM1 < -1.5D0)THEN
            DJORB = DJORB + (DM1 - DM22)*MASS(1)*MASS(1)* &
            SEP*SEP*OORB/(MASS(1)+MASS(2))**2
        ELSEIF(GAMM1 >= 0.D0)THEN
    ...
    ! For very close systems include gravitational radiation.
        ...
        !           IF(IGR.LT.10.OR.ABS(DJGR)/JORB.GT.0.001)THEN
            IF(SEP < 1.05*(RAD(1) + RAD(2)))THEN
                IF (rank == 0 .AND. MOD(IGR,10) == 0) &
        ...
        DMS(1) = KM*DMS(1)
        IF(KSTAR(J1) < 10) DMS(1) = MIN(DMS(1),MASS(1) - MASSC(1))
        DMS(2) = KM*DMS(2)
        IF(KSTAR(J2) < 10) DMS(2) = MIN(DMS(2),MASS(2) - MASSC(2))
    ...
    ! Calculate spin changes owing to stellar winds and magnetic braking.
        ...
        IF(KSTAR(J1) < 10 .AND. MASS(1) >= 0.35)THEN
            CALL MAGBRK(KSTAR(J1),MASS(1),MENV(1),RAD(1),OSPIN(1),DJMB)
            DJSPIN(1) = DJSPIN(1) + DJMB*DTM0
            IMB = IMB + 1
            IF(JSPIN(1) > 0.D0)THEN
                IF(IMB < 2 .OR. ABS(DJMB)/JSPIN(1) > 0.03)THEN
                    if(rank == 0) &
            ...
            ENDIF
        ENDIF
        IF(KSTAR(J2) < 10 .AND. MASS(2) >= 0.35)THEN
            CALL MAGBRK(KSTAR(J2),MASS(2),MENV(2),RAD(2),OSPIN(2),DJMB)
            DJSPIN(2) = DJSPIN(2) + DJMB*DTM0
            IMB = IMB + 1
            IF(JSPIN(2) > 0.D0)THEN
                IF(IMB < 10 .OR. ABS(DJMB)/JSPIN(2) > 0.001)THEN
                    if(rank == 0) &
                    WRITE (6,40)  MASS, SEP, DJMB, DTM
                ENDIF
            ENDIF
    ...
    ! Calculate any changes owing to tidal evolution (includes eccentricity).
        ...
        DELET = 0.D0
        DO 500 K = 1,2
            IF((KSTAR(J) <= 9 .AND. RAD(K) >= 0.01D0*ROL(K)) .OR. &
            (KSTAR(J) >= 10 .AND. J == J1))THEN
                ...
                IF(DTM0 > 0.D0 .AND. ABS(DSPIN) >= TINY)THEN
                    DSPIN0 = DSPIN
                    IF(DSPIN >= 0.D0)THEN
    ...
    ! Update the initial and current masses.
        ...
        IF(KSTAR(J1) <= 1 .OR. KSTAR(J1) == 7) MASS0(1) = MASS(1)
        MASS(2) = MASS(2) + DM22 - DMS(2)
        IF(KSTAR(J2) <= 1 .OR. KSTAR(J2) == 7) MASS0(2) = MASS(2)
    ...
    ! For a HG star check if the initial mass can be reduced.
        ...
        DO 502 K = 1,2
            ...
            IF(JSPIN(K) > JSPBRU)THEN
                DJORB = DJORB - (JSPIN(K) - JSPBRU)
                JSPIN(K) = JSPBRU
            ENDIF
            IF(KSTAR(J) == 2 .AND. MASS0(K) <= ZPARS(3))THEN
                M01 = MASS0(K)
                MASS0(K) = MASS(K)
                CALL star(KSTAR(J),MASS0(K),MASS(K),TMSNEW,TN,TSCLS, &
                LUMS,GB,ZPARS)
                IF(GB(9) < MASSC(K)) MASS0(K) = M01
            ENDIF
            J = J2
        502 END DO
...
! Rejuvenate secondary and age primary if they are on the main sequence.
    ...
    IF(KSTAR(J1) <= 2 .OR. KSTAR(J1) == 7)THEN
    ...
    IF(KSTAR(J2) <= 2 .OR. KSTAR(J2) == 7)THEN
        CALL star(KSTAR(J2),MASS0(2),MASS(2),TM,TN, &
        TSCLS,LUMS,GB,ZPARS)
        IF(KSTAR(J2) == 2)THEN
            AJ(2) = TM + (TSCLS(1)-TM)*(AJ(2)-TMS(2))/(TBGB(2)-TMS(2))
        ELSEIF((MASS(2) < 0.35 .OR. MASS(2) > 1.25) .AND. &
            KSTAR(J2) /= 7)THEN
    ...
    !       Check for blue straggler formation (TM < TPHYS).
        IF(KSTAR(J2) <= 1)THEN
            IF(TMS(2) > TPHYS .AND. TM < TPHYS)THEN
                NBR = NBR + 1
                if(rank == 0) &
...
!       Include mass loss from system.
    ...
    !       Correct neighbour forces and obtain the corresponding energy loss.
        IF (DMT*SMU < 0.05) THEN
            CALL ficorr(I,DMT)
        ELSE
            CALL fcorr(I,DMT,0)
        END IF
        ZMASS = ZMASS - DMT
    ELSE IF (ABS(SEMI - SEMI0) > 1.0D-10*SEMI) THEN
...
!       Check termination by coalescence.
    ...
    !       unless both stars heve zero MASS.
        II = J2
        K = 2
        IF(MASS(1) == 0.0 .AND. MASS(2) /= 0.0)THEN
            II = J1
            K = 1
        ENDIF
        BODY0(II) = 0.D0
...
! Obtain the stellar parameters for the next step (first J1, then J2).
    ...
    !       will change suddenly.
        ...
        IF((KSTAR(J) < 13 .AND. KW >= 13) .OR. &
        (KSTAR(J) < 10 .AND. KW >= 10 .AND. KW <= 12))THEN
            IQ = J
...
!**
    ...
    !       Force co-rotation of primary with orbit if necessary.
        ...
        FAC = OSPIN(1)/OORB
        IF(FAC > 1.D0) FAC = 1.D0/FAC
    
        IF((ECC-0.001D0) > TINY .OR. FAC < 0.98D0)THEN
        ...
        ! Include magnetic braking.
            ...
            ELSE
                DELET = 0.D0
            ENDIF
            IF(ABS(DJSPIN(1)) > 1.0D-14)THEN
                IF(ABS(DSPINK(1)) > 1.0D-20)THEN
            ...
            !     Impose 2.5 % limit on change of JORB (Chris Tout 1/2013).
                IF (ABS(DJORB) > 0.1*JORB .AND. ITRO <= 5) THEN
                    DTM0 = 0.1*JORB*DTM0/ABS(DJORB)
                    DELET = 0.1*JORB*DELET/ABS(DJORB)
                    ITRO = ITRO + 1
                    IF (JORB > 0.0D0) WRITE (6,83)  DJORB/JORB, JORB
            ...
            !     --02/28/13 9:59-lwang-end-add------------------------------------*
                ...
                OORB = TWOPI/TB
                ITERB = ITERB + 1
                IF(ITERB <= 100) GOTO 80
            ELSE
                IF(rank == 0 .AND. ECC > 0.02D0)THEN
                    WRITE(6,*)' WARNING: will not circularize'
                ENDIF
                IF(FAC < 0.98D0 .AND. KSTAR(J1) < 10)THEN
                    if(rank == 0) &
...
!       Check if components should be saved for output, cf. HRPLOT
    J = J1
    IF(TEV(J) >= TPLOT .AND. .NOT. ISAVE)THEN
        ISAVE = .TRUE.
    !       Check if components are already saved (temporary)
        II = 0
        DO 700 K = 1,NRSAVE
            IF(NAME(J1) == NAMER(1,K)) II = NAME(J1)
            IF(NAME(J1) == NAMER(2,K)) II = NAME(J1)
        700 END DO
        IF(II == 0)THEN
            NRSAVE = NRSAVE + 1
            IF(NRSAVE > MMAX)THEN
                IWARN = IWARN + 1
                IF (rank == 0 .AND. IWARN < 20) THEN
...
! See whether the primary still fills its Roche lobe.
    ...
    ! Test for a contact system.
        ...
        IF (RAD(2) > ROL(2) .AND. ITER > 0) THEN
            NCONT = NCONT + 1
            if(rank == 0) &
        ...
        IF (IWARN < 100 .AND. RAD(1) > 2.0*ROL(1)) THEN
            IWARN = IWARN + 1
            if(rank == 0) &
    ...
    !       Produce diagnostics for cases involving degenerate objects.
        IF (ITER == 1 .AND. INEW > 0 .AND. KSTAR(J2) >= 10) THEN
            IF (ABS(DTM) > 1.0D-20) THEN
        ...
        ITER = ITER + 1
        IF(TEV0(I) < TIME) GOTO 10
        IF (ITER == 1 .AND. GAMMA(IPAIR) < GMIN) GO TO 10
    ELSE
        if(rank == 0) &
    ...
    !       Check optional diagnostics for degenerate objects.
        IF(MAX(KSTAR(J1),KSTAR(J2)) >= 10)THEN
            IF(KZ(9) >= 3)THEN
            ...
            ENDIF
        ENDIF
        IF (KSTAR(J1) >= 10 .AND. KSTAR(J1) <= 12) NWD = NWD + 1
...
!       Set new c.m. time only during active stage (otherwise by TRFLOW).
    IF (MOD(KSTAR(I),2) == 1 .AND. IQ == 0) THEN
        TEV(I) = TPHYS0/TSTAR
    ...
    !       Ensure that close degenerate system gets GR check in mdot.
        ...
        IF(KW > 6) TEV(J2) = MIN(TEV(J2),TIME+0.1D0)
