...
!       Dummy index to enable summation of c.m. or resolved components.
    ...
    !****     In jpred, unpert ks will not be resolved
        J1 = 2*J - 1
        IF(LIST(1,J1) == 0) CALL KSRES2(J,J1,J2,RIJ2,TIME)
        KDUM = J1
        K = KDUM
        32 A1 = X(1,K) - XI(1)
        A2 = X(2,K) - XI(2)
...
!**** Note:------------------------------------------------------------**
!$$$      if(i1.eq.1605) then
!$$$         write(125+rank,*)'kspert I1',I1,'IPAIR',ipair,'fp',fp(1),
!$$$     &        'fd',fd(1),'t',time
!$$$         call flush(125+rank)
!$$$      end if
...
!**** Note:------------------------------------------------------------**
!$$$      if(i1.eq.1605) then
!$$$         write(125+rank,*)'fchain I1',I1,'IPAIR',ipair,'fp',fp(1),
!$$$     &        'fd',fd(1),'t',time
!$$$         call flush(125+rank)
!$$$      end if
!     --03/14/14 15:29-lwang-end----------------------------------------*
