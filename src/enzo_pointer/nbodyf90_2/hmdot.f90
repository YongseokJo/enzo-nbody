...
!       Skip further modifications on zero mass loss of same type.
    IF (ABS(DMS/M1) == 0.0D0 .AND. KW == KSTAR(J)) GO TO 50
...
!       Check for possible circularization (tidal or sequential).
    RP = SEMI2*(1.0 - ECC)
    IF (KZ(27) > 0 .AND. RP < 4.0*RADIUS(J) .AND. ECC > 0.002 .AND. &
    KSTARM(IMERGE) >= 0) THEN
        TC = 1.0D+10
        IF (KZ(27) == 2) THEN
            I2 = 0
        ...
        ITERM = 1
        GO TO 50
    ELSE IF (ECC > 0.002) THEN
        IF (RP < 2.0*RADIUS(J)) THEN
            if(rank == 0) &
            WRITE (6,15)  NAME(J), KSTAR(J), KSTARM(IMERGE), ECC, DMS, &
            RP, RADIUS(J)
            ITERM = 1
...
!       Print some diagnostics on non-zero mass loss.
    IF (rank == 0 .AND. DMS > 1.0D-03) THEN
