...
!       Check hierarchical stability condition for bound close pair (RB > a).
    ...
    RB > SEMI) THEN
        IF (RDOT3 > 0.0 .AND. R3 > 3.0*SEMI*(1.0 + ECC1)) THEN
            ITERM = -1
            if(rank == 0) &
    ...
    ELSE IF (PMIN > 3.0*SEMI*(1.0 + ECC) .AND. SEMI > 0.0 .AND. &
        ECC1 < 0.9) THEN
    !       Wait for favourable configuration (R > SEMI, R3 > 3*SEMI & RDOT3 > 0.
        IF (RB > SEMI .AND. R3 > SEMI1 .AND. RDOT3 > 0.0) THEN
            APO = SEMI*(1.0 + ECC)
            if(rank == 0) &
    ...
    ELSE IF (NSTEP1 > 50000 .AND. PMIN > 0.9*PCRIT .AND. &
        R3 > SEMI1) THEN
        ITERM = -1
    ELSE IF (NSTEP1 > 500000 .AND. R3 > SEMI1) THEN
        ITERM = -1
    END IF

    RETURN

    END SUBROUTINE CHSTAB
