...
!       Decide between new and terminated merger.
    ...
    !       Ensure that unperturbed binary is resolved.
        ...
        IF (LIST(1,I1) == 0 .OR. X(1,I1) == X(1,I2)) THEN
            CALL RESOLV(IPAIR,1)
        END IF
        Q = BODY(I)/BODY(JCOMP)
    ...
    ELSE IF (IPHASE == 7) THEN
        PMIN = SEMI*(1.0D0 - ECC)/RAP
    !          PMIN = MIN(PMIN/RAP,999.0D0)
        IF (TK0(IL) <= 0.0D0) TK0(IL) = TK
        TK1 = TK/TK0(IL)
