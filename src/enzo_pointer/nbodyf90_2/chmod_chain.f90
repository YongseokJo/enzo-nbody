...
!       Include test on fast escaper approaching a perturber having RDOT > 0.
    ...
    !       Determine the smallest approaching intruder distance to chain member.
        ...
        !       See whether the intruder is approaching a chain member.
            IF (RDI < 0.0 .AND. RJX2 < RJX**2) THEN
                RJX = SQRT(RJX2)
                RDX = RDI/RJX
            END IF
        3 END DO
    ...
    !       Bypass RDOT < 0 test for approaching ejection candidate.
        IF (RDX < 0.0 .AND. RJX < 2.0*RSUM/FLOAT(NN-1)) THEN
            if(rank == 0) &
...
!       Include conditions for skipping (large RIJ & size or small GPERT).
    IF (RIJ > 3.0*MIN(RSUM,RMIN)) GO TO 10
    IF (RDOT > 0.0 .AND. GPERT < 0.05) GO TO 10
    IF (RSUM > 5.0*RMIN .AND. GPERT < 1.0) GO TO 10
!       Allow triple hierarchy subject to maximum membership of 6.
    5 IF (NN > 3 .AND. NAME(JCLOSE) < 0) GO TO 10
    IF (NN > 4 .AND. JCLOSE > N) GO TO 10
...
!       Widen the impact parameter test to be on safe side.
    IF (PMIN > 1.5*RSUM .AND. GPERT < 0.05) GO TO 10

!       Check option for increasing or decreasing regularization parameters.
    IF (KZ(16) > 2) THEN
        IF (GPERT > 0.05 .AND. NCH <= 4 .AND. KSMAG <= 5) THEN
            KSMAG = KSMAG + 1
            RMIN = 1.2*RMIN
            RMIN2 = RMIN**2
            DTMIN = 1.3*DTMIN
        ...
        !                  CALL FLUSH(77)
        !              END IF
        ELSE IF (GPERT < 0.001 .AND. KSMAG > 1) THEN
            KSMAX = KSMAG - 1
            RMIN = 0.83*RMIN
            RMIN2 = RMIN**2
            DTMIN = 0.77*DTMIN
...
!       Include additional criteria (V^2 > VC^2/2; JCL > N & RIJ < RSUM).
    ...
    IF ((RSUM + RIJ < RMIN) .OR. &
    (RIJ < 2.0*RMIN .AND. PMIN < RMIN) .OR. &
    (RIJ < RSUM .AND. RDOT**2 > 0.5*VC2) .OR. &
    (JCLOSE > N .AND. RIJ < RSUM) .OR. &
    (GPERT > 0.2 .AND. RDOT < 0.0)) THEN
    
    !       Do not allow multiple absorptions (NAMES(NMX,5) = 0 each new chain).
        IF (NAME(JCLOSE) /= NAMES(NMX,5) .AND. NAMES(NMX,5) == 0) THEN
            NAMES(NMX,5) = NAME(JCLOSE)
        !       Include possible return after ejection in bound orbit.
        ELSE IF (RDOT > 0.0) THEN
            GO TO 10
        ELSE IF (NN > 4 .AND. JCLOSE > N .AND. RSUM > 2.0*RMIN) THEN
            GO TO 10
        END IF
    ...
    !       Treat extreme binary as inert on significant slow-down.
        IF (JCLOSE > N .AND. FAC > 10.0) THEN
            ...
            8 END DO
            SMALL = 0.01*(RSUM - RIJ)
            IF (rank == 0 .AND. RM < SMALL .AND. SEMI < SMALL) THEN
    ...
    !          call flush(6)
    !$$$          IF(TIME.LT.TBLOCK) THEN
    !$$$             DTTB = TBLOCK - TIME
    !$$$             IF (DTTB.GT.1.0) then
    !$$$                print*,'Warning! Tblock - Time larger than 1.0'
    !$$$                call flush(6)
    !$$$                IDTTB = INT(DTTB)
    !$$$                DITTB = DTTB - FLOAT(IDTTB)
    !$$$                call STEPK(DITTB,DTTBN)
    !$$$                DTTBN = DTTBN + FLOAT(IDTTB)
    !$$$             else
    !$$$                call STEPK(DTTB,DTTBN)
    !$$$             end if
    !$$$             TBLOCK = TBLOCK - DTTBN
    !$$$          END IF
...
!       Try escape check if middle distance is largest.
    IF (KCASE == 0 .AND. NN >= 5 .AND. &
    1.0/RINV(ISORT(1)) > 2.0*RMIN) THEN
        R1 = 1.0/RINV(1)
        R2 = 1.0/RINV(NN-1)
...
!       Employ parabolic escape criterion (terminate if RI > RMIN & NCH < 5).
    IF ((RI > RESC .AND. RDOT > 0.0 .AND. RB*AINV > 0.99) .OR. &
    (HI > 0.0 .AND. RI > 3.0*RMIN)) THEN
    !       Decide between weakly bound and escape orbit (RMAXS may be large).
        IF (RDOT**2 < 2.0*BODY(ICH)/RI) THEN
        ...
        !       Split into two KS solutions if binaries are well separated.
            IF (NCH == 4 .AND. GB < 0.001) THEN
                KCASE = -1
                GO TO 50
            END IF
        !       Enforce termination if RI > MIN(RMAXS/2,RMIN) and NCH <= 4.
            IF (RI > MIN(0.5*RMAXS(ISUB),RMIN) .AND. NCH <= 4) THEN
                KCASE = -1
                GO TO 50
            !       Accept binary escape for small perturbation & V**2 > M/R (NCH > 4).
            ELSE IF (GB < 0.01 .AND. NCH > 4 .AND. &
                (RDOT**2 > BODY(ICH)/RI .OR. RI > RMIN)) THEN
                if(rank == 0) &
            ...
            !       Check splitting into two KS solutions (R1 + R3 < R2/5 & RDOT > VC).
                ...
                !       Ensure RSUM > 0.1*RMIN for reducing differential force corrections.
                    IF (R13 < 0.2/RINV(2) .AND. VC2 > 1.0 .AND. &
                    RSUM > 0.1*RMIN) THEN
                        KCASE = -1
                        GO TO 50
                    END IF
...
!       Note that arithmetic mean tends to delay escape.
    ...
    IF (NN == 3 .AND. RESC < 0.001*RMIN) RESC = 5.0*RESC
    IF (RM > RESC .AND. RDOT > 0.0) THEN
        IF (RDOT**2 < 2.0*BODY(ICH)/RI) THEN
        ...
        !       Do not permit large length contrast even for bound orbit (NN = 4).
            IF (NN == 4 .AND. GI < 0.01) THEN
                IB = ISORT(NN-1)
            !       Prevent NN = 4 termination near small pericentre (subject to safety).
                IF (1.0/RINV(IB) > 0.1*RGRAV .OR. RSUM > RMIN) THEN
                    KCASE = -1
                    GO TO 50
                ELSE
                    KCASE = 0
        ...
        !*     &            (NN.EQ.3.AND.GI.GT.0.1)) THEN
            ...
            !       Decide between termination or removal for sub-parabolic motion.
                ...
                !       Write E = E_b + E_1 with E_b = -m1*m2/2*a and E_1 = MU3*ER.
                    ...
                    RY = 1.0/RINV(IB)
                    IF (RY < 0.9*SEMI .AND. RI < 2.0*RMIN) THEN
                        if(rank == 0) &
        ...
        !       Note RGRAV = MIN(0.5*RSUM,RGRAV) is OK and RGRAV adjusted on QPMOD.
            IF (RY < 0.9*SEMI .AND. RI < 2.0*RMIN .AND. &
            RI < 50.0*SEMI) THEN
                if(rank == 0) &
                WRITE (6,31)  NSTEP1, RY/SEMI, RI, RDOT**2, &
                2.0*BODY(ICH)/RI, SEMI
    ...
    !       Check that escaper is well separated (ratio > 3).
        ...
        IF (RATIO < 3.0 .AND. RSUM < RMIN) THEN
            KCASE = 0
            GO TO 60
        END IF
