...
!      COMMON/NNBCOT/  nnbave
    ...
    INTEGER :: OUTCOUNT ! by sykim
    INTEGER :: OUTCNUM  ! by sykim
    DATA OUTCOUNT /1/ ! by sykim
    CHARACTER(27) :: OUTFILE
    CHARACTER(27) :: OUTTAIL
    CHARACTER(20) :: TCHAR

    CHARACTER(20) :: OUTFORM ! by sykim
    SAVE OUTCOUNT ! by sykim

    OUTCOUNT = OUTCOUNT + 1

!     Call Computation of Moments of Inertia (with Chr. Theis)
    IF(KZ(49) > 0) CALL ELLAN

!     Obtain energy error in case routine ADJUST not called recently.
    IF (TIME >= TADJ .OR. TIME <= 0.0D0) GO TO 10
...
!       Find smallest semi-major axis and count unperturbed KS pairs.
    DO 20 IPAIR = 1,NPAIRS
        ...
        IF (SEMI > 0.0) AMIN = MIN(AMIN,SEMI)
        IF (LIST(1,2*IPAIR-1) == 0) IUNP = IUNP + 1
        IF (NAME(N+IPAIR) < -2*NZERO) MULT = MULT + 1
    20 END DO
...
!       Perform time-step & neighbour statistics (NS is # single stars).
    ...
    DO 30 I = IFIRST,NTOT
        ...
        CMAX = MAX(CMAX,RHON)
        NNB = NNB + LIST(1,I)
        IF (I <= N .AND. BODY(I) > 0.0D0) NS = NS + 1
        SUM = SUM + BODY(I)**2
    30 END DO
    NS = NS - NSUB
...
!       Check print frequency indicator & optional model counter.
    NPRINT = NPRINT + 1
    IF (NPRINT > NFIX .OR. TIME <= 0.0D0) THEN
        NPRINT = 1
        IF (KZ(3) > 0) MODEL = MODEL + 1
    END IF
...
!** FlorentR - add KZ(14)=9 in case of mode B
    IF (KZ(14) == 3 .OR. (KZ(14) == 9 .AND. TTMODE == 0)) THEN
    !** FRenaud
        #else
        IF (KZ(14) == 3) THEN
            #endif
    ...
    !       Exit if error exceeds restart tolerance (TIME < TADJ means no CHECK).
        IF (ABS(ERROR) > 5.0*QE .AND. TIME < TADJ) GO TO 100
    
    !       Check optional analysis & output of KS binaries.
        IF (KZ(8) > 0 .AND. NPAIRS > 0) THEN
            CALL BINOUT
        END IF
    ...
    !       Check optional diagnostics of evolving stars.
        IF (KZ(12) > 0 .AND. TIME >= TPLOT) THEN
    ...
    !       Check optional writing of data on unit 3 (frequency NFIX).
        IF (KZ(3) == 0 .OR. NPRINT /= 1) GO TO 100
        ...
        !       Include prediction of unperturbed binaries (except ghosts).
            IF (LIST(1,2*IPAIR-1) == 0 .AND. BODY(N+IPAIR) > 0.0D0) THEN
                CALL RESOLV(IPAIR,1)
            END IF
        80 END DO
    ...
    !       Replace any ghosts by actual M, R & V (including 2 binaries).
        ...
        !     Determine merger & ghost index for negative c.m. name.
            ...
            !$$$            if(rank.eq.0)print*,' after findj t,j1,j2,jpair,j,im=',j1,
            !$$$     &           j2,jpair,j,im
    ...
    !        edited by sykim so that the file can be read from directly
        if(rank == -1)then
        ...
        !         WRITE (3)  (AS(K),K=1,NK),
        !         DO J = 1, NTOT
        !            WRITE (3,*) BODYS(J)*ZMBAR,XS(1:3,J)*RBAR,VS(1:3,J)*VSTAR,
        !     &           NAME(J)
        !         END DO
