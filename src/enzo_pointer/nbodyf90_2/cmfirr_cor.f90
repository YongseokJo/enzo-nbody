...
!       Form irregular force components for perturbed KS pair.
    ...
    !**** Note:------------------------------------------------------------**
    !$$$      if (time.eq.14.968991100788116.and.name(i).eq.18637) then
    !$$$         print*,rank,'ll',LL,'j',J,'k',k,'N',name(J),'FD',FD(1),
    !$$$     *        'FI',FIRR(1),'t',time,'xdot',xdot(1,j),'xidot',xdot(1,i)
    !$$$         call flush(6)
    !$$$      end if
