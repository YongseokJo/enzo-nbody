...
!     Particle number counts
    ...
    IF(KZ(7) == 1 .OR. KZ(7) == 2 .OR. KZ(7) == 4) THEN
    ...
    !     If massive black hole exist, exclude it from the rscale determination
        IF (KZ(24) == 1) ZMH = ZMH - BIMBH/ZMBAR
        I = 0
        1 I = I + 1
        IM = JLIST(I)
        ZM = ZM + BODY(IM)
...
!     VROT = mass weighted average rotational velocity projected in x-y plane within a shell
    ...
    DO 15 J = 1,NLENS
        IF(KZ(7) == 2 .OR. KZ(7) == 3 .OR. J == 1) THEN
            VROT(J) = 0.0D0
            VRAVE(J) = 0.0D0
            VAVE(1:3,J) = 0.0D0
            VTAVE(1:3,J) = 0.0D0
    ...
    !     Check whether mass within Langrangian radius is complete.
        IF (I < NP .AND. ZM < FLAGR(J)*ZMASS0) GO TO 20
...
!     Get Half mass radius
    IF(KZ(7) /= 1 .AND. KZ(7) /= 2 .AND. KZ(7) /= 4) &
...
!     Final average
    ...
    !  Determine the Lagrangian radii for singles and binaries separately.
        ...
        DO 17 J = 1, NLENS
            ...
            END IF
            IM = ISLIST(I)
            IF(BODY(IM) == 0.0D0) GO TO 21
            ZMS = ZMS + BODY(IM)
            IF (I < NSNGL .AND. ZMS < FLAGR(J)*ZSMASS0) GO TO 21
        ...
        DO 18 J = 1, NLENS
            ...
            END IF
            IM = IBLIST(I)
            IF(BODY(IM) == 0.0D0) GO TO 22
            ZMB = ZMB + BODY(IM)
            IF (I < NBIN .AND. ZMB < FLAGR(J)*ZBMASS0) GO TO 22
