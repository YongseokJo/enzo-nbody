...
! Set maximum NS mass depending on which NS mass prescription is used.
    mxns = mxns0
    if(nsflag == 1) mxns = mxns1

    mass0 = mass
    if(mass0 > 100.d0) mass = 100.d0
    mt0 = mt
    if(mt0 > 100.d0) mt = 100.d0
...
! Make evolutionary changes to stars that have not reached KW > 5.
    ...
    !        Either on MS or HG
        ...
        !           Star is on the HG
            ...
            mcx = mc
            if(mass <= zpars(2))then
                mc = mcgbf(lums(3),GB,lums(6))
            elseif(mass <= zpars(3))then
    ...
    elseif(aj < tbagb)then
    
    !       Core helium burning star.
    
        if(kw == 3 .AND. mass <= zpars(2))then
            mass = mt
            CALL star(kw,mass,mt,tm,tn,tscls,lums,GB,zpars)
            aj = tscls(2)
        endif
        ...
        ! consists of a blue phase (before tloop) and a RG phase (after tloop).
            ...
            if(aj < tloop)then
                ...
                ry = ragbf(mt,ly,zpars(2))
                taul = 0.d0
                if(ABS(rmin-rx) > tiny)then
                    taul = (log(rx/rmin))**(1.d0/3.d0)
                endif
                tauh = 0.d0
                if(ry > rmin) tauh = (log(ry/rmin))**(1.d0/3.d0)
    ...
    ! is caught by the C core mass and they grow together.
        ...
        ! for SAGB stars (Jarrod, 2014)
            ...
            if(aj >= tscls(11)-2.d0*tiny) aj = tscls(14) + &
    ...
    ! exceeds either the total mass or the maximum allowed core mass.
        ...
        if(mcmax-mcx < tiny)then
            aj = 0.d0
            mc = mcmax
            if(mc < mch)then
                mt = mc
                if(mcbagb < 1.6d0)then
    ...
    if(kw >= 10 .AND. kw <= 12)then
    
    !        White dwarf.
    
        mc = mt
        if(mc >= mch .AND. kw == 12)then
        ...
        ! unless WD is ONe.
            ...
            mt = 1.3d0
        !           mt = 1.17d0 + 0.09d0*mc
        elseif(mc >= (mch-0.06d0) .AND. kw <= 11)then
            kw = 15
            aj = 0.d0
            mt = 0.d0
            lum = 1.0d-10
            ...
            if(mt < 0.000005d0)then
                r = 0.009d0
            elseif(mt < 0.0005d0)then
                r = 0.09d0
            else
...
! remnant that the star will become.
    ...
    elseif(kw == 5)then
        kwp = 9
        if(tn > tbagb) tau = 3.d0*(aj-tbagb)/(tn-tbagb)
        CALL star(kwp,mc,mc,tm,tn,tscls,lums,GB,zpars)
        lx = lmcgbf(mcx,GB)
        if(tau < 1.d0) lx = lums(2)*(lx/lums(2))**tau
        rx = rzhef(mc)
...
! Perturb the luminosity and radius due to small envelope mass.
    ...
    if(kw >= 2 .AND. kw <= 9 .AND. kw /= 7)then
        mew = ((mt-mc)/mt)*MIN(5.d0,MAX(1.2d0,(lum/lum0)**kap))
        if(kw >= 8) mew = ((mtc-mc)/mtc)*5.d0
        if(mew < 1.d0)then
            xx = lpertf(mt,mew)
            lum = lx*(lum/lx)**xx
            if(r <= rx)then
